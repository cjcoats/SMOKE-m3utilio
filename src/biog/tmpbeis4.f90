...
PROGRAM TMPBEIS4
    ...
    !.........  INCLUDES:
    INCLUDE 'EMCNST3.h90'         !
    INCLUDE 'B3V14DIMS3.h90'      ! biogenic-related constants for BEIS3 and BEIS4
    ...
    CHARACTER(80), PARAMETER :: PARMENU( 1 ) = (/ 'Use WRF-MCIP generated radiation' /)
    ...
    VFLAG = ENVYN( 'USE_VARIABLE_GRID', 'Use variable grid definition', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
    ...
    DO HR = 1, BSTEPS
        ...
        IF( .NOT. WRITE3( LNAME, 'ALL', JDATE, JTIME, EMISL ) ) THEN
            MESG = 'Could not write to "' // TRIM( LNAME ) // '"'
            CALL M3EXIT( PROGNAME, JDATE, JTIME, MESG, 2 )
        END IF

        IF( .NOT. WRITE3( SNAME, 'ALL', JDATE, JTIME, EMISS ) ) THEN
            MESG = 'Could not write to "' // TRIM( SNAME ) // '"'
            CALL M3EXIT( PROGNAME, JDATE, JTIME, MESG, 2 )
        END IF
    ...
    IF( .NOT. WRITE3( SOILOUT, 'PTYPE', JDATE, JTIME, PTYPE ) ) THEN
        MESG = 'Could not write "' // TRIM( VTMP ) // '" to file "' // TRIM( SOILOUT ) // '"'
        CALL M3EXIT( PROGNAME, JDATE, JTIME, MESG, 2 )
    END IF
