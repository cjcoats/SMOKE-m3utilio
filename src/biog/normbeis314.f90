...
SUBROUTINE NORMBEIS312( CVSW )
    ...
    !
    !  REVISION  HISTORY:
    !       04/00 Prototype, Jeff Vukovich
    !       01/03 changes to NO, George Pouliot
    ...
    !.......  PARAMETERs:
    CHARACTER(16), PARAMETER :: PNAME = 'NORMBEIS312'      ! Program name
 ...
 INTEGER         NROWS       ! no. of grid rows
    ...
    !***********************************************************************
    !   begin body of subroutine NORMBEIS312
    ...
    GNAMET = PROMPTMFILE(    &
             ...
             FSREAD3, 'BELD3_TOT', PNAME )

    IF ( .NOT. DESC3( GNAMET ) ) THEN
        MESG = 'Could not get description of file "' // TRIM( GNAMET ) // '"'
    ...
    IF ( VFLAG ) THEN
        !.......  Open GRIDDOT2D file
        DOTNAME = PROMPTMFILE( 'Enter logical name for DOT-POINT SURFACE GRID file',    &
        ...
        ALLOCATE( XVALS( NCDOT, NRDOT ),    &
                  YVALS( NCDOT, NRDOT ),    &
              PRCNT2KM2( NCOLS, NROWS ), STAT=IOS )
        CALL CHECKMEM( IOS, 'XVALS,YVALS,PRCNT2KM2', PNAME )

        !.......  Read grid cell coordinates
        IF( .NOT. READ3( DOTNAME, 'LON', 1, 0, 0, XVALS ) ) THEN
            MESG = 'Could not read LON from file "' // TRIM( DOTNAME ) // '"'
            CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
        END IF

        IF( .NOT. READ3( DOTNAME, 'LAT', 1, 0, 0, YVALS ) ) THEN
            MESG = 'Could not read LAT from file "' // TRIM( DOTNAME ) // '"'
            CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
        END IF
    ...
    CALL CHECKMEM( IOS, 'LUNAMA', PNAME )
    ...
    GNAME2 = PROMPTMFILE(    &
             ...
             FSREAD3, 'BELD3_B', PNAME )

    IF ( .NOT. DESC3( GNAME2 ) ) THEN
        MESG = 'Could not get description of file "' // TRIM( GNAME2 ) // '"'
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
    END IF
    ...
    !.......  If grid definition does not match first landuse file then stop
    IF ( EFLAG ) THEN
        MESG = 'Grid in file "' // TRIM( GNAME2 ) // '" does not match previously set grid.'
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
    ENDIF

    !.......  Store landuse variable names from second file
    NVARSB = NVARS3D

    ALLOCATE( LUNAMB( NVARSB ), STAT=IOS )
    CALL CHECKMEM( IOS, 'LUNAMB', PNAME )
    ...
    ALLOCATE( VEGID( NVEG ),    &
              EMFAC( NVEG, NSEF ),    &
               ...
               WFAC( NVEG ),    &
              LFBIO( NVEG ), STAT=IOS )
    CALL CHECKMEM( IOS, 'VEGID...LFBIO', PNAME )
    ...
    !.......  Total number of land use types (should be 230)
    NVARST = NVARSA + NVARSB

    ALLOCATE( LUINDX ( NVARST ),    &
              LUSE ( NCOLS, NROWS, NVEG ), STAT=IOS )
    CALL CHECKMEM( IOS, 'LUINDX,LUSE', PNAME )
    ...
    DO I = 1, NVARST
        ...
        DO J = 1, NVEG
            ...
            !.......  Store vegids for certain USGS categories for use later
        ...
        IF( IFOUND .EQ. 0 ) THEN
            MESG =   LUNAM // ' does NOT have emissions factors in BFAC file'
    ...
    DO M = 1, NVARST
        ...
        IF( N > 0 ) THEN
            ...
            IF( M <= NVARSA ) THEN
                ...
                IF( .NOT. READ3( GNAME1, LUNAMA( M ), 1,0,0, LUSE( 1,1,N ) ) ) THEN
            ...
            ELSE
                ...
                IF( .NOT. READ3( GNAME2, LUNAMB( K ), 1,0,0, LUSE( 1,1,N ) ) ) THEN
    ...
    !.......  Allocate memory and read forest inventory data, output normalized fluxes
    ...
    CALL CHECKMEM( IOS, 'FIA...SUMLAIW', PNAME )
    ...
    !.......  Allocate memory for output normalized fluxes
    ...
    DO I = 1, NCOLS
        ...
        DO M = 1, NVEG
            ...
            !.......  If FIA and USGS data greater than 0, then use shrubland factors
            USE_SHRUB = .FALSE.
            ...
            DO N = 1, NSEF
                    ...
                    IF ( VEGAREA > 0. ) THEN
                        ...
                        IF( IS_AG( M,IUSCGS,IUSCWD,IUSCDY,    &
                            ...
                            IF( IS_HAG (M,IUSCGS,IUSCWD) ) THEN
                                NOEM( 1 ) = NOEM( 1 ) + VEGAREA * EMFAC(M,N)*0.5

                                NOEM( 3 ) = NOEM( 3 ) + VEGAREA * EMFAC(M,N)*0.5
                            ELSE
                ...
                ELSE
                    ...
                    !         Note: assumption that these are the first three species
                    !         in the BIOTYPES array
                    IF ( N .LE. NLAI ) THEN
                        SUMLAI ( N ) = SUMLAI ( N ) + VEGAREA * LAI( M ) * EFTMP
                        SUMLAIW( N ) = SUMLAIW( N ) + VEGAREA * LAI( M ) * EFTMP * WFAC( M )
                    END IF
...
CONTAINS
    ...
    !.......  This internal function checks for "half" agricultural areas
