...
PROGRAM TMPBEIS4
    ...
    !***********************************************************************
    !  program body starts at line  231
    !
    !  DESCRIPTION:
    !       Computes hourly time stepped gridded biogenic emissions using
    !       normalized gridded emissions from Normbeis4 and postprocessed
    !       WRF-MCIP meteorology.
    !
    !  PRECONDITIONS REQUIRED:
    !       Postprocessed WRF-MCIP meteorology that contains temperature,
    !       solar radiation, pressure data, soil temmperature, and other data.
    !       Normalized gridded emissions BEIS_NORM_EMIS from Normbeis4
    !
    !  SUBROUTINES AND FUNCTIONS CALLED:
    !       HRBEIS4, CZANGLE,RDBPRO, PREBMET
    !
    !  REVISION  HISTORY:
    !       04/2022: Prototype by Jeff Vukovich based of Jesse Bash's proposed
    !         modifications
    ...
    !.......  This module contains BEIS3 and BEIS4 specific arrays
    !...... BEIS3 module still used in BEIS4 but some variables
    !...... no longer needed
    ...
    !.......  INCLUDES:
    INCLUDE 'EMCNST3.h90'         !
    INCLUDE 'B3V14DIMS3.h90'      ! biogenic-related constants for BEIS3
    !.......  and BEIS4
    ...
    !.......  PARAMETERs
    CHARACTER(16), PARAMETER :: PROGNAME = 'TMPBEIS4'       !  program name

    CHARACTER(80), PARAMETER :: PARMENU( 1 ) =              &    ! methods to calc. PAR
       (/ 'Use WRF-MCIP generated radiation' /)

    CHARACTER(16), PARAMETER :: UNITSMENU( 2 ) =            &    ! output units
       (/ 'moles/hr ',  &
          'moles/s  '  /)
    ...
    LOGICAL         FDFLAG             ! true: output file meta desc
    ...
    IF ( RDEV .LT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Could not open SPECIATION PROFILES file', 2 )
    END IF
    ...
    CALL RDBPRO( RDEV, SPPRO, NSEF, BIOTYPES, MSPCS, EMSPC, MLFAC, MSFAC )

    !.......  Get the method to calculate PAR
    PARTYPE = ENVINT( 'BG_CLOUD_TYPE', 'How PAR will be calculated', 1, IOS )
    IF ( IOS .GT. 0 ) THEN
    ...
    !.......  Check to compute season function from MCIP soil temperature
    MESG = 'Compute season function from soil temperature?'
    SEASON_CALC = ENVYN( 'COMPUTE_SEASON_FUNCTION', MESG, .TRUE., IOS)
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "COMPUTE_SEASON_FUNCTION"', 2 )
    END IF

    !.......  Check to see if want to use LAI from WRF-MCIP file
    MESG = 'Use LAI from WRF-MCIP?'
    WRF_LAI = ENVYN( 'USE_WRF_LAI', MESG, .TRUE., IOS)
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "USE_WRF_LAI"', 2 )
    END IF

    !.......  Check to see if want to use WSAT from WRF-MCIP file
    MESG = 'Use WSAT from WRF-MCIP?'
    WRF_WSAT = ENVYN( 'USE_WRF_WSAT', MESG, .TRUE., IOS)
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "USE_WRF_WSAT"', 2 )
    END IF
    ...
    !.......  Get normalized emissions file, BEIS_NORM_EMIS
    NNAME = PROMPTMFILE( 'Enter name for NORMALIZED EMISSIONS input file',&
    ...
    !.......  Check that grid description matches BEIS_NORM_EMIS file
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "TMPR_VAR"', 2 )
    END IF
    ...
    IF (PX_VERSION) THEN
        ...
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME,0,0, 'Bad env vble "SOILM_VAR"', 2 )
        END IF
        ...
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME,0,0, 'Bad env vble "SOILT_VAR"', 2 )
        END IF
        ...
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME,0,0, 'Bad env vble "SOILT2_VAR"', 2 )
        END IF
        ...
        !.......  Check for soil saturation variable
            ...
            IF ( IOS .GT. 0 ) THEN
                CALL M3EXIT( PNAME,0,0, 'Bad env vble "WSAT_VAR"', 2 )
            END IF
        ...
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME,0,0, 'Bad env vble "ISLTYP_VAR"', 2 )
        END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "RN_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "RC_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "PRES_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "RGRND_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "RADYNI_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "TEMPG_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "LAI_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "Q2_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "RSTOMI_VAR"', 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "USTAR_VAR"', 2 )
    END IF
    ...
    IF ( CHKNAME .EQ. 'VIRTUAL ' ) THEN
        JRUNLEN = ENVINT( 'G_RUNLEN', 'Duration(HHMMSS)', 490000, IOS )
    ...
    LNAME = PROMPTMFILE( 'Enter name for B4GTS output file - moles',&
    ...
    SNAME = PROMPTMFILE( 'Enter name for B4GTS output file - mass',&
                         FSUNKN3, 'B4GTS_S', PROGNAME )

    !.......  Alolocate gridded variables:
    ...
    EFLAG = .FALSE.
    IF( EFLAG ) THEN
        MESG = 'Grid in file "' // TRIM( GNAME ) // '" does not match previously set grid.'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    !.......  Read GNAME variables:

    IF( .NOT. READ3( GNAME, 'LAT', 1, 0, 0, LAT ) ) THEN
        MESG = 'Could not read LAT from"' // TRIM( GNAME ) // '"'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    IF( .NOT. READ3( GNAME, 'LON', 1, 0, 0, LON ) ) THEN
        MESG = 'Could not read LON from"' // TRIM( GNAME ) // '"'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF
    ...
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME,0,0, 'Bad env vble "INITIAL_RUN"', 2 )
    END IF
    ...
    !....... note if using WRF LAI in BEIS4
    IF ( WRF_LAI ) THEN
        MESG = 'Using WRF LAI instead of estimated LAI from NORMBEIS'
        CALL M3MESG( MESG )
    ELSE
    ...
    DO HR = 1, BSTEPS
        ...
        IF( JDATE .NE. LDATE ) THEN
            ...
            IF ( SWITCH_FILE ) THEN
                ...
                IF( .NOT. READ3( BNAME, 'SEASON', 1,&
                                 JDATE, 0, SWITCH ) ) THEN
                    MESG = 'Could not read SEASON from"' //&
                            TRIM( BNAME ) // '"'
        ...
        CALL CZANGLE( JDATE, JTIME, NCOLS, NROWS, LAT, LON, COSZEN, GETATN )
        ...
        !......  Read soil temperature data layer 2; 1 m deep
            IF( .NOT. READ3( M3NAME, STMP2NAM, 1, MDATE, MTIME, SOILT2 ) ) THEN
                MESG = 'Could not read "' // STMP2NAM // '" from"' // TRIM( M3NAME ) // '"'
                CALL M3EXIT( PROGNAME, MDATE, MTIME, MESG, 2 )
            END IF
        ...
        !.......  Read WSAT data
                IF( .NOT. READ3( M3NAME, WSATNAM, 1, MDATE, MTIME, WSAT ) ) THEN
                    MESG = 'Could not read "' //  WSATNAM // '" from"' // TRIM( M3NAME ) // '"'
                    CALL M3EXIT( PROGNAME, MDATE, MTIME, MESG, 2 )
                END IF
        ...
        !.......  Read radiation data
        IF( .NOT. READ3( M3NAME, RGRNDNAM, ALLAYS3, RDATE, RTIME, RGRND ) ) THEN
            MESG = 'Could not read "' // RGRNDNAM // '" from"' // TRIM( M3NAME ) // '"'
            CALL M3EXIT( PROGNAME, RDATE, RTIME, MESG, 2 )
        END IF

        IF( .NOT. READ3( M3NAME, TEMPGNAM, ALLAYS3, RDATE, RTIME, TEMPG ) ) THEN
            MESG = 'Could not read "' // TEMPGNAM // '" from"' // TRIM( M3NAME ) // '"'
            CALL M3EXIT( PROGNAME, RDATE, RTIME, MESG, 2 )
        END IF

        IF( .NOT. READ3( M3NAME, RADYNINAM, ALLAYS3, RDATE, RTIME, RADYNI ) ) THEN
            MESG = 'Could not read "' // RADYNINAM // '" from"' // TRIM( M3NAME ) // '"'
            CALL M3EXIT( PROGNAME, RDATE, RTIME, MESG, 2 )
        END IF

        IF( .NOT. READ3( M3NAME, LAINAM, ALLAYS3, RDATE, RTIME, LAI ) ) THEN
            MESG = 'Could not read "' // LAINAM // '" from"' // TRIM( M3NAME ) // '"'
            CALL M3EXIT( PROGNAME, RDATE, RTIME, MESG, 2 )
        END IF

        IF( .NOT. READ3( M3NAME, Q2NAM, ALLAYS3, RDATE, RTIME, Q2 ) ) THEN
            MESG = 'Could not read "' // Q2NAM // '" from"' // TRIM( Q2NAM ) // '"'
            CALL M3EXIT( PROGNAME, RDATE, RTIME, MESG, 2 )
        END IF

        IF( .NOT. READ3( M3NAME, RSTOMINAM, ALLAYS3, DATE, RTIME, RSTOMI ) ) THEN
            MESG = 'Could not read "' // RSTOMINAM // " from"' // TRIM( RSTOMINAM ) // '"'
            CALL M3EXIT( PROGNAME, RDATE, RTIME, MESG, 2 )
        END IF

        IF( .NOT. READ3( M3NAME, USTARNAM, ALLAYS3, RDATE, RTIME, USTAR ) ) THEN
        ...
        IF ( SEASON_CALC ) THEN
            ...
            DO I = 1, NCOLS
                ...
                !...... Special case for coastal water cells that may have some
                !...... biogenic emissions due to disconnect between WRF water and
                !...... and BELD6 water.   Note SOIT2 in MCIP doesn't seem to vary
                !...... thru the year in water cells in WRF.

                IF ( ISLTYP( I,J ) .EQ. 14 .AND.&
                      SOILT( I,J ) .GE. 295.0 ) THEN
                    PSEAS = 1.0
                ENDIF
                !......... Apply season function to emissions
                QSEAS = 1.0 - PSEAS
                SEMIS( I, J, 1:NSEF   ) = PSEAS * AVGEMIS( I, J, 1:NSEF, NSUMMER ) +    &
                                          QSEAS * AVGEMIS( I, J, 1:NSEF, NWINTER)
                SLAI( I, J, 1:NLAI )    = PSEAS *  AVGLAI( I, J, 1:NLAI, NSUMMER ) +&
                                          QSEAS *  AVGLAI( I, J, 1:NLAI, NWINTER )
    ...
    IF( .NOT. WRITE3( SOILOUT, 'PTYPE', JDATE, JTIME, PTYPE ) ) THEN
        MESG = 'Could not write "' // TRIM( VTMP ) // '" to "' // TRIM( SOILOUT ) // '"'
        CALL M3EXIT( PROGNAME, JDATE, JTIME, MESG, 2 )
    END IF

    IF( .NOT. WRITE3( SOILOUT, 'PULSEDATE', JDATE, JTIME, PULSEDATE ) ) THEN
        MESG = 'Could not write "' // TRIM( VTMP ) // '" to "' // TRIM( SOILOUT ) // '"'
        CALL M3EXIT( PROGNAME, JDATE, JTIME, MESG, 2 )
    END IF

    IF( .NOT. WRITE3( SOILOUT, 'PULSETIME', JDATE, JTIME, PULSETIME ) ) THEN
        MESG = 'Could not write "' // TRIM( VTMP ) // '" to "' // TRIM( SOILOUT ) // '"'
        CALL M3EXIT( PROGNAME, JDATE, JTIME, MESG, 2 )
    END IF
    ...
    DO I = 1, RVARS
        ...
        IF( .NOT. WRITE3( SOILOUT, VTMP, JDATE, JTIME, AINFALL( 1,1,I ) ) ) THEN
            MESG = 'Could not write "' // TRIM( VTMP ) // '" to "' // TRIM( SOILOUT ) // '"'
            CALL M3EXIT( PROGNAME, JDATE, JTIME, MESG, 2 )
        END IF
    END DO
