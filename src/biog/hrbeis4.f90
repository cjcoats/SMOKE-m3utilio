...
SUBROUTINE HRBEIS4( JDATE, JTIME, NX, NY, MSPCS,                        &
    ...
    !
    !  REVISION  HISTORY:
    !       Prototype 4/22 : by JMV based on CMAQ inline logic with updates
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'HRBEIS4'       !  program name
    ...
    DO C = 1, NX
        ...
        ! atmospheric water vapor variables used for leaf latent heat flux
        IF ( TAIR .LE. STDTEMP ) THEN
            ES = VP0 * EXP( 22.514 - (6.15e3 / TAIR) )
        ELSE
            ES = VP0 * EXP( SVP2 * (TAIR - STDTEMP) / (TAIR - SVP3) )
        END IF
        RH   = Q2( C,R )*(PSFC-ES)/(0.622*ES)
        RH   = MIN(1.0,RH)
        DVAP = ES*(1.0-RH)/PSFC
        SSVP = (SVP2*(STDTEMP-SVP3)*ES/(TAIR-SVP3)**2)/ PSFC

        ! calculate the soil heaf flux under a canopy folowing WRF3.4.1 PX
        SHF = -2.0*PI/SIDAY*(TEMPG( C,R ) - TAIR)/CV

        ! calculate the heat and water vapor quasilaminar boundary layer resistance
        RBH = 5.0/USTAR( C,R )
        RBW = RBH*(SCW/PR)**TWOTHIRDS

        ! calculate the specific heat of air and latent heat of vaporization
        CPAIR = CPD * (1.0 + 0.84 * Q2( C,R ))
        LHV   = LV0 - 2370.0 * (TAIR - STDTEMP)

        ! calculate the leaf  water vapor and heat conductance
        GHT  = 1 / (RATM( C,R ) + RBH  ) + 1/RRAD
        GVAP = 1 / (RATM( C,R ) + RBW + RSTOM( C,R ) * LAI( C,R ))

        ! Calculate the potential temperature conversion from the sensible heat flux in WRF 3.4.1
        CPOT = (STDATMPA/PSFC)**(RDGAS/CPAIR)

        ! Calculate the solar radiation and soil heat flux of the leaf energy budget
        DENS     =  PSFC /( RDGAS * TAIR )
        LHSH_DIV =  DENS * CPOT * CPAIR                     &
                    * (GHT + 1 / (RATM( C,R ) + RBH  )) +   &
                    DENS * LHV * SSVP * GVAP

        ! calculate the latent heat flux portion of the leaf energy budget
        ...
        !.........  Check max/min bounds of PAR and calculate biogenic ISOP
        ...
        DO I = 1, NLAI
            ...
            IF ( TLAI .GT. 10.0 ) THEN
                WRITE( MESG, 94010 ) 'LAI=', TLAI, 'out of range at (C,R)=', C, R
                CALL M3EXIT( PROGNAME, JDATE, JTIME, MESG, 2 )
            END IF
            ...
            IF ( PARDB + PARDIF .EQ. 0.0 ) THEN
                EMPOL( C,R,I ) = 0.0
            ELSE
                CALL CLNEW_SUB( COSZ, PARDB, PARDIF, TLAI, LHSH_DIV,    &
        ...
        ! Only estiamte BVOC emissions for vegatation
        ! Estimate emissions for OVOC and SESQT
        ...
        ELSE
            ! If LAI = 0 zero out emission factors
            CFOVOC  = 0.0
            CFSESQT = 0.0
        END IF
...
CONTAINS
    ...
    SUBROUTINE CLNEW_SUB( CZEN, PARDB, PARDIF, TLAI, LHSH_DIV,  &
        ...
        REAL, INTENT( IN )  :: CZEN             ! cos of solar zenith angle
        ...
        IF ( FIRSTIME ) THEN
            ...
            SQALPHA = SQRT( ALPHA )
        END IF
        ! CN98 - eqn 15.4, assume x=1 (can use a table or atributes to change this)
        ! Set a ceiling for KBE to prevent a blow up at high zenith angles. This has
        ! little impact on the results because direct PAR is low under these conditions
        IF( CZEN .LE. 7.96e-4 ) THEN          !  zen > 1.57
            KBE = 627.9
        ELSE
            KBE = 0.5 / CZEN
        END IF
        IF ( TLAI .GT. 0.1 ) THEN
            IF ( PARDB + PARDIF .GT. 0.0 ) THEN
                ...
                ! CN98 p-259 Sun and shaded areas of the canopy
                ...
                ! calculate the leaf temperature following Campbel and Norman 1998 eq 14.6
                ! with the addition of incomming atmospheric long wave irradiation resulting
                ! in the cacelation of the long wave radiation budget
                SOLSUN    = SOLRAD * PARSUN / ( PARSUN + PARSHADE )
                SOLSHADE  = SOLRAD * PARSHADE / ( PARSUN + PARSHADE )
                ! Because PARSUN + PARSHADE                 != PARDB+PARDIF as the first considers a
                ! hemispheric sphere (crown canopy) and the later a horizontal plane (the
                ! ground). This changes the peak leaf temperature a bit, usually about 1
                ! hour earlier.
                ...
                ! cguen is Guenther's eqn for computing light correction as a function of
                ! PAR...fracSun should probably be higher since sunlit leaves tend to be
                ! thicker than shaded leaves. But since we need to make crude assumptions
                ! regarding leaf orientation (x=1), we will not attempt to fix at the moment.

                CSUBL_SUN   = CGUEN( PARDB + PARDIF, 0.0, LAISUN, KBE )
                ! By definition diffusive radiation, use the diffusive attenuation
                ! coefficient and Diffusive par at the bottom of the sunlit layer
    ...
    REAL FUNCTION CGUEN( PAR, LAI1, LAI2, KBE )
        ...
        ! 11/14 J. Bash - Updated to Niinemets et al. 2010 doi:10.1029/2010JG001436
        !         Big leaf model which updates Guenther et al. 1993 doi:10.1029/93JD00527 for
        !         in-canopy gradients
        ...
        ! Consistent with Niinemets et al. 2010a
        ! Mean of reported Quercus rubra and Liquidambar styraciflua

        REAL, PARAMETER :: ALPHA = 0.0015
        REAL, PARAMETER :: CL    = 1.2716

        !  Niinemets et al. 2010b to return 1 at standard conditions
        ...
        IF ( PAR .LE. 0.01 ) THEN
            CGUEN = 0.0
        ELSE
        ! Niinemets et al. 2010 equation A9 integrated from LAI1 to LAI2
