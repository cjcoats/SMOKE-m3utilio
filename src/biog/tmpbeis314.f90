...
SUBROUTINE TMPBEIS314( CVSW )
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'TMPBEIS314'       !  program name

    !***********************************************************************
    !   begin body of subroutine TMPBEIS314
    ...
    !......  Build name table for variables in normalized emissions, net files
    ALLOCATE( AVGEMIS( NCOLS, NROWS, NSEF, NSEASONS ),    &
               ...
               AVGLAI( NCOLS, NROWS, NLAI, NSEASONS ),    &
                  ...
                  LON( NCOLS, NROWS ),    &
               COSZEN( NCOLS, NROWS ),    &
                TASFC( NCOLS, NROWS ), STAT=IOS )
    CALL CHECKMEM( IOS, 'AVGEMIS...COSZEN', PROGNAME )
    ...
    DO HR = 1, BSTEPS
        ...
        !......  Read radiation data
        IF( .NOT. READ3( M2NAME, RADNAM, ALLAYS3, RDATE, RTIME, TSOLAR ) ) THEN
            MESG = 'Could not read "' // RADNAM // '" from "' // TRIM( M2NAME ) // '"'
...
CONTAINS
    ...
    SUBROUTINE CHECK_VARIABLE( VARNAME, FILENAME )
        ...
        IF( J <= 0 ) THEN
            MESG = 'Could not find "' // TRIM( VARNAME ) // '" in "' // TRIM( FILENAME ) // '"'
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF
