...
SUBROUTINE ASGNCNTL( NSRCIN, WDEV, PKTTYP, PNAM, DATSPFLAG, SINDX )

    !***********************************************************************
    !  subroutine body starts at line
    !
    !  DESCRIPTION:
    !      For each source and pollutant, find the most specific entry in a
    !      control data table to apply.  The control data of interest can vary,
    !      but no matter the control packet being processed, the control data
    !      table index is stored in the ICTL* grouped x-ref tables from MODXREF.
    !      The source-based stored index for each control packet comes in
    !      as a subroutine argument (SINDX), so this can change for each call to
    !      this routine.
    !
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'ASGNCNTL'     ! program name

    !***********************************************************************
    !   begin body of subroutine ASGNCNTL
    ...
    IF( FIRSTIME ) THEN
        ...
        IF ( I .GT. 0 ) THEN
            CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "XREF_SICOVERSCC"', 2 )
        END IF

        !.......  Set up format for creating character FIPS code for non-point
        IF( CATEGORY .NE. 'POINT' ) THEN
            WRITE( FMTFIP, 94300 ) '(I', FIPLEN3, '.', FIPLEN3, ')'
        ENDIF

        !.......  Figure out if SIC and/or MACT and src type codes are available
    ...
    !.......  Set flag to indicate if SIC-specific assignments are
    !           available in the cross-reference file
    SICXREF = ( MAXVAL( TXCNT( 26:31 ) ) .GT. 0 )

    !.......  Initialize SINDX
    SINDX = 0          ! Array

    !.......  For each pollutant of interest
    DATSPFLAG = .FALSE.
    ...
    !.......  Write message, with different message for projections because
    !               they are not yet data-specific
    IF( PKTTYP .NE. 'PROJECTION' ) THEN
        MESG = BLANK5 // 'Assigning controls for pollutant "' // TRIM( PNAM ) // '"...'
        V = INDEX1( PNAM, NIPPA, EANAM )

    ELSE IF ( PNAM .EQ. 'all' ) THEN
        MESG = BLANK5 // 'Assigning projection factors to all pollutants...'
        V = 1
    ELSE
        MESG = BLANK5 // 'Assigning projection factorsfor pollutant ' //&
               '"' //TRIM( PNAM ) // '"...'
        V = INDEX1( PNAM, NIPPA, EANAM )
    END IF
    CALL M3MSG2( MESG )
    ...
    DO S = 1, NSRCIN
        ...
        !.......  Also set search field for SCC-specific assignment with all
        ...
        !                           pollutant-specific PLANT non-blank// MACT match; then
        ...
        !......  Do an additional check for the most detailed source assignment
        !                    with SCC as well.
        ...
        !.......  Depending on SIC/SCC switch, match by SIC first, then SCC
        IF ( SICFIRST ) THEN

            !.......  If SIC available in inventory, assign by SIC combinations 1st
            ISTAT = 0
            IF ( SICFLAG ) CALL SET_BY_SIC( ISTAT )
            IF ( ISTAT == 1 ) CYCLE

            !.......  Assign by SCC combinations 2nd
            ISTAT = 0
        ...
        ELSE
            ...
            !.......  Assign by SCC combinations 1st
            ISTAT = 0
            CALL SET_BY_SCC( ISTAT )
            IF ( ISTAT == 1 ) CYCLE

            !.......  If SIC available in inventory, assign by SIC combinations 2nd
            ISTAT = 0
            IF ( SICFLAG ) CALL SET_BY_SIC( ISTAT )
            IF ( ISTAT == 1 ) CYCLE
        END IF
        ...
        !.......  Try for any country/state code match
        ...
        IF( ICTL01( V ) .NE. IMISS3 .AND. REPDEFLT ) THEN
            ...
            WRITE( MESG,94010 )                             &
                   ...
                   CRLF() // BLANK10 // ' SCC: ' // TSCC_D // ' POL: ' // EANAM( V )
            CALL M3MESG( MESG )
    ...
    IF( EFLAG ) THEN
        MESG = 'Problem assigning ' // PKTTYP // ' controls to sources'
...
CONTAINS
    ...
    SUBROUTINE REPORT_SCC_USE( CHARTNUM )
        ...
        !.......   Subprogram Internal buffering formats...... 94xxx
...
94010   FORMAT( 10( A, :, I8, :, 1X ) )
    ...
    !.......  This internal subprogram checks the source type associated
    !               with a MACT packet entry.
    ...
    !               information by SIC and all combos. All variables are inherited.
    SUBROUTINE SET_BY_SIC( ISTAT )
        ...
        !.......  Try for pollutant-specific  FIPS code  SIC matches; then
        !                  pollutant-specific Cy/st code  SIC matches; then
        !                  pollutant-specific SIC matches
