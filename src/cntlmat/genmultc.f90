...
SUBROUTINE GENMULTC( CDEV, GDEV, LDEV, MDEV, NCPE, PYEAR,   &
                     ENAME, MNAME, CFLAG, GFLAG, LFLAG, MFLAG )
    ...
    !.......  Open reports files
    RPTDEV( 1 ) = PROMPTFFILE( 'Enter logical name for MULTIPLICATIVE CONTROLS REPORT',&
                               .FALSE., .TRUE., CRL // 'CREP', PROGNAME )
    RDEV = RPTDEV( 1 )

    RPTDEV( 5 ) = PROMPTFFILE( 'Enter logical name for DETAILED CONTROL PACKET REPORT',&
    ...
    DO I = 1, NVCMULT
        ...
        !.................
        !.......  Apply /MACT/ packet controls if present for the current pollutant
        ...
        !.................
        !.......  Apply /CONTROL/ packet controls if present for the current pollutant
                    ...
                    IF( CTLRPLC( CTLINDX ) ) THEN
                        ...
                        IF( LCOMPARE ) THEN
                            ...
                            ELSE
                                CSRC = CSOURC( S )
                                CALL FMTCSRC( CSRC, NCHARS, BUFFER, L2 )
                                WRITE( MESG,94110) 'WARNING: CONTROL packet '// &
                                'record number', CTLINDX,                       &
                                'control ignored for source because '//         &
                                'base year controls are greater for:'//         &
                                CRLF()// BLANK10// BUFFER( 1:L2 ) // ' POL:' // PNAM
                                CALL M3MESG( MESG )
        ...
        IF( OPENFLAG ) THEN
            IF( .NOT. WRITESET( MNAME, PNAM, ALLFILES, 0, 0, FACTOR )) THEN
            ...
            L = LEN_TRIM( PNAM )
            IF( .NOT. WRITESET( MNAME, 'CE_'//PNAM(1:L), ALLFILES, 0, 0, CEFF )) THEN
                MESG = 'Failed to write control efficiency ' // &
                       'for pollutant ' // PNAM
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            END IF

            IF( .NOT. WRITESET( MNAME, 'RE_'//PNAM(1:L), ALLFILES, 0, 0, REFF ) ) THEN
                MESG = 'Failed to write rule effectivness ' //  &
                       'for pollutant ' // PNAM
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            END IF

            IF( .NOT. WRITESET( MNAME, 'RP_'//PNAM(1:L), ALLFILES, 0, 0, RPEN ) ) THEN
    ...
    IF( APPLFLAG ) THEN
        ...
        END IF
        IF( MFLAG ) WRITE( RDEV, 93000 ) 'Controls applied with /MACT/ packet'
        IF( CFLAG ) WRITE( RDEV, 93000 ) 'Controls applied with /CONTROL/ packet'
        IF( GFLAG ) WRITE( RDEV, 93000 ) 'Controls applied with /CTG/ packet'
        IF( LFLAG ) WRITE( RDEV, 93000 ) 'Controls applied with /ALLOWABLE/ packet'
        IF( LAVEDAY ) THEN
        ...
        IF ( CATEGORY .EQ. 'POINT' ) THEN
            WRITE( RDEV, 93000 )    &
               'Emissions by controlled facility before and after controls'
        ELSE
            WRITE( RDEV, 93000 )    &
               'Emissions by controlled state/SCC before and after controls'
        END IF

        IF ( CATEGORY .EQ. 'POINT' ) THEN
            WRITE( RDEV, 93400 ) ( PNAMMULT( I ), PNAMMULT( I ), I=1,NVCMULT )
        ELSE
            WRITE( RDEV, 93402 ) ( PNAMMULT( I ), PNAMMULT( I ), I=1,NVCMULT )
        END IF
...
93410 FORMAT( 4X, A3, ';', 6X, A10, ';', 1X, 100( 10X, E11.4, :, ';' ))

93412 FORMAT( 1X, A6, ';', 1X, A15, ';', 1X, 100( 10X, E11.4, :, ';' ))
