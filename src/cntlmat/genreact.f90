...
SUBROUTINE GENREACT( PYEAR, ENAME, RPOL, USEPOL )
    ...
    IF( FIRSTIME ) THEN
        ...
        !.......  Open reports file
        RPTDEV( 3 ) = PROMPTFFILE( 'Enter logical name for REACTIVITY REPORT',  &
        ...
        MESG = 'Speciation profiles needed to process reactivity packet...'
        CALL M3MSG2( MESG )

        PDEV = PROMPTFFILE( 'Enter logical name for SPECIATION PROFILES file',  &
        ...
        IF( RDEV .GT. 0 ) THEN
            ...
            IF ( PYEAR .GT. 0 ) THEN
                WRITE(RDEV,93390) 'Reactivity packet projected year ', PYEAR
            END IF
    ...
    DO J = 2, NCOLS-1
        BUFFER = HDRSTR
        HDRSTR = TRIM( BUFFER ) // ' ' // HEADERS( HINDX( J ) )( 1:HCWID( J ) ) // ';'
    END DO
    BUFFER = HDRSTR
    HDRSTR = TRIM( BUFFER ) // ' ' // HEADERS( HINDX( NCOLS ) )( 1:HCWID( NCOLS ) )
    ...
    DO J = 1, NCOUT
        BUFFER = OUTFMT
        WRITE( OUTFMT, '(A,I2.2,A)' ) TRIM( BUFFER ) // ',A', MCWID( J ), ',"; "'
    END DO
    BUFFER = OUTFMT
    ...
    IF( NSPFUL .LE. 0 ) THEN
        MESG = 'No speciation profiles found for pollutant "' // TRIM( RPOL ) // '"    ! '
...
CONTAINS
    ...
    SUBROUTINE WRITE_WARNING
        ...
        ELSE
            ...
            BUFFER = 'FIP: ' // CFIP // ' SCC: ' // TSCC // ' POL: ' // RPOL
