            ...
            ELSEIF( I .LE. 0 .AND. J .GT. 0 ) THEN
                IF( J .EQ. 1 .OR. LINE( 1:J ) .EQ. ' ' ) THEN
                    WRITE( MESG,94010 ) 'Problem at line', IREC,                &
                           'of control packets file.' // CRLF() // BLANK10 //   &
...
101 CONTINUE            ! Exit from read loop
    ...
    ALLOCATE( CUTCTG( J ),      &
             ...
             FACRACT( J ),      &
             CTGCOMT( J ), STAT=IOS )
    CALL CHECKMEM( IOS, 'CUTCTG...CTGCOMT', PROGNAME )
    CUTCTG  = 0.      ! array
    FACCTG  = 0.      ! array
    FACMACT = 0.      ! array
    FACRACT = 0.      ! array
    ...
    ALLOCATE( CALWSIC( J ),      &
             ...
             EMREPALW( J ),      &
              ALWCOMT( J ), STAT=IOS )
    CALL CHECKMEM( IOS, 'CALWSIC...CTGCOMT', PROGNAME )
    CALWSIC = ' '     ! array
    FACALW   = 0.     ! array
    EMCAPALW = 0.     ! array
    EMREPALW = 0.     ! array
    ...
    ALLOCATE( CREASIC( J ),      &
             ...
             MKTPNREA( J ),      &
              ...
              CSPFREA( J ),      &
              REACOMT( J ), STAT=IOS )
    CALL CHECKMEM( IOS, 'CREASIC...CTGCOMT', PROGNAME )
    CREASIC  = ' '     ! array
    EMREPREA = 0.      ! array
    PRJFCREA = 0.      ! array
    MKTPNREA = 0.      ! array
    ...
    ALLOCATE( CPRJSIC( J ),      &
                PRJFC( J ),      &
              PRJCOMT( J ), STAT=IOS )
    CALL CHECKMEM( IOS, 'CPRJSIC...CTGCOMT', PROGNAME )
    CPRJSIC = ' '     ! array
    PRJFC   = 0.      ! array
    PRJCOMT = " "     ! array
    ...
    ALLOCATE( CMACSRCTYP( J ),      &
                ...
                MACNWFRC( J ),      &
                 MACCOMT( J ), STAT=IOS )
    CALL CHECKMEM( IOS, 'CMACSRCTYP...CTGCOMT', PROGNAME )
    CMACSRCTYP = '00'      ! array
    MACEXEFF = 0.      ! array
    MACNWEFF = 0.      ! array
    MACNWFRC = 0.      ! array
...
CONTAINS
    ...
    SUBROUTINE CHECK_PACKET( LINE, L1, L2, INYEAR, PKTIDX, OUTYEAR, YFLAG )
        ...
        DO I = 1, NPACKET
            ...
            IF( BUFFER .EQ. TRIM( PKTLIST( I ) ) ) THEN
                PKTIDX = I
                EXIT
            END IF
        ...
        IF( L2 .GT. L ) THEN
            ...
            ELSE IF( SYEAR .NE. INYEAR ) THEN
                ...
                IF ( IOS .GT. 0 ) THEN
                    CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "PROJECTION_YR_SPEC"', 2 )
                ELSE IF( YFLAG ) THEN
                    PKTIDX = -1
                    RETURN
                END IF
