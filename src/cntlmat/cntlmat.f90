...
PROGRAM CNTLMAT
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'CNTLMAT'       !  program name
    ...
    CDEV = PROMPTFFILE( 'Enter logical name for ASCII CONTROL PACKETS file',&
                        .TRUE., .TRUE., 'GCNTL', PROGNAME )

    WDEV = PROMPTFFILE( 'Enter logical name for output WARNINGS/ERRORS file',&
    ...
    IF( JFLAG .AND. CPYEAR .LT. 1900 ) THEN
        MESG = 'Misformatted projection packet header is returning year < 1900'
    ...
    ELSE IF ( JFLAG ) THEN
        MESG = 'WARNING: No records from the projection packet matched the inventory.'
        CALL M3MSG2( MESG )
    ...
    ELSE IF ( CFLAG .OR. GFLAG .OR. LFLAG .OR. MFLAG ) THEN
        MESG = 'WARNING: No records from any control packet matched the inventory.'
        CALL M3MSG2( MESG )
