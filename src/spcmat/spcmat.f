...
C.........  LOCAL PARAMETERS and their descriptions:
     ...
     &  CVSW = '$Name SMOKEv5.0_Jun2023$'  ! CVS revision tag
...
C.........  Open and read map file
     ...
     &           .TRUE., .TRUE., 'GSREF', PROGNAME )
        IF ( XDEV .LT. 0 ) THEN
            CALL M3EXIT( PROGNAME,0,0, 'Could not open SPECIATION XREF file', 2 )
        END IF
     ...
     &           .TRUE., .TRUE., 'GSCNV', PROGNAME )
        IF ( KDEV .LT. 0 ) THEN
            CALL M3EXIT( PROGNAME,0,0, 'Could not open POLLUTANT CONVERSION file', 2 )
        END IF
     ...
     &           .TRUE., .TRUE., 'GSTAG', PROGNAME )
        IF ( GDEV .LT. 0 ) THEN
            CALL M3EXIT( PROGNAME,0,0, 'Could not open TAGGING file', 2 )
        END IF
     ...
     &           .TRUE., .TRUE., 'INVTABLE', PROGNAME )
        IF ( VDEV .LT. 0 ) THEN
            CALL M3EXIT( PROGNAME,0,0, 'Could not open INVENTORY DATA TABLE', 2 )
        END IF
