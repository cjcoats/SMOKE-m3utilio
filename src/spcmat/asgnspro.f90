...
SUBROUTINE ASGNSPRO( MASSOUT, MOLEOUT, REPORT, NSRCIN, UDEV,    &
    ...
    !
    !  DESCRIPTION:
    !    For each source and current pollutant, find the most specific speciation
    !    profile that applies to that source. Do this using the grouped tables of
    !    speciation cross references from RDSREF.  The hierarchical order is
    !    defined in this subroutine, and can be determined from the in-source
    !    comments below. Once a profile code has been identified, search for this
    !    code in the speciation profile tables (from RDSPROF) and use this profile
    !    to update the previously initialized speciation matrices.
    !
    ...
    USE MODINFO, ONLY: CATEGORY, NCHARS, JSCC, NIPPA, EANAM, LSCCEND
    ...
    !***********************************************************************
    !   begin body of subroutine ASGNSPRO

    !.......  For first time routine is called in all cases,
    IF( FIRSTIME ) THEN

        !.......  Get maximum number of warnings/errors
        ...
        !.......  Retrieve skipping speciation profile flags for zero emissions
    ...
    !.......  Initialize that combo file would need to be read
    !    for this pollutant at first instance of COMBO in GSREF
    LRDCOMBO = .TRUE.

    !.......  Set number of output fields for FMTCSRC to use
    ...
    !.......  Write pollutant of interest to the supplemental file
    WRITE( UDEV, '(A)' ) '"' // ENAM // '"'
    ...
    !.......  Find index in complete list of pollutants and set length of name
    V  = INDEX1( ENAM, NIPPA, EANAM )

    DO S = 1, NSRCIN

        !.......  If emissions are zero for this source, then skip it
        ...
        ELSE          !  if emissions are not zero for this source
            ...
            !.......  For default speciation profile, make sure that it has been
            !    defined for the current pollutant, and that we want to report
            !    the use of defaults.
            IF( CSPT01( V ) .NE. EMCMISS3 .AND.    &
                ...
                MESG = 'NOTE: Using default speciation profile "' // SPCODE // '" for:'//    &
                       CRLF() // BLANK10 // BUFFER( 1:L2 ) //    &
                       CRLF() // BLANK10 // 'SCC: ' // TSCCINIT // ' POL: ' // EANAM( V )
                CALL M3MESG( MESG )
...
CONTAINS
    ...
    SUBROUTINE SETSOURCE_SMATS
        ...
        !.......  If speciation profile code assigned is a combination profile...
        !.......  This internal subroutine checks to see if the assignment has
        !    been made to a COMBO profile and takes action accordingly.
        !    Its main jobs are to read in the GSREF_COMBO file, find the
        !    combination profile in the dataset, and to setup the number
        !    of profiles in a combination profile.
        !.......  NOTE:SREFDEX( SPCODE ) =
        !    0 -- not an SREF* profile
        !    -9999:  out-of-range/invalid SREF* profile
        !    1-SREFCNT:  SREF* subscript
        F = CMBDEX( SPCODE )
        ...
        ELSE IF ( ADJUSTL( SPCODE ) .EQ. 'COMBO' ) THEN
            ...
            !....... For the first combo profile encountered, open the GSPRO_COMBO
            !    file. Arrays are passed back via the MODSPRO module
            IF ( FIRSTCOMBO ) THEN

                CDEV = PROMPTFFILE( 'Enter logical name for COMBINATION PROFILE file',    &
            ...
            !.......  If first time for this pollutant, read the combo file
            !.......  It will store only for pollutant/type ENAM and SPCMAT_PERIOD
            ...
            !.......  Match the FIPS code to the combination profile
            F = FINDC( CFIP, NINVIFIP, INVCFIP )

            !.......  If FIPS code is not found, then internal error since these
            !    arrays are supposed to be built from inventory list of FIPs codes
            IF ( F <= 0 ) THEN
            ...
            !.......  Otherwise, store information from combination data, if found
            ELSE

                NPCOMBO = CMBNP( F )

                !.......  If FIPs code found, set the arrays for SETSOURCE_SMATS
                IF ( NPCOMBO > 0 ) THEN
                ...
                !.......  Otherwise, a COMBO was specified, but none found
                !.......  Note that NPCOMBO = 0 should cause loop in SETSOURCE_SMAT
                !    to be skipped
                ELSE
                    MESG = 'ERROR: No FIPS-specific entry in GSPRO_COMBO for:'//    &
                           CRLF() // BLANK10 //    &
                           'FIP: '// TRIM(CFIP) // ' and'//    &
                           'pollutant "'// TRIM( ENAM ) //'"'
        ...
        !.......  If this entry is not a combination profile, setup to simply
        !    use the single profile specified
        ELSE
        ...
        !.......  Write speciation profile codes and fractions for this source
        !    to the speciation supplemental file (to be used by SMKREPORT).
        ...
        !.......  Loop over all profiles in combination profile
        !.......  Note that for non-combo profiles, NPCOMBO has been set to 1
        !    and CCODE(1) to SPCODE.
        DO NP = 1, NPCOMBO

            VALID = .TRUE.
            K = MAX( INDEX1( CCODE( NP ), NSPROF, SPROFN ),0 )

            !.......  If specific profile is not found in set of profiles,
            !    try to apply the default for this pollutant.
            !    If COMBO or fractional, give a different error and
            !    don't look for the default)
            IF( K .LE. 0 ) THEN
                ...
                ELSE IF ( USEFRACS ) THEN
                    ...
                    MESG = 'ERROR: Speciation profile "'// CCODE(NP)//    &
                       '" is not in profiles, but it was assigned' //    &
                       CRLF() // BLANK10 // 'as as fractional-profile to source:' //    &
                       CRLF() // BLANK10 // BUFFER( 1:L2 ) //    &
                       ' SCC: ' // TSCCINIT // ' POL: ' // EANAM( V )
                    IF( ERRCNT(5) <= MXERR ) CALL M3MESG( MESG )
                    ERRCNT(5) = ERRCNT(5) + 1
            ...
            IF( NCNV4 .GT. 0 .AND. VALID ) THEN
                ...
                !.......  CNVFC00( V ) will equal 1.0 if it has not been set, so
                !    there is no need for error checking
                ELSE
                    CNVFAC = CNVFC00( V )
            ...
            !.......  Screen for pollutant-to-pollutant conversion factors by checking
            !    if they have been allocated
            ELSE IF( ALLOCATED( CNVRT03 ) .AND. VALID ) THEN

                !.......  If have a combination profile, then this is not a valid
                !    way to set the pollutant-to-pollutant factors
                IF( USECOMBO ) THEN
                ...
                !.......  Error check there is no conversion fac found
                ELSE
                    CNVFAC = CNVFC00( V )
                    CALL FMTCSRC( CSRC, NCOUT, BUFFER, L2 )
                    IF( CNVFAC == 1.0 .AND. CNVFLAG( V ) ) THEN
            ...
            !.......  If they don't exist, simply set the conversion factor to one
            ELSE
            ...
            IF( K .GT. 0 ) THEN
                ...
                DO N = 1, NTBL
                    ...
                    IF( MASSOUT ) THEN
                        MASSMATX( S,J )= MASSMATX( S,J ) + CWEIGHT( NP ) * CNVFAC * MASSFACT( I )
                    END IF

                    IF( MOLEOUT ) THEN
                        MOLEMATX( S,J )= MOLEMATX( S,J ) + CWEIGHT( NP ) * CNVFAC * MOLEFACT( I )
                    END IF
