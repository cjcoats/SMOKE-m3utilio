...
MODULE MODLISTS
    ...
    !.......  Full list of inventory pollutants/activities (in output order)
    !.......  Dimensioned by MXIDAT
    ...
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: INVDNAM( : )     ! data name
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: INVDUNT( : )     ! units for SMOKE intmdt inventory
    CHARACTER(DDSLEN3), ALLOCATABLE, PUBLIC :: INVDDSC( : )     ! inventory data description

    !.......  Inventory table arrays - unsorted raw data, dimensioned by NINVTBL
    ...
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: ITNAMA ( : )     ! data name
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: ITUNTA ( : )     ! units for SMOKE intmdt inventory
    ...
    !.......  CAS codes in sorted order WITH DUPLICATES and reference index back to
    !           input order - dimensioned by NINVTBL
    INTEGER,            ALLOCATABLE, PUBLIC :: SCASIDX( : )     ! index to ITNAMA
    CHARACTER(CASLEN3), ALLOCATABLE, PUBLIC :: SORTCAS( : )     ! CAS code (left justified)

    !.......  CAS codes in sorted order WITHOUT DUPLICATES and count of pollutants
    !           for each CAS code (0 pollutants indicates that no pollutants for that
    !           CAS are kept) - dimensioned by NUNIQCAS
    ...
    !.......  SPECIATE4 IDs in sorted order and reference index back to input order
    !           Dimensioned by MXIDAT
    INTEGER, ALLOCATABLE, PUBLIC :: IDXCOD ( : )     ! index to INVDNAM
    INTEGER, ALLOCATABLE, PUBLIC :: SORTCOD( : )     ! SPECIATE4 ID

    !.......  Arrays for per CAS reports - dimensioned by NUNIQCAS
    !           The array order is the same as UNIQCAS
    ...
    !.......  Arrays for CAS/pollutant combo reports - dimensioned by NINVTBL
    !           The array order is the same as SORTCAS
