...
MODULE MODREPRT
    ...
    CHARACTER(RPKTLEN), PARAMETER, PUBLIC :: ALLPCKTS( NALLPCKT ) = &
                                    (/ '/CREATE REPORT/      ',     &
                                       '/REPORT TIME/        ',     &
                                       '/AVEDAY/             ',     &
                                       '/NEWFILE/            ',     &
    ...
    TYPE :: EACHRPT
        ...
        CHARACTER(NAMLEN3) :: SPCPOL            ! pollutant for BYSPC
    ...
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: SPCPOL( : )     ! pols for BYSPC
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: ASCNAM( : )     ! pols from ASCII elevated file
    ...
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: INDNAM ( :,: )     ! var nams
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: OUTDNAM( :,: )     ! var nams
    CHARACTER(NAMLEN3), ALLOCATABLE, PUBLIC :: RDNAMES( :,: )     ! for reads
    CHARACTER(MXDLEN3), ALLOCATABLE, PUBLIC :: ALLUSET( :,: )     ! units
    ...
    CHARACTER(MXDLEN3), PUBLIC :: UNITSET      ! current line units
    CHARACTER(LENTTL3), PUBLIC :: TITLE        ! current line title

        ! Output units for each output data column
    CHARACTER(MXDLEN3), ALLOCATABLE, PUBLIC :: OUTUNIT( : )
