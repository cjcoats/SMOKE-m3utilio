...
SUBROUTINE RDINVDATA( FDEV, FNAME, NRAWBP, NONPOINT )
    ...
    USE MODMOBIL, ONLY: SCCMAPFLAG, SCCMAPLIST, NSCCMAP, EXCLSCCFLAG
    ...
    CHARACTER(16), PARAMETER :: PROGNAME =  'RDINVDATA'     ! program name
    ...
    DO
        ...
        IF( IOS > 0 ) THEN
            EFLAG = .TRUE.
            WRITE( MESG,94010 ) 'I/O error', IOS, 'reading inventory file at line', IREC
            CALL M3MESG( MESG )
            CYCLE
        END IF
        ...
        IF( IOS < 0 ) THEN
            ...
            IF( LSTFLG ) THEN
                ...
                IF( CURFIL <= NLINE ) THEN
                    ...
                    ELSE
                        WRITE( MESG,94010 )    &
                          'Successful OPEN for inventory file(s):' // CRLF() //    &
                          BLANK5 // TRIM( INFILE )
                        CALL M3MSG2( MESG )
    ...
    !                       SMOKE doesn't need emission factor, so why create warning?
                ...
                DO J = 0, NPOLPERCAS - 1
                    ...
                    IF( CURFMT == ORLFMT  .OR. CURFMT == ORLNPFMT .OR.    &
                        ...
                        IF( POLANN > 0. ) THEN
                            EMISBYPOL( SCASPOS ) =  EMISBYPOL( SCASPOS ) + POLANN
                        END IF
            ...
            IF( CATEGORY == 'POINT' ) THEN
                ...
                IF( CFLAG ) THEN
                    RBUF = 0.25 * PI * STKDM( CURSRC ) * STKDM( CURSRC )
    ...
    IF( EFLAG ) THEN
        MESG = 'Error reading data from inventory file ' // TRIM( FNAME )
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    !.......  Sort inventory data by source
    CALL M3MESG( 'Sorting inventory data by source and pollutant...' )
