...
SUBROUTINE WRPDEMIS( DAYFLAG, JDATE, JTIME, TIDX, NPDSRC, NVAR,     &
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'WRPDEMIS'     !  program name
    ...
    IF( FIRSTIME .AND. .NOT. HOURFLAG ) THEN
        ...
        ALLOCATE( NOMISS( NSRC,NVASP ),    &
                  EAIDX2( MXEA ), STAT=IOS )
        ...
        IF( INTGRFLAG ) THEN
            ...
            ALLOCATE( NHAPMOD( NNHV ),    &
                      NHAPPOS( NNHV ), STAT=IOS )
    ...
    CALL SORTI2( NPDPT( TIDX ), IDXSRC( 1,TIDX ), SPDIDA( 1,TIDX ), CIDXA( 1,TIDX ) )
    ...
    DO I = 1, NOUT
        ...
        IF ( PDDATA( I,V ) .LT. AMISS3 ) THEN
            ...
            !.......  For fires data, reset "missing" values to zero,
            !         since Temporal will not need to know the difference
