...
SUBROUTINE RDMEDSPD( FDEV, TZONE, TYPNAM, LASTFLAG )
    ...
    !.......   Local list of bad sources to prevent duplicate writing of error messages
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDMEDSPD'     !  program name
    ...
    IF( FIRSTIME ) THEN
        ...
        ALLOCATE( BADSRC( NSRC ),       &
                STARTSRC( NINVIFIP ),   &
                  ENDSRC( NINVIFIP ), STAT=IOS )
        CALL CHECKMEM( IOS, 'BADSRC,,,ENDSRC', PROGNAME )
    ...
    DO             !  Head of period-specific file read loop
        ...
        !         same file.
        !.......  If the file is hourly but the only the daily is to be read,
        !         then behave as if it is a daily file.
