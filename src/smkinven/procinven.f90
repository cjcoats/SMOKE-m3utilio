...
SUBROUTINE PROCINVEN( NRAWBP, UDEV, YDEV, CDEV, LDEV )
    ...
    !
    !  DESCRIPTION:
    !      This subroutine sorts and stores the inventory data including source
    !      specific characteristics and emissions data.
    !      Many places in the in-line documentation refers to pollutants, but
    !      means pollutants or activity data
    !
    ...
    !.......   Variables dimensioned by subroutine arguments
    INTEGER         TMPSTAT( MXIDAT )     ! tmp data status
    ! NOTE: GET RID OF THIS ALLOCATION METHOD    !
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'PROCINVEN'     ! program name

    !***********************************************************************
    !   begin body of subroutine PROCINVEN
    ...
    !.......  Allocate memory for sorted inventory data
    ALLOCATE( NPCNT( NSRC ),    &
             POLVAL( NRAWBP,NPPOL ),    &
            IPOSCOD( NRAWBP ), STAT=IOS )
    CALL CHECKMEM( IOS, 'IPOSCOD', PROGNAME )

    !.......  Initialize pollutant/activity-specific values.
    !.......  Initialize annual and average day values with 0.
    !.......  Inititalize integer
    !           values with the real version of the missing integer flag, since
    !           these are stored as reals until output
    ...
    !.......  Initialize pollutant count per source array
    NPCNT = 0      ! array

    !.......  Initialize temporary data status
    TMPSTAT = 0      ! array
    ...
    DO I = 1, NRAWBP
        ...
        !.......  Update pointer for list of actual pollutants and activities
        TMPSTAT( POLCOD ) = 2
        ...
        !.......  Skip rest of loop if duplicates are not allowed
        IF( EFLAG ) THEN
            LS     = S
            PIPCOD = POLCOD
            PCAS   = CASNUM
        ...
        !.......  Check if current pollutant is an activity
        ACTFLAG = .FALSE.
        IF( INVSTAT( POLCOD ) < 0 ) ACTFLAG = .TRUE.

        !.......  Reset emissions values to zero, if it's negative
        IF ( POLVLA( J, NEM ) < 0 .AND.    &
            ...
            IF ( NWARN < MXWARN .AND. POLCOD /= PIPCOD ) THEN
                CALL FMTCSRC( CSOURC( S ), NCHARS, BUFFER, L2 )
                    ...
                    !.......  If negative are not okay
                IF( NEGOK ) THEN
                    IF( ACTFLAG ) THEN
                        MESG = 'WARNING: Negative inventory data retained for:' //    &
                               CRLF() // BLANK5 // BUFFER( 1:L2 )
                    ELSE
                        MESG = 'WARNING: Negative annual data retained for:' //    &
                                CRLF() // BLANK5 // BUFFER( 1:L2 )
                    END IF

                    !.......  If negative values are not okay
                ELSE
                    IF( ACTFLAG ) THEN
                        MESG = 'WARNING: Negative inventory data reset to zero for:' //     &
                               CRLF() // BLANK5 // BUFFER( 1:L2 )
                    ELSE
                        MESG = 'WARNING: Negative annual data reset to zero for:' //    &
                                CRLF() // BLANK5 // BUFFER( 1:L2 )
                    END IF
                END IF
        ...
        IF( .NOT. ACTFLAG ) THEN
            IF ( POLVLA( J, NDY ) < 0 .AND.    &
                ...
                IF ( NWARN < MXWARN .AND. POLCOD /= PIPCOD ) THEN
                    ...
                    !.......  If negative emissions are okay
                    IF( NEGOK ) THEN
                        MESG = 'WARNING: Negative average day data retained for:' //    &
                               CRLF() // BLANK5 // BUFFER( 1:L2 )

                    !.......  If negative emissions are not okay
                    ELSE
                        MESG = 'WARNING: Negative average day data reset to zero for:' //    &
                               CRLF() // BLANK5 // BUFFER( 1:L2 )
                    END IF
        ...
        !.......  For a new source or a new pollutant code...
        IF( S /= LS .OR. POLCOD /= PIPCOD ) THEN

            !.......  Sum up the number of pollutants/activities by source,
            !                   but do this here only, because this part of the IF
            !                   statement is for new pollutants
            NPCNT( S ) = NPCNT( S ) + 1
            K = K + 1
            ...
            !.......  Store position of the pol/act in the
            !                   master list in sorted array.
            IPOSCOD( K ) = POLCOD
        ...
        ELSE
            ...
            !.......  Continue in loop if zero emissions
            ELSE
                CYCLE
    ...
    !.......  Report the number of records that were duplicated
    IF( IDUP /= 0 ) THEN
        WRITE( MESG,94010 ) 'NOTE: The number of duplicate records was', IDUP, '.'

        IF( .NOT. DFLAG ) THEN
            MESG = TRIM( MESG ) // CRLF() // BLANK10 //    &
                   'The inventory data were summed for these sources.'
        END IF
