...
SUBROUTINE RDCEMPD( FDEV, TZONE, TSTEP, MXPDSRC, GETSIZES,      &
    ...
    !***************************************************************************
    !  subroutine body starts at line 189
    !
    !  DESCRIPTION:
    !      This subroutine reads the hourly CEM data, matches it with
    !      the inventory data, and gets set up to output PHOUR and ASCII formats
    !      with hourly data.
    !
    ...
    CHARACTER(NAMLEN3), PARAMETER :: CEMPOLS( NCEMPOL ) =       &
        (/ 'NOX             ', 'SO2             ' /)
    ...
    CHARACTER(NAMLEN3) :: ENAME      ! emis i/o api inven logical name
    CHARACTER(NAMLEN3) :: ANAME      ! emis ASCII inven logical name
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDCEMPD'     !  program name

    !***********************************************************************
    !   begin body of program RDCEMPD
    ...
    IF( FIRSTIME ) THEN
        ...
        IF( SFLAG ) THEN
            SFLAG = .FALSE.
            MESG = 'NOTE: Ignoring HOURLY_TO_DAILY setting for reading CEM emissions data'
            CALL M3MSG2( MESG )
        END IF

        !.......  Get environment variable for calculating flow rate
        MESG = 'Heat input factor for calculating flow rate (ft^3/MMBTU)'
        FLOWFAC = ENVREAL( 'FLOW_RATE_FACTOR', MESG, 0.0, IOS )
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "FLOW_RATE_FACTOR"', 2 )
        END IF
        FLOWPOS = MXSPDAT + CODFLAG3

        IF( FLOWFAC > 0. ) THEN
            MESG = 'NOTE: Hourly flow rates will be calculated from CEM data'
            CALL M3MSG2( MESG )
        ELSE
            MESG = 'NOTE: Hourly flow rates will not be calculated from CEM data'
            CALL M3MSG2( MESG )
        END IF

        !.......  Store location of CEM pollutants in master pollutants array
        CEMPIDX = 0                ! array
        DO V = 1, NCEMPOL
        ...
        !.......  Create unique lists with ORIS
        ORISFLAG = .TRUE.
        CALL GENUSLST

        !.......  Read CEM summary information
        CALL RDCEMSUM
    ...
    !.......  Allocate memory for bad ORIS IDs
    IF ( .NOT. GETSIZES             .AND.    &
    ...
    IF( FIRSTCALL ) THEN
        ...
        !.......  If SDATE and EDATE have been set by the data, then compute
        !         total time steps in the data
        IF( DFLAG ) THEN
        ...
        !.......  Set up counters for number of times this routine has been called
    ...
    !.......  Get count of records per time step if we're ready
    !.......  Every sources that matches the master list of ORIS/boilers
    !         will have emissions for every time step
    IF( GETCOUNT ) THEN
        IF( MXPDPT( 1 ) == 0 ) THEN
        ...
        !.......  Allocate memory to indicate if the CEM data contains entries
        !               for a given ORIS/boiler and time step
    ...
    !           1. In the first section, determine the minimum and maximum date.
    !              Use a reference date to do this.
    !           2. In the second section, read and store the data.  When storing
    ...
    DO             !  Head of period-specific file read loop
        ...
        !.......  Skip comments or blank lines (also includes #CEM header)
        ...
        !.......  There is no error checking to help speed things up
        ...
        !.......  Write message about which record number we're on
        ...
        !.......  Skip entry if not in master list of ORIS/boilers
        !.......  If CEM summary and input CEM data are consistent, the only
        !               non-matches will be entries skipped by CEMScan
        MASOBPOS = FINDC( CORS // BLID, NOBRLIST, OBRLIST )
        IF( MASOBPOS <= 0 ) CYCLE

        !.......  Search for ORIS ID in ORIS/FIPs code list and then get time
        !               zone from FIPS/Time zone list.
        INVORPOS = FINDC( CORS, NINVORIS, INVORIS )

        !.......  If ORIS ID is not found in list...
        IF( INVORPOS <= 0 ) THEN
            ...
            ELSE IF( NUNFDORS > 0 ) THEN
                ...
                !.......  Check to see if ORIS in in list
        ...
        !.......  Skip entry if not in inventory list of ORIS/boilers
        ...
        !.......  Ensure that year of data is consistent with base year
        IF ( YEAR .NE. BYEAR ) YFLAG = .TRUE.

        JDATE = 1000 * YEAR + JULIAN( YEAR, MONTH, DAY )

        !.......  Convert from times 0 through 23 to HHMMSS
        JTIME = HH * 10000
        ...
        !.......  If only getting dates, go to next loop iteration
        IF( GETSIZES ) CYCLE
        ...
        !.......  Store indicator for this ORIS/boiler and time step
        MASLIST( MASOBPOS, PTR ) = .TRUE.

        !.......  Get number of inventory sources for this ORIS/boiler
        NS = OBSRCNT( INVOBPOS )
        S1 = OBSRCBG( INVOBPOS )

        ALLZERO = .FALSE.

        !.......  Compute factor for calculate hourly emissions; check that
        !               hourly value is valid
        ...
        !.......  Compute emissions from rate and heat input and adjust
        ...
        DO V = 1, NIPPA
            ...
            !.......  Check annual emissions for each source; annual emissions
            !         are always used to calculate hourly emissions, so check
            ...
            DO I = 0, NS - 1
                ...
                IF( EMIS( S,V ) < 0. ) THEN
                    EFLAG = .TRUE.
                    WRITE( MESG,94010 ) 'ERROR: Missing "' // TRIM( EANAM( V ) ) //     &
                        '" emissions for source number ', S
                    CALL M3MESG( MESG )
                END IF
            END DO
            ...
            !.......  Check if pollutant is CEM pollutant
            CEMPOL = .FALSE.
            DO N = 1, NCEMPOL
                IF( CEMPIDX( N ) == V ) THEN

                    !.......  Only use CEM data for NOX and SO2 if annual emissions are non-zero
                    IF( N == SO2IDX ) THEN
            ...
            IF( CEMPOL ) THEN
                ...
                !.......  Create weights for allocating CEM emissions to the
                !                       sources based on the emissions in the annual inventory
                DENOM = 0.
                ...
                !.......  If annual emissions are all zero, use uniform weights
                IF( DENOM == 0. ) THEN
                    DO I = 0, NS - 1
                        S = OBSRCNM( S1 + I )
                        EMIS( S,V ) = 1.
        ...
        IF( FLOWFAC > 0. .AND. HTINPUT > 0. ) THEN
            ...
            !.......  Calculate rate for each stack corresponding to ORIS/boiler
            MXSTKS = OBSRCNT( INVOBPOS )

            IF( ALLOCATED( STKNAM ) ) DEALLOCATE( STKNAM, STKVAL )
            ALLOCATE( STKNAM( MXSTKS ),    &
                      STKVAL( MXSTKS ), STAT=IOS )
            ...
            DO I = 0, NS - 1
                ...
                CALL STORE_SOURCE_DATA( S, PTR, FLOWPOS, STKVAL( J ) )
            END DO
...
299 CONTINUE       ! Exit from read loop
    ...
    !.......  If this is the last time through this routine, make sure that
    !           all ORIS/boilers are accounted for
    IF( .NOT. GETSIZES .AND.    &
        .NOT. GETCOUNT .AND.    &
        NCALL == MXCALL      ) THEN
