...
SUBROUTINE OPENPDOUT( NPDSRC, NVAR, TZONE, SDATE, STIME, TSTEP,    &
    ...
    USE MODINFO, ONLY: CATDESC, CRL, BYEAR, NIPOL, NIACT, EANAM
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'OPENPDOUT'     ! program name
    ...
    ELSE IF( TYPNAM .EQ. 'hour' ) THEN
        VARNAM = 'INDXH'
    ELSE
        MESG = 'INTERNAL ERROR: Type name "'// TYPNAM // '" not recognized'
    ...
    DO V = 1, NVAR
        ...
        IF( PFLAG ) THEN
            UNITS3D( J ) = 'n/a'
            VDESC3D( J ) = TYPNAM // '-specific ' // TRIM( VBUF ) // ' diurnal profile'

        !....... If outputs are data values...
        ELSE
            UNITS3D( J ) = 'ton/' // TYPNAM
            VDESC3D( J ) = TYPNAM // '-specific ' // TRIM( VBUF ) // ' data'
        END IF
    ...
    DO V = 1, MXSPDAT
        IF( SPSTAT( V ) > 0 ) THEN
            ...
            VDESC3D( J ) = TYPNAM // '-specific ' // TRIM( SPDATDSC( V ) ) // ' data'
    ...
    FDESC3D( 1 ) = CATDESC // TRIM( TYPNAM ) // '-specific source inventory'
    ...
    CALL UPCASE( SCRNAM )
    IF( PFLAG ) THEN
        MESG = 'Enter logical name for ' // TRIM( SCRNAM ) // '-SPECIFIC PROFILES output file'
        FNAME = CRL // TYPNAM // 'PRO'

    ELSE
        MESG = 'Enter logical name for ' // TRIM( SCRNAM ) // '-SPECIFIC output file'
        FNAME = CRL // TYPNAM
    END IF
    ...
    IF ( FILFMT .EQ. CEMFMT .AND. RDEV .LE. 0 ) THEN
        ...
        RDEV = PROMPTFFILE( MESG, .FALSE., .TRUE.,  'REPINVEN', PROGNAME )
