...
SUBROUTINE OPENGRWOUT( ENAME, FYEAR, NAME1, SFLAG, OFLAG,       &
                       ODEV, DDEV, VDEV, RDEV, ONAME, VARPATH )
    ...
    IF( SFLAG ) THEN
        ...
        IF ( ODEV .LT. 0 ) THEN         !  failure to open
            ...
            MESG = 'Could not open INVENTORY MAP file: "' // TRIM( NNAME ) // '"'
            CALL M3MSG2( MESG )
        ...
        IF( .NOT. DESCSET( ENAME,ALLFILES ) ) THEN
            MESG = 'Could not read description for "' // TRIM( ENAME ) // '"'
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF
    ...
    IF( OFLAG ) THEN
        MESG  = 'Enter logical name for the ORL EMISSIONS output file'
