...
SUBROUTINE ASGNNHAPX
    ...
    !
    !  DESCRIPTION:
    !      For each source, find the most specific match to determine
    !      if the source should be excluded from the calculation of NONHAPVOC
    !      pollutant in the inventory.  The algorithm allows for all four
    !      levels of SCC matching.
    !
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'ASGNNHAPX'     ! program name

    !***********************************************************************
    !   begin body of subroutine ASGNNHAPX

    !.......  Allocate memory for source-based array from MODLISTS
    ...
    DO S = 1, NSRC
        ...
        IF ( CATEGORY == 'POINT' ) THEN
            ...
            !.......  Look for plant/char/scc assignments at various levels
            ...
            !.......  Look for plant/char assignments at various levels
            ...
            !.......  Look for plant/SCC and plant assignments
            F1 = FINDC( CHK11, TXCNT( 11 ), CHRT11 )
            F0 = FINDC( CHK10, TXCNT( 10 ), CHRT10 )

            !.......  Choose the most specific assignment first
            IF( F6 .GT. 0 ) THEN
                LNONHAP( S ) = .FALSE.
        ...
        !.......  Try for any FIPS code  SCC matches; then any Cy/st code     &
        !               SCC matches
    ...
    !.......  Now try for county or state match prior to SCC-only match,
    !               which is different from other SMOKE assignment priorities.
    !               This is different because it is likely that an entire
        !               state or county must be excluded from the NONHAP calculation.
    ...
    !.......  Reverse non-HAP exclusions to inclusions if the header is
    !           defined as '/INCLUDE/' in NHAPEXCLUDE file
