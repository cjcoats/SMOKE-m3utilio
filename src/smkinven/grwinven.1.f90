...
!.........  Open country, state, and county file
    ...
    IF( ZDEV .LT. 0 ) THEN
        MESG = 'Could not open COUNTRY, etc., file'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF
...
!.........  If using ORL output format, open and read inventory table
    IF( ORLFLAG ) THEN
        ...
        IF( ZDEV .LT. 0 ) THEN
            MESG = 'Could not open INVENTORY TABLE'
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF
        CALL RDCODNAM( IDEV )
    END IF
...
!.................  Check if projection matrix base year is consistent with
!                   inventory
...
!.........  If area sources and XLOC and YLOC are being used, then
!           set flag
...
!                   and projection matrices
            ...
            IF( K .GT. 0 ) THEN
                ...
                IF( CTYPE(I) .NE. CTYPPROJ ) THEN  ! not for PROJECTION packet/matrix
                    ...
                    IF( .NOT. READSET( CNAME( I ), 'CE_'//VARBUF,&
                    ...
                    IF( .NOT. READSET( CNAME( I ), 'RP_'//VARBUF,&
