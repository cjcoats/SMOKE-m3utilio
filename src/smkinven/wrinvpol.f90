...
SUBROUTINE WRINVPOL( CATEGORY, INPATH, INNAM, NREC,    &
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'WRINVPOL'     !  program name
    ...
    ELSE IF( .NOT. OPENSET( FNAME, FSNEW3, PROGNAME )) THEN
        ...
        MESG = 'ERROR: Could not open I/O API "' // TRIM( VBUF )//  &
               '" file for file name:' // CRLF() // BLANK10 // TRIM( FPHYS )
        CALL M3MSG2( MESG )

    END IF

    !.......  Write source ID index
    IF( .NOT. WRITESET( FNAME, 'SRCID', ALLFILES,0,0, SRCID               )) THEN
        EFLAG = .TRUE.
    ...
    DO J = 1, NPVAR
        ...
        IF( VTYPESET( J+1 ) .EQ. M3REAL ) THEN
            IF( .NOT. WRITESET( FNAME, VNAMESET( J+1 ), ALLFILES,0,0, POLBUF(1,J) ) ) MSGFLAG = .TRUE.

        ELSE
            INTBUF( 1:NREC ) = INT( POLBUF( 1:NREC, J ) )      ! array
            IF( .NOT. WRITESET( FNAME, VNAMESET( J+1 ),  ALLFILES,0,0, INTBUF ) ) MSGFLAG = .TRUE.
        END IF

        IF( MSGFLAG ) THEN
            EFLAG = .TRUE.
            MESG = 'Could not write "'// TRIM( VNAMESET(J+1) )    &
                   // '" to file:'// CRLF()// BLANK10// TRIM( FPHYS )
    ...
    IF( .NOT. CLOSESET( FNAME ) ) THEN
        MESG = 'Could not close file:'//CRLF()//BLANK10// TRIM( FPHYS )
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF
