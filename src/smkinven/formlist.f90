...
SUBROUTINE FORMLIST
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'FORMLIST'        !  program name
    ...
    DO F = 1, NCOMP
        ...
        IF( V1 .GT. 0 ) THEN
            TFLAG = .TRUE.
            MESG = 'ERROR: Variable "'//TRIM( VIN_A( F ) ) // '" is an'//    &
        ...
        IF( V2 .GT. 0 ) THEN
            TFLAG = .TRUE.
            MESG = 'ERROR: Variable "'// TRIM( VIN_B( F ) ) //'" is an'//    &
                   'activity, which is not allowed in a formula.'
            CALL M3MSG2( MESG )
        END IF

        IF( TFLAG ) THEN
            WRITE( MESG,94010 ) 'ERROR: Problem processing formula', F,     &
                                ': "'//TRIM(FORMULAS(F)) // '"'
            CALL M3MSG2( MESG )
        END IF
