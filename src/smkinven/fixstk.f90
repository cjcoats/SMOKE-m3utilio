...
SUBROUTINE  FIXSTK( FDEV, NSRC )
    ...
    DO S = 1, NSRC
        ...
        IF ( HT .GT. MAXHT ) THEN
            IF( NWARN <= MXWARN )  WRITE( LDEV,94030 ) 'Height', HT, MAXHT
            HT = MAXHT

        ELSEIF( HT .LT. MINHT .AND. HT .GT. 0 ) THEN
            IF( NWARN <= MXWARN )  WRITE( LDEV,94040 ) 'Height', HT, MINHT
            HT = MINHT
        ...
        ELSEIF( DM .LT. MINDM .AND. DM .GT. 0 ) THEN
            IF( NWARN <= MXWARN )  WRITE( LDEV,94040 ) '  Diam', DM, MINDM
            DM = MINDM

        END IF

        IF ( TK .GT. MAXTK )THEN
            IF( NWARN <= MXWARN )  WRITE( LDEV,94030 ) '  Temp', TK, MAXTK
            TK = MAXTK

        ELSEIF( TK .LT. MINTK .AND. TK .GT. 0 ) THEN
            IF( NWARN <= MXWARN )  WRITE( LDEV,94040 ) '  Temp', TK, MINTK
            TK = MINTK

        END IF

        IF ( VE .GT. MAXVE )THEN
            IF( NWARN <= MXWARN )  WRITE( LDEV,94030 ) ' Veloc', VE, MAXVE
            VE = MAXVE

        ELSEIF( VE .LT. MINVE .AND. VE .GT. 0 ) THEN
            IF( NWARN <= MXWARN )  WRITE( LDEV,94040 ) ' Veloc', VE, MINVE
            VE = MINVE
    ...
    DO S = 1, NSRC
        ...
        IF ( STKHT( S ) .LE. 0.0 ) THEN
            ...
            IF( K .LE. 0 )  K = FINDC( CFIPSCCZ, NR3, TBL3 )
            ...
            ELSE
                ...
                IF( K .LE. 0 )  K = FINDC( CSTASCCZ, NR2, TBL2 )
            ...
            IF( .NOT. DFLAG( S ) ) THEN
                ...
                IF( NWARN <= MXWARN )  WRITE( LDEV,94020 ) 'Height', STKHT( S ), HT
                STKHT( S ) = HT

                IF ( STKDM( S ) .LE. 0 ) THEN
                    IF( NWARN <= MXWARN )  WRITE( LDEV,94020 ) '  Diam', STKDM( S ), DM
                    STKDM( S ) = DM
                END IF

                IF ( STKTK( S ) .LE. 0 ) THEN
                    IF( NWARN <= MXWARN )  WRITE( LDEV,94020 ) '  Temp', STKTK( S ), TK
                    STKTK( S ) = TK
                END IF

                IF ( STKVE( S ) .LE. 0 ) THEN
                    IF( NWARN <= MXWARN )  WRITE( LDEV,94020 ) ' Veloc', STKVE( S ), VE
                    STKVE( S ) = VE
                END IF
            END IF
    ...
    DO S = 1, NSRC
        ...
        IF( DFLAG( S ) ) THEN
            ...
            IF ( STKHT( S ) .LE. 0 ) THEN
                IF( NWARN <= MXWARN )  WRITE( LDEV,94020 ) 'Height', STKHT( S ), HT0
                STKHT( S ) = HT0
            END IF

            IF ( STKDM( S ) .LE. 0 ) THEN
                IF( NWARN <= MXWARN )  WRITE( LDEV,94020 ) '  Diam', STKDM( S ), DM0
                STKDM( S ) = DM0
            END IF

            IF ( STKTK( S ) .LE. 0 ) THEN
                IF( NWARN <= MXWARN )  WRITE( LDEV,94020 ) '  Temp', STKTK( S ), TK0
                STKTK( S ) = TK0
            END IF

            IF ( STKVE( S ) .LE. 0 ) THEN
                IF( NWARN <= MXWARN )  WRITE( LDEV,94020 ) ' Veloc', STKVE( S ), VE0
                STKVE( S ) = VE0
            END IF
