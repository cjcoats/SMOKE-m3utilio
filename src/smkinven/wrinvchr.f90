...
SUBROUTINE WRINVCHR( ENAME, SDEV, A2PFLAG, NONPOINT )
    ...
    !
    !  DESCRIPTION:
    !      This subroutine writes the source characteristics to the
    !      I/O API and ASCII area, mobile, or point source inventory files
    !
    ...
    USE MODINFO, ONLY: CATEGORY, NSRC, NCHARS, MXCHRS, SC_BEGP, SC_ENDP
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'WRINVCHR'     !  program name

    !***********************************************************************
    !   begin body of subroutine WRINVCHR

    !.......  Write the I/O API file, one variable at a time

    MESG = 'Error writing output file "' // TRIM( ENAME ) // '"'

    IF ( .NOT. WRITESET( ENAME, 'TZONES', ALLFILES,0,0, TZONES ) ) THEN
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    IF ( .NOT. WRITESET( ENAME, 'TPFLAG', ALLFILES,0,0, TPFLAG ) ) THEN
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    IF ( .NOT. WRITESET( ENAME, 'INVYR', ALLFILES,0,0, INVYR ) ) THEN
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF
    ...
    SELECT CASE( CATEGORY )
      CASE( 'AREA' )
        ...
        IF( A2PFLAG ) THEN
            IF ( .NOT. WRITESET( ENAME, 'XLOCA', ALLFILES,0,0, XLOCA ) ) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            END IF

            IF ( .NOT. WRITESET( ENAME, 'YLOCA', ALLFILES,0,0, YLOCA ) ) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            END IF
        END IF

        IF ( .NOT. WRITESET( ENAME, 'CELLID', ALLFILES,0,0, CELLID ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

      CASE( 'MOBILE' )

        IF ( .NOT. WRITESET( ENAME, 'IRCLAS', ALLFILES,0,0, IRCLAS ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'IVTYPE', ALLFILES,0,0, IVTYPE ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'XLOC1', ALLFILES,0,0, XLOC1 ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'YLOC1', ALLFILES,0,0, YLOC1 ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'XLOC2', ALLFILES,0,0, XLOC2 ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'YLOC2', ALLFILES,0,0, YLOC2 ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF
      ...
      CASE( 'POINT' )
        ...
        IF ( .NOT. WRITESET( ENAME, 'STKHT', ALLFILES,0,0, STKHT ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'STKDM', ALLFILES,0,0, STKDM ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'STKTK', ALLFILES,0,0, STKTK ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'STKVE', ALLFILES,0,0, STKVE ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'FUG_HEIGHT', ALLFILES,0,0, FUGHGT ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'FUG_WIDTH', ALLFILES,0,0, FUGWID ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'FUG_LENGTH', ALLFILES,0,0, FUGLEN ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        IF ( .NOT. WRITESET( ENAME, 'FUG_ANGLE', ALLFILES,0,0, FUGANG ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

    END SELECT

    !.......  End subroutine if ASCII file is not to be written
    IF( SDEV .LE. 0 ) RETURN

    !.......  Set the number of potential ASCII columns in SDEV output file
    SELECT CASE( CATEGORY )
      CASE( 'AREA' )
        NASCII = MXARCHR3 + 4
    ...
    !.......  Allocate memory for and populate the output header fields from
    !           the source-specific fields
    ...
    !.......  Set positions for outputs to ASCII file that are after the source
    ...
    !.......  It is possible that a source characteristic (such as segment or
    !           VTYPE) may never be used, while SCC or link is defined and also
    !           considered a source characteristic.  So, make sure that if there
    !           are no gaps in the list of source characteristics, even though
    !           this means outputting blank fields.
    !.......  Find the most specific defined source characteristic
    ...
    !.......  Consider that every source might not have the same thing defined    !
    !.......  If a column is defined for _any_ source, then it must be
    !           written for all sources.  Set flags for which are defined.
    DO K = 1, NASCII
        LF( K ) = ( COLWID( K ) > 0 )
    END DO

    !.......  Set number of columns for ASCII file (initialize +1 b/c MXARCHR3
    !           does not include the SMOKE source ID column)
    NL = NASCII + 1
    DO K = 1, NASCII
        IF( .NOT. LF( K ) ) NL = NL - 1
    ENDDO

    !.......  Store the format statement in OUTFMT for well-formatted output
    WRITE( OUTFMT, 94100 ) '(I', COLWID0

    DO K = 1, NASCII
        IF( LF( K ) ) THEN
            WRITE( MESG, 94100 ) ',1X,A', COLWID( K )
            OUTFMT = TRIM( OUTFMT ) // MESG
        ENDIF
    ENDDO
    OUTFMT = TRIM( OUTFMT ) // ')'
    ...
    DO S = 1, NSRC
        ...
        !.......  Split source characteristics into separate fields (CHARS)
        CALL PARSCSRC( CSOURC( S ), MXCHRS, SC_BEGP, SC_ENDP, LF,    &
                       NC, CHARS )

        !.......  Store remaining source attributes in separate fields (CHARS)
        SELECT CASE ( CATEGORY )
          CASE( 'AREA' )
        ...
        !.......  Write source characteristics and attributes
