...
SUBROUTINE WRINVEMIS( IDEV, DATPATH )
    ...
    INTEGER      , PARAMETER :: RIMISS3  = REAL( IMISS3 )
    CHARACTER(16), PARAMETER :: FORMEVNM = 'SMKINVEN_FORMULA'
    CHARACTER(16), PARAMETER :: PROGNAME = 'WRINVEMIS'     !  program name
    ...
    IF( IOS .NE. 0 ) THEN
        EFLAG = .TRUE.
        MESG = 'ERROR: INVNAME1 environment variable is not defined'
        CALL M3MSG2( MESG )
    END IF
    ...
    IF( IOS .NE. 0 ) THEN
        EFLAG = .TRUE.
        MESG = 'ERROR: INVNAME2 environment variable is not defined'
        CALL M3MSG2( MESG )
    END IF
    ...
    NEGOK = ENVYN( 'ALLOW_NEGATIVE', 'Allow negative output data', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
    ...
    IF( NIPOL .GT. 0 )    &
        CALL LOOP_FOR_OUTPUT( NIPOL, NPPOL, IDEV, EIIDX, EINAM, MCNT )

    !.......  Loop through formula-based pollutants, compute, and write to inventory file
    IF( NCOMP .GT. 0 )    &
        CALL OUTPUT_FORMULAS( NIPOL, NPPOL, IDEV, EIIDX, EINAM, MCNT )
    ...
    IF( NIACT .GT. 0 )  CALL LOOP_FOR_OUTPUT( NIACT, NPACT, IDEV, AVIDX, ACTVTY, MCNT )
...
CONTAINS
    ...
    SUBROUTINE LOOP_FOR_OUTPUT( NOUT, NPVAR, IDEV, INDX, NAMES, MCNT )
        ...
        DO I = 1, NOUT
            ...
            IF( NREC .EQ. 0 ) THEN
                MESG = 'WARNING: no sources with data for output '//    &
                       ...
                       //CRLF()//BLANK10// 'File will not be written.'
                CALL M3MSG2( MESG )
                CYCLE
            END IF
            ...
            IF( K .LE. 0 ) THEN
                MESG = 'ERROR: Cannot find pollutant "'//   &
                       TRIM( EONAMES( I,1 ) ) //            &
                       '" from  inventory in INVTABLE file.'
                CALL M3MSG2( MESG )
                EFLAG = .TRUE.
                CYCLE
            END IF
...
94020   FORMAT( 10( A, :, E10.2, :, 1X ) )
    ...
    !               inventory data.
    !.......  Most variables are defined from the main subroutine
    SUBROUTINE OUTPUT_FORMULAS( NOUT, NPVAR, IDEV, INDX, NAMES, MCNT )
        ...
        DO F = 1, NCOMP
            ...
            IF( NREC .EQ. 0 ) THEN
                ...
                MESG = 'ERROR: no sources with data for output '//    &
                       ...
                       //CRLF()//BLANK10//    &
                       'Variable "'//TRIM(VNAME(F))//'" will not be computed.'
                CALL M3MSG2( MESG )
                CYCLE
            END IF
            ...
            DO N = 1, NREC
                ...
                IF( NPVAR .GT. 1 )  COMPUTED( S,2 ) = MAX( 0., SRCPOL( N,2 ) )     ! Average day value

                IF( NPVAR .GT. 2 )  COMPUTED( S,3:NPVAR ) = SRCPOL( N,3:NPVAR )
            ...
            DO N = 1, NREC
                ...
                IF( CHKPLUS( F ) ) THEN         ! Formula is an addition
                    ...
                    COMPUTED( S,1 ) = COMPUTED( S,1 ) + MAX( 0., SRCPOL( N,1 ) )

                    IF ( NPVAR .GT. 1 )    &
                         COMPUTED( S,2 ) = COMPUTED( S,2 ) + MAX( 0.,SRCPOL( N,2 ) )
                ...
                ELSE IF( CHKMINUS( F ) ) THEN        ! Formula is a subtraction
                    ...
                    IF( COMPUTED( S,1 ) .LT. 0 ) THEN
                        ...
                        !.......  If warning count is less than max and not fires.
                        IF ( WARNCNT_A .LE. MXWARN .AND. .NOT. FIREFLAG ) THEN
