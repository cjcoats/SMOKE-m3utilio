...
SUBROUTINE CHKLSTFL( NLINE, FNAME, NLSTSTR, FILFMT )
    ...
    LOGICAL   :: EMSFLAG            !  true: at least one file is EMS format
    LOGICAL   :: ORLFLAG            !  true: at least one file is ORL format

    CHARACTER(300)  INFILE          !  input file line buffer
    CHARACTER(500)  MESG            !  message buffer

    CHARACTER(16), PARAMETER :: PROGNAME =  'CHKLSTFL'     ! program name
    ...
    DO J = 1, NLINE
        ...
        IF( I .GT. 0 ) THEN
            ...
            ELSE IF( INDEX( INFILE, 'ORL' ) > 0 ) THEN
                ...
                ELSE
                    EXTFORMAT = ORLFMT
                    WRITE( MESG,94010 ) 'WARNING: No format included '//    &
                      'with #LIST ORL entry in inventory input file.'//     &
                      CRLF() // BLANK5 // 'Assuming ORL point, nonroad, or on-road.'
        ...
        IF( ORLFLAG                   .AND.    &
            ...
            WRITE( MESG,94010 )                                         &
                   'ERROR: In SMOKE list-formatted inventory file, '    &
                   // TRIM( FNAME ) // ', at least one file is ' //     &
                   CRLF() // BLANK10 // 'ORL format while another is ' //    &
                   'not ORL format.' // CRLF() // BLANK10 //            &
