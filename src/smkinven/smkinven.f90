...
PROGRAM SMKINVEN
    ...
    USE MODSOURC, ONLY: CIFIP, CISIC, CSRCTYP, TZONES, CSCC, IDIU, IWEK, CINTGR, CEXTORL
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'SMKINVEN'       !  program name
    ...
    !.......  Set IOAPI gridded input file name, if available
    ...
    IF( IFLAG ) THEN
        ...
        IF( INV_MON == 0 ) THEN
            ...
            CALL M3MSG2( MESG )
        ELSE
            MESG = 'Processing ' // TRIM( MON_NAME( INV_MON ) ) // ' inventory'
            CALL M3MSG2( MESG )
        END IF
    ...
    END IF      ! For gridded I/O API inventory
    ...
    IF( IFLAG ) THEN
        ...
        CALL OPENINVOUT( A2PFLAG, GRDNM, ENAME, ANAME, MDEV, SDEV, ADEV, VARPATH )

        MESG = 'Writing SMOKE ' // TRIM( CATEGORY ) // ' SOURCE INVENTORY file...'
