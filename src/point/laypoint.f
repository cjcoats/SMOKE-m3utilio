...
C.........  Make sure only run for point sources
        IF( CATEGORY .NE. 'POINT' ) THEN
            MESG = 'ERROR: ' // PNAME( 1:LEN_TRIM( PNAME ) ) //
...
C.............  Check for plume top and plume bottom
     ...
     &                 'to allocate plumes for some sources.'
                ...
                CALL CHECKMEM( IOS, 'PLMBOT', PNAME )
...
C.........  Allocate per-source, per-source-per-layerarrays, 
        ALLOCATE( HFX( NSRC ),
     &            HMIX( NSRC ),
     &            TSFC( NSRC ),
     &           USTAR( NSRC ),
...
C.............  Layer fractions, per-layer arrays for all sources,
C.............  tmp gridded, layered cross-point met data
...
C----------------------------------------------------------------------
     ...
     &                 'to a variable name from file '// FILNAM
                    CALL M3MSG2( MESG )
                END IF
