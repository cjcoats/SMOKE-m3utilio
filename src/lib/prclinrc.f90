...
SUBROUTINE PRCLINRC( IREC, NSEGS, LINE, SEGMENT )
    ...
    !
    !  DESCRIPTION:
    !    The PRCLINRC processes the REPCONFIG instructions by setting variables
    !    that can be used by the calling routine(s) for futher processing.
    !    This routine is used for several cases.  It is be able to interact
    !    with various other routines at various stages of processing. It resolves
    !    any implies report settings that affect input file requirements (e.g.,
    !    BY CELL implies GRIDDING). Settings are passed to other routines with
    !    modules.
    !
    !  PRECONDITIONS REQUIRED:
    !    REPCONFIG file is opened
    !    LINE read, separated into SEGMENTS, and left-justified
    !
    ...
    USE MODREPRT, ONLY: GRPNRECS, INPACKET, INGROUP, INREPORT,      &
                        ...
                        LAB_IDX, LENLAB3, DLFLAG, MATFLAG, NFDFLAG
    ...
    !...........   Local parameters
    CHARACTER(1),  PARAMETER :: SCCLEV( NSCCLV3 ) = (/ '1', '2', '3', '4', '5' /)
    CHARACTER(16), PARAMETER:: PROGNAME = 'PRCLINRC'     ! program name
    ...
    !***********************************************************************
    !   begin body of subroutine PRCLINRC

    !.........  Initialize for start of file
    ...
    !.........  Section for resetting for start of file (only access for read loops
    !           2 and higher)
    IF( IREC .LT. PREC ) THEN
        FIRSTLOOP = .FALSE.

        !.............  As a double check, make sure that the source category has been
        !               set
        IF( .NOT. LCATSET ) THEN
            RC_ERROR = .TRUE.
    ...
    !.........  Section for first loop
    IF( FIRSTLOOP ) THEN

        !.............  Check for source category
        !.............  Warning if it is reset to something else
    ...
    !.........  Initializations needed for every line
    LIN_DEFGRP  = .FALSE.
    LIN_GROUP   = .FALSE.
    LIN_SUBDATA = .FALSE.
    LIN_SUBGRID = .FALSE.
    ...
    !.........  Set length of full line
    L2 = LEN_TRIM( LINE )

    !.........  If a packet is not currently being read, determine what packet
    !           starts on the current line
    !.........  Will skip line if no packet has been found and the line does not
    !           start with "/"
    IF( .NOT. INPACKET ) THEN

        J = INDEX( LINE( 2:L2 ), '/' )

        !.............  Error if packet started but does not end with a slash
        IF( LINE( 1:1 ) .EQ. '/' .AND. J .LE. 0 ) THEN
            ...
            !.............  Packet found. Interpret it...
        ELSE IF( J .GT. 0 ) THEN
            J = J + 1

            !.................  Set packet name and extract group label
            PCKTNAM = LINE( 1:J )

            !.................  Find packet name in list of valid names
            PKT_IDX = INDEX1( PCKTNAM, NALLPCKT, ALLPCKTS )

            !.................  Warning if packet is not recognized
            ...
            ELSE
                ...
                !.....................  Count the number of groups and the number of reports
                PKTCOUNT ( PKT_IDX ) = PKTCOUNT( PKT_IDX ) + 1
                PKTSTATUS( PKT_IDX ) = .TRUE.

                INPACKET = .TRUE.

                !.....................  Set first line number of packet
                PKTSTART = IREC

                !.....................  Packet-specific processing
                SELECT CASE( PKT_IDX )

                    !.....................  A reporting time packet
                  CASE( TIM_IDX )

                    IF( J+1 .GE. L2 ) THEN
                        !.........................  Ensure reporting time is set
                    ...
                    !.....................  An average day data usage packet
                  CASE( ADY_IDX )

                    !.........................  Ensure average day setting made
                    IF( J+1 .GE. L2 ) THEN
                        CALL NO_SETTING_FOUND( IREC, PKT_IDX )
                  ...
                  CASE( FIL_IDX )
                    ...
                    !.........................  Set default logical/physical file name
                    WRITE( FILNUM,'(I6)' ) PKTCOUNT ( FIL_IDX )
                    FILNUM = ADJUSTL( FILNUM )
                    FIL_ONAME = 'REPORT' // FILNUM

                    !.........................  Extract file name (logical or physical, if any)
                    IF( J+1 .LE. L2 ) THEN
                    ...
                    !.....................  A new delimiter packet
                  CASE( DEL_IDX )

                    !.........................  Ensure delimeter setting made
                    IF( J+1 .GE. L2 ) THEN
                        CALL NO_SETTING_FOUND( IREC, PKT_IDX )
                    ELSE
                    ...
                    !.....................  A region group or subgrid definition packet
                    !.....................  Reset group specific to defaults
                  CASE( REG_IDX, SBG_IDX )
                    ...
                    !.......................  If a region packet, read state/county file
                    IF( PKT_IDX .EQ. REG_IDX ) YFLAG = .TRUE.

                    !.....................  Specification of elevated groups, PinG sources, or
                    !                       elevated sources
                  CASE( ELG_IDX, PNG_IDX, ELV_IDX )
                    ...
                    !.....................  Set a label - figure out what the label is...
                  CASE( LAB_IDX )
                    ...
                    !.........................  If label is too long, it will be truncated
                    ...
                    !.........................  Check if label is an environment variable
                    IF( TMPLABEL2( 1:1 ) == '$' ) THEN

                    !.............................  Try to evaluate environment variable
                        ENVBUF = TMPLABEL2( 2:LENLAB3 )
                        CALL ENVSTR( ENVBUF,' ',' ',TMPLABEL2,IOS )

                    !.............................  If E.V. can't be evaluated, write message
                        IF( IOS /= 0 ) THEN
                            WRITE( MESG,94010 )&
                    ...
                    !.........................  If label is set to OFF, then turn it off
                    ELSE IF( TMPLABEL2( 1:3 ) == 'OFF' ) THEN
                        LLABEL = .FALSE.

                    !.........................  If not environment variable, then set label for report
                    ELSE
                        LLABEL = .TRUE.
                        TMPLABEL = TMPLABEL2
                    END IF
                  ...
                  CASE( RPT_IDX )
                    ...
                    !.........................  If have not found a NEWFILE packet, then
                    !                           set the first output file name
                    ...
                    RPT_%SPCPOL    = ' '
                    TITLE          = ' '
                    IF( .NOT. LDELIM )  RPT_%DELIM = ';'        ! default to semi-colon
                    IF( LLABEL       )  RPT_%LABEL = TMPLABEL
    ...
    ELSE IF( SEGMENT( 1 ) .EQ. '/END/' ) THEN
        ...
        !.............  Resolve status of settings that cannot be determined
        !               definitively until the end of the packet...

        !.............  If one report contains the ASCIIELEV instruction,
        !               all reports must contain it.
        IF( PKTCOUNT( RPT_IDX ) .GT. 1 .AND. ASCFLAG ) THEN
        ...
        !.............  If packet was a report packet and no data have
        !               been selected, then set input file global setting
        IF( INREPORT .AND. RPT_%NUMDATA .LE. 0 ) THEN
            DATAMISS = .TRUE.
        END IF

        !.............  If using elevated settings and no layer fractions, make sure
        !               PELV file will be read in
        ...
        !.............  In in a group and no records read, give warning
        IF( INGROUP .AND. GRPNRECS .EQ. 0 .AND. FIRSTLOOP ) THEN
            MESG = 'WARNING: No valid records found for group "'// TRIM( GRP_LABEL ) // '"'
            CALL M3MSG2( MESG )
        END IF

        !.............  Reset all packet-specific settings
        GRPNRECS   = 0
        PKTEND     = IREC
        INPACKET   = .FALSE.
        INGROUP    = .FALSE.
        ...
        !........  Missing end of packet and already in a packet
    ELSE IF( LINE( 1:1 ) .EQ. '/' ) THEN
        RC_ERROR = .TRUE.
        WRITE( MESG,94010 ) 'ERROR: Missing end of packet at line', IREC
        CALL M3MSG2( MESG )
    ...
    !.........  General group processing
    IF( INGROUP ) THEN

        !.............  Check the status of INCLUDE versus EXCLUDE
        ...
        !.............  If neither, count entries in the current group definition
        ELSE
            LIN_GROUP = .TRUE.
            GRPNRECS = GRPNRECS + 1
    ...
    IF( INSPCIFY ) THEN
        ...
        !.............  Determine how many conditions appear for this line
        J = NSEGS
        I = 0
        DO
            I = I + 1
        ...
        !.............  Increment the OR field count (one per row)
        SPCF_NOR  = SPCF_NOR + 1

    END IF

    !.............  Check current line for report settings
    !.............  Set global and line-by-line logical variables, shared through
    !               MODREPRT
    IF( INREPORT ) THEN
        ...
        SELECT CASE( SEGMENT( 1 ) )
          ...
          CASE( 'ARRANGE' )
            ...
            !...............  If number of variables per report is greater than the maximum,
            !                 then reset to maximum value
            ...
            !.............  ASCII elevated sources file used for report
          CASE( 'ASCIIELEV' )
            ...
            !.............  Check control or projection matrix versus report
          CASE( 'CHECK' )
            ...
            !.............  Multiplicative controls used for report
          CASE( 'CONTROL' )
            CUFLAG        = .TRUE.
            RPT_%USECUMAT = .TRUE.

            !.............  Gridding used for report
          CASE( 'GRIDDING' )
            IF( .NOT. AFLAG ) THEN
                GFLAG      = .TRUE.
                RPT_%USEGMAT  = .TRUE.
            ...
            !.............  Source mapping for report
          CASE( 'CROSSWALK' )
            IF( CATEGORY .EQ. 'POINT' ) RPT_%SRCMAP = .TRUE.

            !.............  Projection used for report
          CASE( 'PROJECTION' )
            PRFLAG        = .TRUE.
            RPT_%USEPRMAT = .TRUE.

            !.............  Reactivity controls used for report
          CASE( 'REACTIVITY' )
            CRFLAG        = .TRUE.
            RPT_%USECRMAT = .TRUE.

            !.............  Speciation used for report
          CASE( 'SPECIATION' )
            ...
            !.............  Temporal allocated emission used for report
          CASE( 'TEMPORAL' )
            TFLAG     = .TRUE.
            RPT_%USEHOUR = .TRUE.
            RPT_%BYDATE = .TRUE.

            !.............  AERMOD support report
          CASE( 'AERMOD' )
            ...
            !.............  BY options affecting inputs needed
          CASE( 'BY' )
            ...
            SELECT CASE( SEGMENT( 2 ) )
              ...
              CASE( 'LAYER' )
                IF( NOT_ASCIIELEV( 'BY ' // SEGMENT( 2 ) ) ) THEN
                    IF( CATEGORY .EQ. 'POINT' ) THEN
                        ...
                        !.........................  Daily layered emission is set to Y if BYHOUR is not set at this point
              ...
              CASE( 'SCC' )
                IF( NOT_ASCIIELEV( 'BY ' // SEGMENT( 2 ) ) ) THEN
                    ...
                    ELSE
                        WRITE( MESG,94010 )                             &
                          'WARNING: BY SCC instruction at line',        &
                          IREC, 'does not include proper '//            &
              ...
              CASE( 'STACK' )
                ...
                IF( SEGMENT( 3 ) .EQ. 'STACKPARM' .OR.  &
                    SEGMENT( 4 ) .EQ. 'STACKPARM'      )  RPT_%STKPARM = .TRUE.

                IF( SEGMENT( 3 ) .EQ. 'LATLON' .OR.     &
                    SEGMENT( 4 ) .EQ. 'LATLON'      )     RPT_%LATLON = .TRUE.
            ...
            !.............  Setting for the QA extract data format for CARB
            !               Fixed setting coditions
            !               BY EIC and BY COABDIST (if not gridding)
            !               BY DAY is default and BY HOUR (optional)
            !
          CASE( 'CARB_QADEF' )
            ...
            !.............  Setting for the use of layer fractions
          CASE( 'LAYFRAC' )
            ...
            !.............  Setting for the normalize instruction
          CASE( 'NORMALIZE' )
            ...
            !.............  Settings the output data format
          CASE( 'NUMBER' )
            RPT_%DATAFMT = SEGMENT( 2 )

            !.............  SELECT options affecting inputs needed
          CASE( 'SELECT' )
            ...
            SELECT CASE( SEGMENT( 2 ) )
              ...
              CASE( 'REGION' )
                ...
                !.....................  Warning if used more than once
                ...
                LREGION = .TRUE.
                LIN_SUBREGN = .TRUE.
                CALL EXTRACT_LABEL( IREC, 'REGION', LINE, RPT_%REGNNAM )
              ...
              CASE( 'SUBGRID' )
                ...
                LSUBGRID = .TRUE.
                LIN_SUBGRID = .TRUE.
                CALL EXTRACT_LABEL( IREC, 'SUBGRID', LINE, RPT_%SUBGNAM )
          ...
          CASE( 'TITLE' )
            ...
            !.................  Keep any leading spaces in title with WRITE
            TITLE = ADJUSTL( LINE( 7:L2 ) )

          CASE( 'UNITS' )

            !.................  Get units to use for all output data
            ...
            !.................  In future, support data-specific unit setting
            ELSE
    ...
    !.........  Save variables from current call and exit from subroutine
999 CONTINUE
...
CONTAINS    !******************  INTERNAL SUBPROGRAMS  *****************************
    ...
    !.............  This internal subprogram prints a warning that lines of the
    !               report configuration file are being ignored
...
94010   FORMAT( 10( A, :, I8, :, 1X ) )
    ...
    !.............  This internal subprogram prints a warning that instruction
    !               is not valid for the current source category
...
94010   FORMAT( 10( A, :, I8, :, 1X ) )
    ...
    END SUBROUTINE WRONG_SOURCE_CATEGORY
        ...
        !.............  This internal subprogram prints a warning that no setting
        !               was made by a packet that could have, and the previous
    ...
    SUBROUTINE NO_SETTING_FOUND( IREC, IDX )
        ...
        WRITE( MESG,94010 ) 'WARNING: No value listed with "' //        &
               TRIM( ALLPCKTS( IDX ) ) // '" packet at' // CRLF() //    &
...
94010   FORMAT( 10( A, :, I8, :, 1X ) )
    ...
    !.............  This internal subprogram extracts the label of the group or
    !               subgrid from the SELECT instruction
    SUBROUTINE EXTRACT_LABEL( IREC, KEYWORD, LINE, LABEL )
        ...
        !.............  Determine first position after keyword
        K = INDEX( LINE, KEYWORD ) + LK + 1

        !.............  Make sure a label is there. If not, error.
        IF( LL .LE. K ) THEN
            ...
            !.............  If so, store it in output argument
        ELSE
...
94010   FORMAT( 10( A, :, I8, :, 1X ) )
    ...
    !.............  This internal function returns true if the ASCIIELEV instruction
    !               is not in use; otherwise it prints a warning and returns false
    LOGICAL FUNCTION NOT_ASCIIELEV( COMMAND )
        ...
        IF( RPT_%USEASCELEV ) THEN
            ...
            WRITE( MESG, 94010 )                                            &
               'WARNING: ' // TRIM( COMMAND ) // ' instruction at line',    &
               IREC, 'is not allowed with the ASCIIELEV instruction.'
            CALL M3MSG2( MESG )
        END IF
