...
SUBROUTINE WRORLOUT( RDEV, DATNAM, NREC, NPVAR, SRCID, SRCDAT, STATUS )
    ...
    !.......  For area sources...
      ...
      CASE ( 'POINT' )
        ...
        DO C = 1, NREC
            ...
            IF ( SEGMENT   .EQ. ' '      ) SEGMENT = '0'
            IF ( SCC(1:2)  .EQ. '00'     ) SCC     = SCC(3:SCCLEN3)
            IF ( MACT      .EQ. '000000' ) MACT    = '-9'
            IF ( MACT(1:2) .EQ. '00'     ) MACT    = MACT(3:MACLEN3)
            IF ( MACT      .EQ. ' '      ) MACT    = '-9'
            IF ( SRCTYPE   .EQ. ' '      ) SRCTYPE = '-9'
            IF ( ERPTYPE   .EQ. ' '      ) ERPTYPE = '-9'
            IF ( NAICS     .EQ. '000000' ) NAICS   = '-9'
...
93300 FORMAT( I5.5, ',', A, ',', A, ',', E15.8, ',', E15.8, ',', A, A )   ! onroad
