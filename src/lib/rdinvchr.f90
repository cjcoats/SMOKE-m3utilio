...
SUBROUTINE RDINVCHR( CATEGORY, INFILE, FDEV, NSRC, NVARS, VNAMES )

    !***********************************************************************
    !  program body starts at line
    !
    !  DESCRIPTION:
    !     Reads in SMOKE inventory characteristics for any source category
    !
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDINVCHR'       !  program name
    ...
    !***********************************************************************
    !   begin body of program RDINVCHR

    MESG = 'Reading source data from inventory file...'
    CALL M3MSG2( MESG )

    !.......  Loop through input variables
    !.......  Allocate memory and read the ones that are needed from I/O API file
    ...
    DO N = 1, NVARS
        ...
        SELECT CASE( INVAR )
          ...
          CASE( 'IRCLAS' )
            ...
            IF(.NOT. READSET(INFILE,'IRCLAS',ALLAYS3,1,0,0,IRCLAS)) THEN
          ...
          CASE( 'IVTYPE' )
            ...
            IF(.NOT. READSET(INFILE,'IVTYPE',ALLAYS3,1,0,0,IVTYPE)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'CELLID' )
            ...
            IF(.NOT. READSET(INFILE,'CELLID',ALLAYS3,1,0,0,CELLID)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'TZONES' )
            ...
            IF(.NOT. READSET(INFILE,'TZONES',ALLAYS3,1,0,0,TZONES)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'TPFLAG' )
            ...
            IF(.NOT. READSET(INFILE,'TPFLAG',ALLAYS3,1,0,0,TPFLAG)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'INVYR' )
            ...
            IF( .NOT. READSET(INFILE,'INVYR',ALLAYS3,1,0,0,INVYR)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'XLOCA' )
            ...
            IF( .NOT. READSET(INFILE,'XLOCA',ALLAYS3,1,0,0,XLOCA)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'YLOCA' )
            ...
            IF( .NOT. READSET(INFILE,'YLOCA',ALLAYS3,1,0,0,YLOCA)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'XLOC1' )
            ...
            IF( .NOT. READSET(INFILE,'XLOC1',ALLAYS3,1,0,0,XLOC1)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'YLOC1' )
            ...
            IF( .NOT. READSET(INFILE,'YLOC1',ALLAYS3,1,0,0,YLOC1)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'XLOC2' )
            ...
            IF( .NOT. READSET(INFILE,'XLOC2',ALLAYS3,1,0,0,XLOC2)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'YLOC2' )
            ...
            IF( .NOT. READSET(INFILE,'YLOC2',ALLAYS3,1,0,0,YLOC2)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'SPEED' )
            ...
            IF( .NOT. READSET(INFILE,'SPEED',ALLAYS3,1,0,0,SPEED)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'STKHT' )
            ...
            IF( .NOT. READSET(INFILE,'STKHT',ALLAYS3,1,0,0,STKHT) ) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'STKDM' )
            ...
            IF( .NOT. READSET(INFILE,'STKDM',ALLAYS3,1,0,0,STKDM) ) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'STKTK' )
            ...
            IF( .NOT. READSET(INFILE,'STKTK',ALLAYS3,1,0,0,STKTK)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
          ...
          CASE( 'STKVE' )
            ...
            IF( .NOT. READSET(INFILE,'STKVE',ALLAYS3,1,0,0,STKVE)) THEN
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
    ...
    !.......  Section if the ASCII file is also to be read in
    IF( FDEV .LE. 0 .AND. UNREAD .GT. 0 ) THEN
        ...
        DO J = 1, UNREAD
            ...
            MESG = 'INTERNAL ERROR: Program "' //TRIM( PROGNAME ) //    &
                   '" found unread variable "' //TRIM( INVAR ) //       &
    ...
    ELSEIF( FDEV .GT. 0 ) THEN
        ...
        !.......  Allocate memory for the data that are needed from the ASCII file
        ...
        DO J = 1, UNREAD
            ...
            SELECT CASE( INVAR )
              ...
              CASE DEFAULT
                EFLAG = .TRUE.
                MESG = 'INTERNAL ERROR: Program "' // TRIM( PROGNAME ) //   &
                    '" does not know about variable "' // TRIM( INVAR ) // '"'
                CALL M3MSG2( MESG )
       ...
       EXTIN = ( J .GT. 0 )
        ...
        END IF
            ...
            !C............  Depending on source category, read in and store ASCII source
            !               characteristics

        SELECT CASE ( CATEGORY )
          CASE ( 'AREA' )

            !.......  Determine if MACT code is present
            J = INDEX1( 'MACT code', NCOL, HEADER )
            MCTIN = ( J > 0 )

            !.......  Determine if NAICS code is present
            J = INDEX1( 'NAICS code', NCOL, HEADER )
            NAIIN = ( J > 0 )

            !.......  Determine if DOE plant ID is present
            J = INDEX1( 'DOE plant ID', NCOL, HEADER )
            ORSIN = ( J .GT. 0 )

            !.......  Determine if SIC code is present
            J = INDEX1( 'SIC', NCOL, HEADER )
            SICIN = ( J > 0 )

            !.......  If MACT not present but has been requested, then
            !                   internal err
            ...
            !.......  Advance to next line
                ...
                IF( EXTFLAG .AND. EXTIN ) CEXTORL( S ) = ADJUSTL( CEXT )
          ...
          CASE ( 'POINT' )
            ...
            !.......  Determine if boiler is present
            ...
            !.......  Determine if emission release code is present
            ...
            !.......  If emission release code not present but has been requested,
            !                   deallocate memory for array
            ...
            DO S = 1, NSRC
                ...
                !.......  Initialize temporary characteristics
                CHARS = ' '                  ! array
                ...
                IF( NEIFLAG .AND. NEIIN ) CNEIUID( S ) = ADJUSTR( CNEI )
                ...
                IF( CSRFLAG ) CALL BLDCSRC( CFIP, FCID, CHARS(1), CHARS(2), &
...
94010   FORMAT( 10( A, :, I10, :, 1X ) )
    ...
    SUBROUTINE READ_NEXT_VAL( VARIABLE )
        ...
        !.......  Extract string format and read variable
        FMTITEM = FMTITEM + 1
        CALL BUILD_FMTSEG( FMTSEGS( FMTITEM ) )
