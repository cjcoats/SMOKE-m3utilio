...
SUBROUTINE RDMAPPOL( NSRC, NVARS, NPVAR, VARNAMS, OUTVALS )
    ...
    DO V = 1, NVARS
        ...
        IF( VARNAMS( V )( 1:3 ) .EQ. AVEDAYRT ) THEN
            ...
            IF( NPVAR .GT. 1 ) THEN
                MESG = 'INTERNAL ERROR: Cannot specify average day value and have NPVAR > 1'
                CALL M3MSG2( MESG )
        ...
        CALL OPENPHYS( PROGNAME, RNAME, FSREAD3, MAPFIL( M ), EFLAG )
        ...
        IF( .NOT. CLOSESET( RNAME ) ) THEN
            MESG = 'Could not close file:'//CRLF()//BLANK10// MAPFIL( M )
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF
