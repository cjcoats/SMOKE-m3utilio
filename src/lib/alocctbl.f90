...
SUBROUTINE ALOCCTBL( NIPPA, ICSIZE )
    ...
    !
    !  DESCRIPTION:
    !      This subroutine allocates memory for the portion of the control
    !      cross-reference tables that contain the index to the control data, and
    ...
    !      Note that these tables are used multiple times in the same program for
    !      different control packets, which are processed one at a time.
    !
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'ALOCCTBL'     ! program name

    !***********************************************************************
    !   begin body of subroutine ALOCCTBL
