...
SUBROUTINE RDPELV( FDEV, NSRC, ASCIFLAG, NMAJOR, NPING )
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDPELV'     ! program name
    ...
    CALL CHECKMEM( IOS, 'LMAJOR...GROUPID', PROGNAME )
    ...
    DO               !  head of the FDEV-read loop
        ...
        IF( IMAJR .GT. NSRC .OR. IPING .GT. NSRC ) THEN
            ...
            WRITE( MESG,94010 ) 'ERROR: Source ID', NSRC,               &
                   ...
                   CRLF()//BLANK10 //'the maximum number of sourced!' //&
...
23  CONTINUE        !  end of read loop
    ...
    ELSE
        WRITE( MESG,94010 ) 'NOTE:', NPING,' PinG sources will be modeled'
    END IF
    CALL M3MSG2( MESG )
