...
SUBROUTINE BLDENAMS( CATEGORY_L, NIPPA_L, NPPOA, EANAM_L,&
    ...
    DATA PTPREFIX / AVEDAYRT, CTLEFFRT, RULEFFRT, EMISFCRT, CECOD1RT, CECOD2RT /
    ...
    DO I = 1, NIPPA_L
        ...
        IF( NAM .EQ. LNAM ) THEN
            ...
            ABRNAMA( J ) = NAM( 1:L2 ) // '_' // TRIM( BUFFER )    ! store name
        ELSE
            LCNT = 0
            LNAM = NAM
        ENDIF
    ...
    !               to the annual data description
        ...
        SELECT CASE( CATEGORY_L )
          CASE( 'AREA' )
            ...
            IF( POLLNUM > 0 ) THEN
                OUTDESCS( I,1 ) = TRIM( ARDESCS( 1 ) ) // ' for ' // INVDDSC( POLLNUM )
            ELSE
          ...
          CASE( 'MOBILE' )
            ...
            IF( POLLNUM > 0 ) THEN
                OUTDESCS( I,1 ) = TRIM( MBDESCS( 1 ) ) // ' for ' // INVDDSC( POLLNUM )
            ELSE
          ...
          CASE( 'POINT' )
            ...
            IF( POLLNUM > 0 ) THEN
                OUTDESCS( I,1 ) = TRIM( PTDESCS( 1 ) ) // ' for ' // INVDDSC( POLLNUM )
            ELSE
