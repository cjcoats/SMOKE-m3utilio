...
SUBROUTINE GETCTGRY
    ...
    CHARACTER(1), PARAMETER :: LETLIST( NLIST ) = (/ 'A', 'B', 'M', 'P', 'E' /)

    CHARACTER(6), PARAMETER :: CATTYPE( NLIST ) =   &
        (/ 'AREA  ', 'BIOGEN', 'MOBILE', 'POINT ', 'EVERY ' /)

    CHARACTER(6), PARAMETER :: CATLDSC( NLIST ) =   &
        (/ 'Area  ', 'Biogen', 'Mobile', 'Point ', 'Every ' /)

    CHARACTER(16), PARAMETER :: PNAME = 'GETCTGRY'        ! Program name
    ...
    IF( J .LE. 0 ) THEN
        ...
        MESG = 'ERROR: Do not recognize SMK_SOURCE environment variable setting "' &
               // CRL // '"'
        CALL M3MSG2( MESG )
