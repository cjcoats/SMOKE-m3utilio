...
SUBROUTINE DSCSPROF( FDEV, NIPOL, EINAM )
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'DSCSPROF'     ! program name
    ...
    IF( CRL == 'B' ) THEN
        ...
        IF( IOS .NE. 0 ) THEN
            MESG = 'ERROR: Variable BIOG_SPRO needs to be set to run'
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF
    END IF
    ...
    DO I = 1, NLINES
        ...
        IF( L .GT. NAMLEN3 ) THEN
            EFLAG = .TRUE.
            WRITE( MESG,94010 ) 'ERROR: Pollutant name exceeds ' //     &
                   'max width of', NAMLEN3, 'at line', IREC
            CALL M3MESG( MESG )
        END IF
        ...
        IF( L .GT. NAMLEN3 ) THEN
            EFLAG = .TRUE.
            WRITE( MESG,94010 ) 'ERROR: Species name exceeds ' //       &
                   'max width of', NAMLEN3, 'at line', IREC
            CALL M3MESG( MESG )
        END IF
