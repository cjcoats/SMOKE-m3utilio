...
SUBROUTINE RDAR2PT( FDEV, CDEV, LDEV )
    ...
    INTEGER, PARAMETER :: FBEG( NFIELDS ) = &
                        (/ 1 , 8, 11, 34, 39, 63, 86,  97, 114, 126 /)
    INTEGER, PARAMETER :: FEND( NFIELDS ) = &
                        (/ 6 , 9, 32, 37, 61, 84, 93, 109, 124, 139 /)
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDAR2PT'     ! program name
    ...
    DO J = 1, NINVSCC
        K = SCCIDX( J )
        WRITE( LDEV,94675 ) INVSCC( J ), SCCSECTN( K ), TRIM( SCCDESC( J ) )
    END DO
    WRITE( LDEV, 94010 ) ' '
...
CONTAINS    !******************  INTERNAL SUBPROGRAMS  *****************************
    ...
    SUBROUTINE READ_ARTOPT_FILE( STATUS, MXSCC )
        ...
        !.......  Abort with errors...
2002    WRITE( MESG,94010 ) 'ERROR: Unexpected end of area-to-point'//  &
