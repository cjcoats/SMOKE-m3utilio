...
SUBROUTINE RDGEOCODES( NCODES, INCODES )
    ...
    !
    !  DESCRIPTION:
    !       This subroutine opens and reads the four files that contain
    !       the codes and names of the geographic regions.
    !
    !  REVISION HISTORY:
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDGEOCODES'     ! program name

    !***********************************************************************
    !   begin body of subroutine RDGEOCODES
    ...
    DO FILENUM = 1, 4
        ...
        DO I = 1, NLINES
            ...
            IF( IOS > 0 ) THEN
                EFLAG = .TRUE.
                WRITE( MESG, 94010 ) 'I/O error', IOS, 'reading', TRIM( FILEDESC ), 'at line', I
                CALL M3MESG( MESG )
                CYCLE
            END IF
            ...
            !.......  Store fields in unsorted arrays
            K = K + 1
            INDEXA( K ) = K
            ...
            !.......  Read time zone from level 4 file
            IF( FILENUM == 4 ) THEN
                TZNAMA( K ) = ADJUSTL( SEGMENT( 3 )( 1:3 ) )

                !.......  Find time zone name in master list
                J = INDEX1( TZNAMA( K ), MXTZONE, TZONNAM )

                IF( J > 0 ) THEN
                    TZONEA( K ) = TZONNUM( J )

                !.......  Use default time zone if name is not found
                ELSE
                    WRITE( MESG,94010 )&
        ...
        !.......  Store number of codes and allocate memory for sorted data
        SELECT CASE( FILENUM )
          CASE( 1 )
            NGEOLEV1 = K
            ALLOCATE( GEOLEV1COD( NGEOLEV1 ),       &
                      GEOLEV1NAM( NGEOLEV1 ), STAT=IOS )
            CALL CHECKMEM( IOS, 'GEOLEV1NAM', PROGNAME )
          CASE( 2 )
            NCOUNTRY = K
            ALLOCATE( CTRYCOD( NCOUNTRY ),      &
                      CTRYNAM( NCOUNTRY ), STAT=IOS )
            CALL CHECKMEM( IOS, 'CTRYNAM', PROGNAME )
          CASE( 3 )
            NSTATE = K
            ALLOCATE( STATCOD( NSTATE ),        &
                      STATNAM( NSTATE ), STAT=IOS )
            CALL CHECKMEM( IOS, 'STATNAM', PROGNAME )
          CASE( 4 )
            NCOUNTY = K
            ALLOCATE( CNTYCOD( NCOUNTY ),       &
                      CNTYNAM( NCOUNTY ),       &
                     CNTYTZON( NCOUNTY ),       &
                     CNTYTZNM( NCOUNTY ),       &
                     USEDAYLT( NCOUNTY ), STAT=IOS )
        ...
        !.......  Check for duplicates and store sorted codes
        PREVCODE = ' '
        DO I = 1, K
