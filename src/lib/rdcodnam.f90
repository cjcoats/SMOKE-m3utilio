...
SUBROUTINE RDCODNAM( FDEV )
    ...
    !
    !  DESCRIPTION:
    !     Reads the inventory table to obtain a list of valid pollutants and
    !     activities and their properties. The routine converts all names to
    !     uppercase and allocates the MODLISTS arrays needed for storing the
    !     inventory table. It creates sorted lists of CAS numbers, both with
    !     duplicates and without. The subroutine returns the names in their
    ...
    !     Updated Aug 2006 by M. Houyoux for SMOKE 2.3 to add mode and change
    !         internal code documentation from SAROAD to SPECIATE4 ID.
    !     Updated Sep 2006 by M. Houyoux to automatically add _NOI pollutants
    !         for entries that are no-integrate model species.
    !
    ...
    CHARACTER(NAMLEN3), ALLOCATABLE :: INVDUNTA( : )     ! units for SMOKE intmdt inventory
    ...
    !.......  Get file sizes and allocate memory...

    !.......  Get no. lines in pollutant codes  activities files for allocating
    !           memory
    IF( FDEV .GT. 0 ) THEN
        NINVTBL = GETFLINE( FDEV, 'Inventory data table file' )
    END IF

    !.......  Allocate memory for storing raw unsorted inventory table
    ALLOCATE(  ITIDXA( NINVTBL ),       &
              ITIDXA2( NINVTBL ),       &
              ITLINNO( NINVTBL ),       &
               ITCODA( NINVTBL ),       &
               ITNTIA( NINVTBL ),       &
    ...
    !.......  Error if data name is not provided
        IF( L .LE. 0 ) THEN
            ...
            EFLAG = .TRUE.
            WRITE( MESG,94010 ) 'ERROR: Data name is blank at line', IREC
            CALL M3MSG2( MESG )
...
22  CONTINUE            !  exit from loop reading FDEV
    ...
    IF( NDAT .GT. NINVTBL ) THEN
        EFLAG = .TRUE.
        WRITE( MESG,94010 ) 'ERROR: Number of pollutant records :', NDAT,   &
    ...
    !.......  Abort if error found during read
    IF ( EFLAG ) THEN
        MESG = 'Problem with INVTABLE input file contents.'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    !.......  Exclude comment lines from count of inventory table entries and
    !           calculate dropped entries
    NINVTBL = NDAT
    NINVDROP = NINVTBL - NINVKEEP

    !.......  Sort by CAS number then pollutant name
    CALL SORTIC( NINVTBL, ITIDXA, ITCASDNMA )

    !.......  Count the number of unique CAS numbers.  Blank CAS numbers
    !           will be first in the sorted list, so they will not be counted.
    PCAS =  ' '
    DO N = 1, NINVTBL
        J = ITIDXA( N )
    ...
    !.......  Allocate memory for sorted CAS number arrays
    !.......  Allocate memory for reporting of emissions (used in reader routines)

    ALLOCATE( SCASIDX( NINVTBL ),       &
              SORTCAS( NINVTBL ),       &
              UCASIDX( NUNIQCAS ),      &
             UCASNPOL( NUNIQCAS ),      &
             UCASNKEP( NUNIQCAS ),      &
    ...
    !.......  Store sorted CAS numbers in both sorted order arrays
    PCAS =  ' '
    NUNIQCAS = 0
    CNT      = 0
    CNTK     = 0
    DO N = 1, NINVTBL
        ...
        !.......  Increment unique count if this CAS is different than previous
        !.......  Also, reset count of records per CAS and keep status
        ...
        !.......  Give warning if keep status for this iteration is not the same as
        !               that for the current CAS code
        ...
        !.......  Store the data units if not yet set, otherwise, check that its the same
        IF( ITCASDSCA( J ) .NE. DSCVAL ) THEN
            WRITE( MESG,94010 ) 'WARNING: Different descriptions '//    &
                   'for the same CAS number at line', IREC, '.'
            CALL M3MSG2( MESG )
        END IF
        ...
        !.......  Set previous CAS for next iteration
        PCAS = ITCASA( J )

    END DO

    !.......  Sort inventory table to create unique data list and associated arrays
    CALL SORTIC( NINVTBL, ITIDXA2, ITNAMA )

    !.......  Count the number of unique data names that are kept
    LNAM = ' '
    NNOI = 0
    MXIDAT = 0
    DO N = 1, NINVTBL
        ...
        !.......  Check if this pollutant  name is not the same as the previous one
        IF( ITNAMA( J ) .NE. LNAM ) THEN

            !.......  Count the number of unique data names
            MXIDAT = MXIDAT + 1

            !.......  If pollutant is a part of VOC or TOG, is "no-integrate"
            !           and a model species, increase the count for inserting
            !           the _NOI pollutants.
    ...
    !.......  Allocate memory for local inventory pollutants/activities
    !.......  Allocate memory for output inventory pollutants/activities
    !.......  Allocate memory for sorted SPECATE4 IDs and index

    ALLOCATE( LOCATIDX( MXIDAT ),&
               INVPOSA( MXIDAT ),&
              INVDCODA( MXIDAT ),&
              INVSTATA( MXIDAT ),&
               LADDNOI( MXIDAT ),&
    ...
    !.......  Store kept pollutants and activities in original (unsorted) order
    LNAM = ' '
    MXIDAT = 0
    DO N = 1, NINVTBL
        J = ITIDXA2( N )

        !.......  If current data name is not kept, skip to next
        IF( .NOT. ITKEEPA( J ) ) CYCLE

        IREC = ITLINNO( J )

        !.......  For the next data name...
        IF( ITNAMA( J ) .NE. LNAM ) THEN
            MXIDAT = MXIDAT + 1

        !.......  Reset keep status for current inventory data value
            KEEPVAL = ITKEEPA( J )

        END IF

        !.......  Store first position in original table
        IF( INVPOSA( MXIDAT ) .EQ. 0. ) THEN
            LOCATIDX ( MXIDAT ) = MXIDAT
            INVPOSA( MXIDAT ) = REAL( INDEX1( ITNAMA( J ), NINVTBL, ITNAMA ) )
        END IF

        !.......  Store SPECIATE4 ID if not yet set, otherwise, check that its the same code
        ...
        ELSE IF( ITCODA( J ) .NE. INVDCODA( MXIDAT ) ) THEN
            WRITE( MESG,94010 ) 'WARNING: Different SPECIATE4 ' //  &
                   ...
                   INVDCODA( MXIDAT ), 'and ignoring code', ITCODA( J )
            CALL M3MSG2( MESG )
        ...
        !.......  Store activity status if not yet set, otherwise, check that its the same
        ...
        !.......  Store VOC/TOG status if not yet set, otherwise, check that its the same
        ...
        !.......  Store the data description if not yet set, otherwise, check that its the same
        ...
        !.......  Store the CAS number if not yet set, otherwise, check that its the same
        ...
        ELSE IF( ITDSCA( J ) .NE. INVDDSCA( MXIDAT ) ) THEN
            WRITE( MESG,94010 ) 'WARNING: Different descriptions '//    &
                   ...
                   CRLF()// BLANK16// '"'// TRIM( ITDSCA( J ) ) // '".'
            CALL M3MSG2( MESG )

        END IF

        !.......  Give warning if keep status for this iteration is not the same as
        !               that for the current data name
        IF( ( ITKEEPA( J ) .AND. .NOT. KEEPVAL ) .OR.   &
            ( .NOT. ITKEEPA( J ) .AND. KEEPVAL )      ) THEN
            BUF1 = 'N'
            IF( KEEPVAL ) BUF1 = 'Y'
        ...
        !.......  Error msg for exact duplicate entries
        !              : added to prevent doubling emssions by exact duplicate entries (ex: PM10)
        ...
        !.......  Insert no-integrate entries (memory already increased with NNOI variable)
        IF ( ( ITVTSA(J) .EQ. 'V' .OR. ITVTSA(J) .EQ. 'T' ).AND.&
               ITMSPC( J ) .AND. .NOT. ITEXPL( J ) ) THEN
        ...
        !.......  Set data previous name for next iteration
        LNAM = ITNAMA( J )
    ...
    !.......  Resort inventory data names based on original position in inventory table
    ...
    DO N = 1, MXIDAT
        ...
        !.......  Reset LOCATIDX for use in next sorting step
        LOCATIDX( N ) = N

    END DO

    !.......  Sort data list by SPECIATE IDs
    CALL SORTI1( MXIDAT, LOCATIDX, INVDCOD )

    !.......  Store SPECIATE IDs in sorted order
    DO N = 1, MXIDAT
        J = LOCATIDX( N )
        IDXCOD( N )  = J
        SORTCOD( N ) = INVDCOD( J )
