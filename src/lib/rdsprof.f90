...
SUBROUTINE RDSPROF( FDEV, POLNAM, NMSPC )
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDSPROF'     ! program name
    ...
    DO
        ...
        !........  If we haven't already passed the headers, skip all header lines
...
120 CONTINUE        ! End of read on input file
    ...
    DO I = 1, NSPFUL
        ...
        IF( TMPPRF .EQ. PPRF   .AND. SPECNM .EQ. PSPCNM ) THEN
...
CONTAINS
    ...
    SUBROUTINE READ_GSPRO_HEADER( FDEV, STATUS )
        ...
        !               for local counters.
        IF ( FIRSTIME ) THEN
            ALLOCATE( NCNT( MXIDAT ),           &
                      NHAPINDX( MXIDAT ),       &
                      NHAPFLAG( MXIDAT ),       &
                      NHAPUSPC( MXIDAT ), STAT=IOS )
            CALL CHECKMEM( IOS, 'NCNT...NHAPUSPC', PROGNAME )
            FIRSTIME = .FALSE.
        END IF
        ...
        !          of headers, and get the maximum number of entries per header.
