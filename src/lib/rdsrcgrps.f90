...
SUBROUTINE RDSRCGRPS( SGDEV, SPCFLAG, TSFLAG )
    ...
    !
    !  DESCRIPTION:
    !     This subroutine reads the source grouping file and assigns sources
    !     to groups.
    !
    !  PRECONDITIONS REQUIRED:
    !     File unit SGDEV is open
    !     State and country codes have been loaded by INITSTCY
    !
    !  SUBROUTINES AND FUNCTIONS CALLED:
    !
    !  REVISION  HISTORY:
    !       Created 5/2013 by Dongmei Yang
    ...
    USE MODMERGE, ONLY: NSRCGRP, IGRPNUM, ISRCGRP,                      &
                        ...
                        NMSPC, NSTEPS,                                  &
                        AENAME, ASDEV, MENAME, MSDEV, PENAME, PSDEV
    ...
    LOGICAL, EXTERNAL :: BLKORCMT, CHKINT, USEEXPGEO

    !...........   Local parameters
    INTEGER, PARAMETER :: MXCOL = 5
    INTEGER, PARAMETER :: SRCLEN = FIPLEN3+SCCLEN3+PLTLEN3+CHRLEN3

    CHARACTER(16), PARAMETER :: PROGNAME = 'RDSRCGRPS'     ! program name
    ...
    !**********************************************************************
    !   begin body of subroutine RDSRCGRPS

    !.........  Read source characteristics needed for matching from inventory

    !.........  Check if Movesmrg has already loaded the data it needs
    ...
    !.........  Allocate memory for unsorted data
    ALLOCATE( INDEXA( NLINES ),     &
            IGRPNUMA( NLINES ),     &
            CGRPSRCA( NLINES ), STAT=IOS )
    CALL CHECKMEM( IOS, 'INDEXA...CGRPSRCA', PROGNAME )

    !.........  Read lines and store unsorted data
    IREC   = 0
    N      = 0
    ...
    !.........  Write warning if no groups match inventory
    IF( .NOT. EFLAG .AND. N .EQ. 0 ) THEN
        MESG = 'WARNING: All sources assigned to default group.'
        CALL M3MSG2( MESG )
    END IF

    !.........  Check for errors reading file, and abort
    IF( EFLAG ) THEN
    ...
    !.........  Save total number of source groups; add additional spot
    !           for default group
    NSRCGRP = N + 1

    !.........  Allocate memory for sorted groups
    ALLOCATE( IGRPNUM( NSRCGRP ),       &
              CGRPSRC( N ), STAT=IOS )
    CALL CHECKMEM( IOS, 'CGRPSRC', PROGNAME )

    !.........  Sort source groups by FIPS code
    ...
    DO I = 1, N
        ...
        !.............  Check for duplicate group info
        CSRC = CGRPSRCA( J )
    ...
    !.........  Sort source groups by group ID
    ...
    ALLOCATE( IUGRPNUM( NGRPS ),        &
              IUGRPIDX( MXGRPNUM ), STAT=IOS )
    ...
    !.........  Set default source group
    IGRPNUM( NSRCGRP ) = 0

    !.........  Output source groups report
    RDEV = PROMPTFFILE( 'Enter name for SOURCE GROUPS REPORT file',     &
                        .FALSE., .TRUE., 'SRCGRP_REPORT', PROGNAME )

    !.........  Assign sources to source groups
    ...
    DO I = 1, NSRC
        ...
        IF( PFLAG ) THEN
            ...
            !.................  full FIPS, blank SCC, plant, point
    ...
    !.........  Reassign elevated stack groups
    IF( PFLAG .AND. NGROUP .NE. 0 ) THEN

        ALLOCATE( ELEVGRPID( NSRC ),            &
                  COMBOGRPS( NSRC ), STAT=IOS )
    ...
    !.............  Allocate storage for summed emissions per group
        EMELEVGRP = 0.

    END IF

    !.........  Determine number of source group / grid cell interactions
    NSGOUTPUT = 0
    ...
    IF( AFLAG ) THEN
        CALL SRCGRPCNT( NSRC, ANGMAT, AGMATX( 1 ), AGMATX( NGRID+1 ) )
    END IF
    ...
    IF( MFLAG ) THEN
        CALL SRCGRPCNT( NSRC, MNGMAT, MGMATX( 1 ), MGMATX( NGRID+1 ) )
    END IF

    IF( PFLAG ) THEN
        CALL SRCGRPCNT( NSRC, NSRC, PGMATX( 1 ), PGMATX( NGRID+1 ) )

    !.............  Increment number of output records to account
    !               for elevated sources
        NSGOUTPUT = NSGOUTPUT + NELEVGRPS
    END IF

    !.........  Upate no of srcgrp when SUBSECFLAG output is set to Y
    IF( SUBSECFLAG ) NSRCGRP = NGRPS

    !.........  Allocate memory for emissions data
    ALLOCATE( EMGGRD( NGRID, NSRCGRP ), STAT=IOS )
    CALL CHECKMEM( IOS, 'EMGGRD', PROGNAME )
    EMGGRD = 0.      ! array

    !.........  Allocate additional arrays for Movesmrg processing
    IF( SPCFLAG ) THEN
        IF( TSFLAG ) THEN
