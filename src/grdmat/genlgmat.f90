...
SUBROUTINE GENLGMAT( GNAME, FDEV, NCOEF, CMAX, CMIN )
    ...
    !***********************************************************************
    !  FUNCTION:
    !      Compute gridding matrix going from Lat-Lon grid to a
    !      specified modeling grid, converting Kg/(M^2 S) to Kg/(cell S)
    !
    !      Also supports non-Standard WMO "lat-lon" that claims
    !      longitude-range is [0,360], contrary to ISO Standard 6709.
    !      Does NOT apply units-conversion (e.g., Kg/(M^2 S) ~> Ton/(cell year) )
    !
    !  PRECONDITIONS:
    !      I/O API 3.2 or later (for MODGCTP/GRID2XY())
    !      Initialized MODULE MODGRID
    !      setenv GRIDMASK     <path-name> for Lat-Lon emissions-input file
    !      setenv GRID_CRO_2D  <path-name> for model-grid GRIDCRO2 file
    !
    !  REVISION  HISTORY:
    ...
    ! smoke@unc.edu
    !
    !  Version "$Id: genlgmat.f 2016-02-05 15:53:52Z coats $"
    ...
    !.........   Get "raw-netCDF" input-grid description from "GRIDMASK" header:
    ...
    !.........   Open and check dims/proj/grid for GRID_CRO_2D file:
    ...
    ELSE
    !.............   Allocate local variables for output modeling domain
    !.............   Save modeling-domain description in MODGRID variables:
        NCOLS = NCOLS3D
        NROWS = NROWS3D
        NGRID = NCOLS * NROWS
        XORIG = XORIG3D
    ...
    ELSE
        ...
        IF ( GDTYPE .EQ. LATGRD3 ) THEN
            ...
            !  compute grid refinement ratio (at least 2x finer than input-grid)
            !  Need min of cos(lat) on the modeling grid in order to find the
            !  smallest meters "dx" for that grid
            !  This min will occur either on the top row or on the bottom row,
            !  depending upon (N or S) hemisphere coverage of modeling grid...
            !  Needs XCELLE,YCELLE converted to meters...
    ...
    !.........   Allocate  and compute Edgar-grid-to-source matrix:
    ...
    !.........   Compute fractions:
    !.........   First, find input-grid cell-centers for refined modeling grid:
    CALL GRID2XY( GDTYPE,  P_ALPE, P_BETE, P_GAME, XCENTE, YCENTE,  &
                  GDTYP,   P_ALP,  P_BET,  P_GAM,  XCENT,  YCENT,   &
                  NCOLS2, NROWS2, XORIG, YORIG, XCELL2, YCELL2,     &
                  XLOC, YLOC )

    !  Fix-up for WMO-style [0,360] input-grid longitudes
...
END SUBROUTINE GENLGMAT
    ...
    !---------------------------------------------------------------------
    !     Assemble contiguous matrix in memory and write it to GNAME:
    !     (needs to be a separate routine because of type-checking)
    !---------------------------------------------------------------------
SUBROUTINE  WRITELLG( GNAME, NGRID, NCOEF, NSRC, NX, IX, CX )
    ...
    !.........  Initialize header by using the DESC3(GRID_CRO_2D) results.
    NCOLS3D = NCOEF
    NROWS3D = NGRID
    NTHIK3D = NSRC
    NVARS3D = 1
