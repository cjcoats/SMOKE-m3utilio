...
SUBROUTINE OPENGMAT( NMATX, NMATXU, UFLAG, INVPROG, INVVERS,    &
    ...
    IF( .NOT. NFLAG ) THEN
        NAMBUF = PROMPTMFILE( 'Enter logical name for GRIDDING MATRIX output file',&
    ...
    IF( UFLAG ) THEN
        ...
        NAMBUF = PROMPTMFILE( 'Enter logical name for UNGRIDDING MATRIX output file',   &
    ...
    !.......  Open report file
    IF( CATEGORY .EQ. 'AREA'   .OR. CATEGORY .EQ. 'MOBILE' ) THEN

        MESG = 'Enter logical name for the GRIDDING SUPPLEMENTAL file'
        FDEV = PROMPTFFILE( MESG, .FALSE., .TRUE., CRL // 'GSUP', PROGNAME )
