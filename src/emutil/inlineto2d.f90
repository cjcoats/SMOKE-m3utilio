...
PROGRAM INLINETO2D
    ...
    CALL CHECKMEM( IOS, 'EIN...NEW_ROW', PROGNAME )
    ...
    IF ( LATLON_IN_FILE ) THEN
        ...
        DO S = 1, NSRC
            IF( .NOT. INGRID( XLOCA(S), YLOCA(S), NCOLS_N, NROWS_N, COL(S), ROW(S) ) ) THEN
    ...
    DO T = 1, TMAX
        ...
        DO V = 1, NVARSA
            IF( .NOT. READ3( 'INLN', VNAMEA(V), 1, JDATE, JTIME, EIN(1,V))) THEN
                MESG = 'Could not read "' // VNAMEA(V) // '" from file INLN "'
            ...
            IF( .NOT. WRITE3( ONAME, VNAMEA(V), JDATE, JTIME, EOUT(1,1,V))) THEN
                MESG = 'Could not write "' // VNAMEA(V) // '" to file OUTFILE "'
