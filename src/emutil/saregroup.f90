...
PROGRAM REGROUP
    ...
    !***********************************************************************
    !  program REGROUP body starts at line 118
    !
    !  DESCRIPTION:
    !       The purpose of this program is to merge multiple source-groups
    !       from program SMKMERGE, with optional renaming, and then output
    !       the set of new groups.
    !
    ...
    !
    !  REVISION  HISTORY:
    !       Initial version 10/29/2019 by Carlie J. Coats, Jr., UNC IE.
    ...
    '',                                                                     &
    'Program REGROUP to merge/rename multiple source-groups from program',  &
    'SMKMERGE, and then output the set of new groups.',                     &
    '',                                                                     &
    'PRECONDITIONS REQUIRED:',                                              &
    '   setenv  MRG_SOURCE      <A|P|B|M> (one letter only)',               &
    '   setenv  MRG_SECTOR      <sector',                                   &
    '',                                                                     &
    '   setenv  REGROUP_GROUPS  <ASCII re-group definitions, with sets',    &
    '   of lines formatted',                                                &
    '',                                                                     &
    ...
    !.......  Process REGROUP_GROUPS:  First read and parse header-line
    !.......  then read group-renaming data:
    ...
    DO L = 2, MXGID         !    !  process data lines
        ...
        IF ( GIDOUT( N ) .EQ. IMISS3 ) THEN
            WRITE( MESG, '( A, I9, :, 2X )' ) 'Bad GIDOUT in "REGROUP_GROUPS" at line', L
            CALL M3MESG( MESG )
            EFLAG = .TRUE.
        END IF

        IF ( GID_IN( N ) .EQ. IMISS3 ) THEN
            WRITE( MESG, '( A, I9 )' ) 'Bad GID_IN in "REGROUP_GROUPS" at line', L
            CALL M3MESG( MESG )
            EFLAG = .TRUE.
        END IF
...
199 CONTINUE
    ...
    DO L = 1, NRPTLINE
        ...
        IF ( ISTAT .NE. 0 ) THEN
            WRITE( MESG, '( A, I9)' ) 'ERROR=', ISTAT, 'reading "IN_REPORT" at line', L
            CALL M3MESG( MESG )
            EFLAG = .TRUE.
            CYCLE
        END IF
        ...
        SELECT CASE ( CATEGORY )
          CASE( 'A' )
            READ( LINE, '(I8,1X,A12,1X,A20,1X,I8)') M, CFIPIN(M), CSCCIN(M), IGROUP(M)
            CPLTIN( M ) = BLANK
            CPNTIN( M ) = BLANK
          CASE( 'B' )
            READ( LINE, '(A12,1X,I8)') CFIPIN(M), IGROUP(M)
            M = L
            CSCCIN( M ) = BLANK
            CPLTIN( M ) = BLANK
            CPNTIN( M ) = BLANK
...
399 CONTINUE
    ...
    !C.......  Check consistency for regrouping:
    !
    !        DO N = 1, NGRPS
    !            IGRP = GIDOUT( N )
    !            IF ( INDEXINT1( IGRP, NGRPS,   GID_IN ) .GT. 0 )  CYCLE
    !            IF ( INDEXINT1( IGRP, NSRCALL, IGROUP ) .GT. 0 )  THEN
    !                EFLAG = .TRUE.
    !                WRITE( MESG, '(A,1X,I8)' ) 'Invalid output group ID ', IGRP
    !                CALL M3MESG( MESG )
    !            END IF
    !        END DO
    !
    !        IF ( EFLAG ) THEN
    !            CALL M3EXIT( PROGNAME, 0,0, 'Invalid output group ID(s)', 2 )
    !        END IF


    !.......  Write the OUT_REPORT

    CALL WRRPT_GRPS()


    !.......  Open and check the IN_SRCS file
    ...
    !.......  allocate / calculate output-group arrays
    ...
    !.......  Compute the input-vector :: output-group mapping
    ...
    !.......  No group is not allowed for point sources.
    IF( CATEGORY == 'P' ) THEN
        L = NVECSSRC
        VECCNT = 1
        VECDEX = INDXIN
    ...
    !.......  Open the OUT_SRCS, borrowing much of description from IN_SRCS
    ...
    !.......  Open and check the IN_EMIS,OUT_EMIS files
