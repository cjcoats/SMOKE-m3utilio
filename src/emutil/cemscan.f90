...
PROGRAM CEMSCAN
    ...
    !***********************************************************************
    !  program body starts at line 39
    !
    !  DESCRIPTION:
    !      Program to read a year's worth of CEM data and calculate summed annual
    !      NOx emissions, SO2 emissions, heat input, gross load, and steam load.
    !      The output from CEMScan is used by Smkinven to calculate hourly emissions
    !      from annual inventory data when reading CEM data.
    !
    ...
    !.......  LOCAL PARAMETERS
    CHARACTER(16), PARAMETER :: PROGNAME = 'CEMSCAN'       ! program name
    ...
    !.......  Get program setting environment variables
    MESG = 'Maximum number of CEM units'
    MXUNITS = ENVGET( 'MAX_CEM_UNITS', MESG, 1, 999999999, 1, IOS )
    IF( IOS /= 0 ) THEN
        MESG = 'Environment variable MAX_CEM_UNITS must be set'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    !.......  Allocate memory for storing CEM data
    ALLOCATE( UNITLIST( MXUNITS ),&
               UNITIDX( MXUNITS ),&
               NOXEMIS( MXUNITS ),&
               SO2EMIS( MXUNITS ),&
             OPTIMESUM( MXUNITS ),&
             ...
             MINOPTIME( MXUNITS ),&
              ...
              GLDHOURS( MXUNITS ),&
              SLDHOURS( MXUNITS ),&
             HEATHOURS( MXUNITS ), STAT=IOS )
    CALL CHECKMEM( IOS, 'UNITLIST...HEATHOURS', PROGNAME )
    ...
    !.......  Determine maximum number of input files
    ...
    DO I = 1, MXFILES
        ...
        DO
            ...
            IF( IOS > 0 ) THEN
                EFLAG = .TRUE.
                WRITE( MESG,94010 ) 'ERROR: I/O error ', IOS, 'reading input file'
                CALL M3MESG( MESG )
                EXIT
            END IF
            ...
            !.......  Determine where current unit should go in sorted list of
            !                   units and check if it is already present
            IF( NUNITS == 0 ) THEN
                K = 1
            ELSE
            ...
            IF( K == -1 ) THEN
                ...
                CALL STORE_DATA( NOX, NOXEMIS( IDX ),&
                                 NOXHOURS( IDX ), MAXNOX( IDX ),   MINNOX( IDX ) )
                CALL STORE_DATA( SO2, SO2EMIS( IDX ),&
                                 SO2HOURS( IDX ), MAXSO2( IDX ),   MINSO2( IDX ) )
                CALL STORE_DATA( OPTIME, OPTIMESUM( IDX ),&
                                 OPHOURS( IDX ),  MAXOPTIME( IDX ),MINOPTIME( IDX ) )
                CALL STORE_DATA( GLOAD, GLOADSUM( IDX ),&
                                 GLDHOURS( IDX ), MAXGLOAD( IDX ), MINGLOAD( IDX ) )
                CALL STORE_DATA( SLOAD, SLOADSUM( IDX ),&
                                 SLDHOURS( IDX ), MAXSLOAD( IDX ), MINSLOAD( IDX ) )
                CALL STORE_DATA( HTINPUT, HEATINPUT( IDX ),&
                                 HEATHOURS( IDX ),MAXHEAT( IDX ),  MINHEAT( IDX ) )

                CYCLE
            END IF

            !.......  Make sure we have space to add unit
            IF( NUNITS + 1 > MXUNITS ) THEN
                MESG = 'MAX_CEM_UNITS value too small'
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ELSE

                !.......  Move existing entries down in unit list to make room
                DO L = NUNITS, K, -1
    ...
    DO I = 1, NUNITS
        ...
        WRITE( RDEV,93020 ) ORIS, BLRID, OUT, NUMHOURS( IDX ),&
          NOXHOURS( IDX ),   NOXEMIS( IDX ),    MAXNOX( IDX ),    MINNOX( IDX ),&
          SO2HOURS( IDX ),   SO2EMIS( IDX ),    MAXSO2( IDX ),    MINSO2( IDX ),&
          OPHOURS( IDX ),  OPTIMESUM( IDX ), MAXOPTIME( IDX ), MINOPTIME( IDX ),&
          GLDHOURS( IDX ),  GLOADSUM( IDX ),  MAXGLOAD( IDX ),  MINGLOAD( IDX ),&
          SLDHOURS( IDX ),  SLOADSUM( IDX ),  MAXSLOAD( IDX ),  MINSLOAD( IDX ),&
          HEATHOURS( IDX ),HEATINPUT( IDX ),   MAXHEAT( IDX ),   MINHEAT( IDX )
    END DO
