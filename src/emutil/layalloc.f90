...
PROGRAM LAYALLOC
    ...
    IF ( .NOT. DESC3( XNAME ) ) THEN
        MESG = 'Could not get description of file "' // TRIM( XNAME ) // '"'
    ...
    FDEV = PROMPTFFILE( MESG, .TRUE., .TRUE., 'LAYER_FRACTION', PROGNAME )
    ULAYS = GETFLINE( FDEV, 'User-defined Vertical Layer fraction file' )
    IF ( ULAYS .GT. MXLAYS3+1 ) THEN
        WRITE( MESG, '(A,I10)' ) 'Max layers exceeded in LAYER_FRACTION file:', ULAYS
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF
    ...
    DO L = 1, ULAYS
        ...
        IF ( IOS .GT. 0 ) THEN
            WRITE( MESG, 94020) 'I/O error', IOS, 'reading LAYERS file at line',IREC
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF
    ...
    CNAME = PROMPTMFILE( 'Enter name for I/O API 2-d emissions file',&
                         FSREAD3, 'INFILE', PROGNAME )

    IF ( .NOT. DESC3( CNAME ) ) THEN
        MESG = 'Could not get description of file "' // TRIM( CNAME ) // '"'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    ELSE IF ( .NOT.FILCHK3( CNAME, GRDDED3, NCOLS, NROWS, 1, NTHIK3D ) ) THEN
        MESG = 'Bad dimensions for file "' // TRIM( CNAME ) // '"'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF
    ...
    DO IHR = 1, NSTEPS
        ...
        IF( .NOT. READ3( XNAME, 'ZF', -1, JDATE, JTIME, ZZF ) ) THEN
        ...
        DO K = 1,NVARS3D
            ...
            IF( .NOT. READ3( CNAME, VNM, 1, JDATE, JTIME, TMPBUF ) ) THEN
                MESG = 'ERROR : Could not read ' //VNAME3D( K ) // ' from file ' // CNAME
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            END IF
...
222                         CONTINUE
                        ...
                        PFRAC = ( ( ZZF( C,R,LBOT ) - ZBOT ) / PDIFF ) * ZFRAC
                        LFRAC( LBOT ) = LFRAC( LBOT ) + PFRAC

                        !.......  Calculate a fraction for the top layer
                        PFRAC = ( ( ZTOP - ZZF( C,R,LTOP-1 ) ) / PDIFF ) * ZFRAC
                ...
                IF ( LTOT < 0.999 ) THEN
                    MESG = 'ERROR: Calculated layer fractions are '//   &
                        'less than 1.0. Emissions will be dropped.'
                    CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
                ELSE IF ( LTOT > 1.001 ) THEN
                    MESG = 'ERROR: Calculated layer fractions are '//&
                           'greater than 1.0. Emissions  will be increased.'
                    CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
                ENDIF
            ...
            IF ( .NOT. WRITE3( ONAME, VNM, JDATE, JTIME, TMP3D ) ) THEN
                WRITE( MESG, 93000 ) 'Could not write to "' // TRIM( ONAME ) // '".'
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            ENDIF
