...
PROGRAM SMK2EMIS
    ...
    !
    !  DESCRIPTION:
    !       Use SMOKE I/O API gridded input to produce UAM EMISSIONS input
    ...
    CHARACTER(4)       :: SPCNAM ( MXVARS3,10 )      ! output species names
    CHARACTER(NAMLEN3) :: OUTNAMS( MXVARS3 )     ! output spec names
    ...
    CHARACTER(16)  ENAME         !  I/O API logical input file name
    ...
    MAPFLAG = ENVYN( 'SMK2EMIS_VMAP_YN', 'Use names mapping file', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
    ...
    ENAME = PROMPTMFILE( 'Enter logical name for SMOKE gridded input (I/O API) file',    &
                         FSREAD3, CRL // 'GTS_L', PROGNAME )

    !.......  Prompt for variable mapping file, if needed
    IF( MAPFLAG ) THEN

        MDEV = PROMPTFFILE( 'Enter logical name for VARIABLE NAME MAP file',        &
    ...
    ODEV = PROMPTFFILE( 'Enter logical name for ' // HDRKEY // ' UAM output file ',&
    ...
    NZUPPR = ENVINT( 'UAM4_LAYABOVE', 'Layers above diffbreak', 0 , IOS )
    IF ( IOS .GT. 0 ) THEN
    ...
    !.......  Read I/O API header information:
    ...
    SDATE  = GETNUM( JDATE, 9999999, JDATE, 'Enter starting date (YYYYDDD)' )

    IF( SDATE .NE. JDATE ) JTIME = 0      ! Reset because now could be 0 -24

    !.......  Prompt for starting time of UAM output file

    STIME  = GETNUM( JTIME, 235959, JTIME, 'Enter starting time (HHMMSS)' )
    ...
    IF ( MAPFLAG ) THEN
        ...
        DO I = 1, NLINE
            ...
            IF( K .LE. 0 ) THEN
                ...
                MESG = 'WARNING: Ignoring "' // TRIM( SEGMENT( 1 ) ) //&
            ...
            ELSE
                ...
                MESG = 'NOTE: Renaming variable "' // TRIM( OUTNAMS( K ) ) //   &
                       '" as "' // TRIM( SEGMENT( 2 ) ) // '"'
                CALL M3MSG2( MESG )
    ...
    DO HR = 1, NSTEPS
        ...
        DO K = 1, NSPECS
            ...
            IF ( .NOT. READ3( ENAME, VNAME3D( K ), 1, SDATE, STIME, EMIS )) THEN
