...
PROGRAM SURGTOOL
    ...
    !
    !  DESCRIPTION:
    !      Inputs SMOKE-formatted gridding surrogates
    !      for a "fine" input grid and a grid definition for a "coarse" output
    !      grid.  It produces approximate "coarse" grid surrogates file, the
    !      accuracy of which depends on how fine the resolution of the input grid
    !      is relative to that of the output grid.
    !
    !  PRECONDITIONS REQUIRED:
    !      - Input  output surrogates on Lat-Lon, Lambert Conformal, or UTM map
    !        projections (can perform Lambert-to-Lambert and UTM zone-to-zone
    !        transformations).
    !      - Input grid resolution is much finer than output grid resolution
    ...
    !
    !  REVISION  HISTORY:
    !       Prototype  4/1998 by .Carlie J. Coats, Jr as program CROSSGRID
    !       Version    4/1999 by CJC to support both EPS2 and EMS85 files
    !       Version    10/00 by JMV:  source appropriated without permission as SURGTOOL
    !       Version    1/02 : Updated for Models-3 surrogate format and included
    !                     in SMOKE emutil module
    !       Version 11/2023 by CJC:  USE M3UTILIO, MODGCTP, conversion to ".f90",
    !       and related changes
    !***********************************************************************
    !
    ! Program copyright (C) 1998-2002 MCNC and (C) 2--3-2004 Baron Advanced
    ! Meteorological Systems, LLC, and released under Version 2 of the
    ...
    !***********************************************************************
    !   begin body of program SURGTOOL
    ...
    WRITE( *, '(5X, A)' )                                                   &
    ' ',                                                                    &
    'Program CROSSGRID to take the SMOKE 2 format surrogate coefficient',   &
    'file for a "fine" grid, a grid definition for that "coarse" grid,',    &
    'and produce the approximate "coarse" surrogate coefficient file. ',    &
    ' ',                                                                    &
    'Program copyright (C) 1998-2002 MCN, (C) 2003-2004 Baron Advanced',    &
    'Meteorological Systems, LLC, and  (C) 2023 Carlie J. Coats, Jr. ',     &
    ...
    WRITE( *, '(5X, A)' )                                                   &
    'USAGE:',                                                               &
    'You will need to enter GRIDDESC grid names for the input and output',  &
    'grids, and the logical names for the input and output files (and ',    &
    'to have set them prior to program launch, using ',                     &
    ' ',                                                                    &
    '    "setenv <logicalname> <pathname>").  ',                            &
    ' ',                                                                    &
    'Input files must have been sorted according to SMOKE conventions, ',   &
    'prior to program execution.',                                          &
    ' ',                                                                    &
    'You may use END_OF-FILE (control-D) to quit the program during',       &
    'logical-name entry. Default responses are indicated in brackets',      &
    '[LIKE THIS].',                                                         &
    ' ',                                                                    &
    'Program version $Id$', ' '

    !.......  Open input surrogates file
    SDEV = PROMPTFFILE( 'Enter logical name for input GRIDDING SURROGATE file',&
    ...
    !.......  Initialize gridding module
    CALL CHKGRID( GDNAM3D, 'SURROGATES', 1, EFLAG )

    !.......  Get grid parameters from grid description file for output grid.
    ...
    !......  Compute X-Y coordinates of fine grid relative to coarse grid
    !......  and then mapping C2 from fine cells to coarse cells:
    ...
    DO  C = 1, NCOLS
        ...
        !.......  If center of input cell (col,row) is outside output grid,
        !         then store missing intersection.
        IF ( XZ  .LT. 0.0d0   .OR.  YZ  .LT. 0.0d0  .OR.&
             COL .GT. NCOLS2  .OR.  ROW .GT. NROWS2 ) THEN
            C2( C,R ) = IMISS3

        !.......  Store which cell in output grid for current input cell (col,row)
        ELSE
    ...
    !.......  Open output file
    XDEV = PROMPTFFILE( 'Enter logical name for output GRIDDING SURROGATE file',&
    ...
    !.......  Allocate memory for output surrogates assuming that number of
    !           records from input file will > output records.
    ...
    !.......  Regrid the surrogates based on previously computed factors
    OUTNSRG = 0
    DO J = 1, NSRGREC
    ...
    !.......  Do not store output cells if input cell does not intersect
    !               any output cells
    ...
    !.......   Give error if couldn't store all of the output intersections
    ...
    !.......   Sort and process the surrogates:
    ...
    !.......  Loop through output entries and write them
    LCEL = IMISS3
    LFIP = IMISS3
    LSSC = IMISS3
    M    = 0
    ...
    !.......  If same FIPS code, cell, and surrogate, sum surrogate fraction
        IF ( F .EQ. LFIP  .AND.     &
             C .EQ. LCEL  .AND.     &
             S .EQ. LSSC ) THEN
    ...
    !.......  Otherwise...
        ELSE
            ...
            IF ( N .GT. 0 ) THEN
                ...
                WRITE( XDEV, 93020, IOSTAT=IOS ) LSSC, LFIP, COL, ROW, FRAC
    ...
    !.......  Write the last entry in the output file
    IF ( N .GT. 0 ) THEN
        ROW = 1 + INT( (LCEL-1) / NCOLS2 )
        COL = LCEL - NCOLS2 * ( ROW-1 )
        ...
        WRITE( XDEV, 93020, IOSTAT=IOS ) LSSC, F, COL, ROW, FRAC

    !.......  Write error message, if needed
        IF ( IOS .GT. 0 ) THEN
            EFLAG = .TRUE.
            WRITE( MESG, 94010 ) 'ERROR: I/O error', IOS, 'writing output file at line', M
        END IF
...
93980 FORMAT( A, 1X, 4( E15.8, 1X ), 3( I8, 1X ), 2( A, 1X ), 5( E15.8, 1X ) )
