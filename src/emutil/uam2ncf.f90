...
PROGRAM UAM2NCF
    ...
    !***********************************************************************
    !
    !  DESCRIPTION: Takes UAM EMISSIONS gridded files and creates
    !               I/O API gridded file which can be merge with other
    !               gridded SMOKE I/O API files.
    !
    !  PRECONDITIONS REQUIRED:
    !               Tested only for lat-lon projection
    !
    !  SUBROUTINES AND FUNCTIONS CALLED:
    !
    !  REVISION  HISTORY:
    !       Prototype 4/2000 :  by JMV
    !       Version  11/2023 by CJC:  USE M3UTILIO, conversion to ".f90",
    !       Numerically sound coordinate-matching code; species-count test;
    !       I/O status checking, and related changes
    !***********************************************************************
    !
    ...
    !.......  Input and output emissions

    REAL, ALLOCATABLE ::  EMOB( :, : )             ! UAM input emissions

    !.......   UAM header variables
    ...
    CHARACTER(16), ALLOCATABLE ::  KSPEC( : )      ! I/O API output species
    ...
    !.......   Get UAM emissions file name

    IDEV = PROMPTFFILE( 'Enter logical name for UAM EMISSIONS file',&
    ...
    !.......   Read UAM emissions file header

    READ(IDEV, IOSTAT=IOS) IFILE,NOTE,NSEG,NSPECS,IDATE,BEGTIM,JDATE,ENDTIM
    IF ( IOS .NE. 0 ) THEN
        WRITE( MESG, 94010 ) 'I/O error', IOS, 'reading UAM header at line', 1
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF
    ...
    IF ( IOS .NE. 0 ) THEN
        WRITE( MESG, 94010 ) 'I/O error', IOS, 'reading UAM header at line', 2
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    READ(IDEV, IOSTAT=IOS) IX,IY,NXCLL,NYCLL
    IF ( IOS .NE. 0 ) THEN
        WRITE( MESG, 94010 ) 'I/O error', IOS, 'reading UAM header at line', 3
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    CALL M3MESG( 'Checking grid parameters from header' )

    !.......   Checking grid name parameters versus UAM header info
    ...
    !.......  Allocate memory for emissions species variables
    ...
    !.......  Write UAM file description to FDESC3
    ...
    ENAME = PROMPTMFILE( 'Enter logical name for OUTPUT EMIS file ', &
                         FSUNKN3, 'E2DNCF',  PROGNAME )

    !.......   Write out emissions values
    ...
    DO  199  T = 1, NSTEPS
        ...
        IF ( IOS .NE. 0 ) THEN
            WRITE( MESG, 94010 ) 'I/O error', IOS, 'reading UAM data at line', IREC
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF
        IREC = IREC + 1
    ...
    !.......   If this is a new month, or new day, write message
        IF ( LDATE .NE. KDATE ) THEN

            MESG = 'Processing ' // DAYS( WKDAY( KDATE ) ) // MMDDYY( KDATE )
            CALL M3MSG2( MESG )
    ...
    !.......  Read UAM emissions
            ...
            READ ( IDEV ) ISEG, (MSPEC( M, V),M = 1,10), ((EMOB(I,J), I=1,NX),J=1,NY)
            IF ( IOS .NE. 0 ) THEN
                WRITE( MESG, 94010 ) 'I/O error', IOS, 'reading UAM data at line', IREC
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            END IF
            ...
            IF ( .NOT. WRITE3( ENAME, KSPEC( V ), JDATE, JTIME, EMOB ) ) THEN

                CALL M3EXIT( PROGNAME , JDATE, JTIME,&
                             'Could not write "' // KSPEC( V ) // '" to ' // ENAME, 2 )
...
CONTAINS        !    !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    LOGICAL FUNCTION COORDERR( P, Q )
        REAL*8, INTENT( IN ) :: P
        REAL  , INTENT( IN ) :: Q
        COORDERR = ( (P - Q)**2  .GT.  1.0D-10*( P*P + Q*Q + 1.0D-5 ) )
        RETURN
