...
SUBROUTINE WMRGELEV( VNAME, NSRC, NMAJOR, JDATE, JTIME )
    ...
    IF( FIRSTIME ) THEN
        ...
        !.....  Method of midpoint depends on vertical structure
            SELECT CASE ( VGTYP )
              CASE ( VGHVAL3 )
                ...
                DO L = 1, EMLAYS
                    LAYRMID( L ) = VGLVS( L-1 ) + 0.5 * ( VGLVS( L ) - VGLVS( L-1 ) )
                END DO

              CASE DEFAULT
                WRITE( MESG,94010 ) 'Do not know how to set ' //    &
                       'layer midpoints for layer structure type', VGTYP
        ...
        IF( OUTFMT .EQ. 'UAM' ) THEN
            ...
            CALL ENVSTR( 'UAM_VERTICAL_METHOD', MESG, 'STACKHGT', VTYPE, IOS )
            IF ( IOS .GT. 0 ) THEN
                CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "UAM_VERTICAL_METHOD"', 2 )
            END IF

            !.....  Get UAM file note
            NOTEDEF = 'UAM elev pt emis from ' // PROGNAME
            ...
            !.....  Insert grid name
            FNOTE  = TRIM( GRDNM ) // ' ' // ADJUSTL( UNOTE )
            ...
            NULAYS = ENVINT( 'UAM_LAYERS', MESG, EML , IOS )
            IF ( IOS .GT. 0 ) THEN
                CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "UAM_LAYERS"', 2 )
            END IF

            NZLOWR = ENVINT( 'UAM4_LAYBELOW', 'Layers below diffbreak', 3, IOS )
            IF ( IOS .GT. 0 ) THEN
                CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "UAM4_LAYBELOW"', 2 )
            END IF

            NZUPPR = NULAYS - NZLOWR
            NZUPPR = ENVINT( 'UAM4_LAYABOVE', 'Layers above diffbreak', NZUPPR, IOS )
            IF ( IOS .GT. 0 ) THEN
        ...
        !.....  Control packet settings
            ...
            IF( VTYPE .EQ. 'PLUMERISE' ) THEN
                ...
                TDEVOUT= ENVINT( 'UAM4_TEMPERATUR_UNIT', MESG, 14, IOS )
                IF ( IOS .GT. 0 ) THEN
                    CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "UAM4_TEMPERATUR_UNIT"', 2 )
                END IF

                MESG = 'METSCALARS file unit number'
                MDEVOUT= ENVINT( 'UAM4_METSCALARS_UNIT', MESG, 15, IOS )
                IF ( IOS .GT. 0 ) THEN
            ...
            WRITE( EVDEV,93015 ) USWITCH, LSWITCH, 0, MSWITCH, VSWITCH
            WRITE( EVDEV,93015 ) 1, 0, ESWITCH
            WRITE( EVDEV,93015 ) DDEVOUT, RDEVOUT, 0, TDEVOUT, MDEVOUT, WDEVOUT
            ...
            DO I = 1, NMSPC
                ...
                IF( SPCNAM .NE. EMNAM( I ) ) THEN
                    MESG = 'WARNING: Species name "' // TRIM( EMNAM( I ) )//    &
    ...
    IF( JTIME .NE. PTIME .AND. OUTFMT .EQ. 'UAM' ) THEN
        ...
        WRITE( EVDEV, 93000 ) 'STD       ALL       EMVALUES  0.        50000.'
        WRITE( EVDEV, 93000 ) 'END'
        ...
        WRITE( EVDEV, 93000 ) 'STD       ALL       ' // VTYPE // ' 0.       10000.'
        WRITE( EVDEV, 93000 ) 'END'
...
CONTAINS
    ...
    !.....  This internal subprogram tries to read a variable from an
    !       I/O API file, and aborts if not successful.
    SUBROUTINE SAFE_READ3_I( FILNAM, VARNAM, LAYER, JDATE, JTIME, IBUF     )
        ...
        IF ( .NOT. READ3( FILNAM, VARNAM, LAYER,JDATE,JTIME, IBUF ) ) THEN
    ...
    SUBROUTINE GET_ESUM_FORMAT( VBUF, VAL, FMT )
        ...
        IF( VAL .GT. 999999999. ) THEN
            ...
            WRITE( MESG,94020 )                                             &
              'WARNING: "' // TRIM( VBUF ) // '"Emissions value of', ESUM,  &
              CRLF()// BLANK10//  '" is too large for file format, so ' //  &
              'writing in scientific notation for source:',    &
