...
CONTAINS
    ...
    SUBROUTINE CREATE_HEADER( CATNAME )
        ...
        DATANAM = ' average'
        IF( LAVEDAY .AND. ( AFLAG .OR. PFLAG ) )  DATANAM = ' average day'
        ...
        IF( JDATE .NE. 0 ) THEN
            ...
            WRITE( HEADER,94010 ) TRIM( HEADER ) // ' from ' //         &
               CRLF() // '#' // BLANK5(1:4) // TRIM( DAYS( K1 ) ) //    &
               ' ' // MMDDYY( PDATE ) // ' at', PTIME, 'to' //          &
               CRLF() // '#' // BLANK5(1:4) // TRIM( DAYS( K2 ) ) //    &
               ' '// MMDDYY( JDATE ) // ' at', JTIME
...
94010   FORMAT( 10( A, :, I8, :, 1X ) )
    ...
    SUBROUTINE CREATE_FORMATS( NDIM, MAXCOL1, INNAMS, INUNIT,    &
        ...
        DO J = 1, NDIM
            ...
            I1 = WIDTHS( J ) - L1
            IF( I1 .GT. 0 ) THEN
                WRITE( OUTNAMS( J ), '(A,A)' ) SPACE( 1:I1 ), TRIM( INNAMS( J ) )
            ELSE
            ...
            I2 = WIDTHS( J ) - L2
            IF( I2 .GT. 0 ) THEN
                WRITE( OUTUNIT( J ), '(A,A)' ) SPACE( 1:I2 ), TRIM( INUNIT( J ) )
            ELSE
    ...
    SUBROUTINE WRITE_STA( FDEV, NS, NDIM, VNAMES, INUNIT, ST_EMIS )
        ...
        !.....  Build output format depending on data values
            CALL DYNAMIC_FORMATS( NSTATE, NDIM, I, ST_EMIS, MAXWID(1), DATFMT )

        !.....  Write out state name and converted emissions
            WRITE( FDEV, DATFMT ) STATNAM( I ), ( ST_EMIS( I,J ), J=1, NDIM )
        END DO
    ...
    SUBROUTINE WRITE_CNY( FDEV, NC, NDIM, VNAMES, INUNIT, CY_EMIS )
        ...
        DO I = 1, NC
            ...
            WRITE( FDEV,DATFMT ) CDATFIP // ' '// STATNAM(N) //    &
                                 CNTYNAM(I), ( CY_EMIS( I,J ), J=1, NDIM )
