...
SUBROUTINE GETMRGEV
    ...
    !       Bug-fix in ENVINT calls at lines 253,260 and related changes
    !***********************************************************************
    !
    ...
    IF( MRGSRC .EQ. ' ' ) THEN
        MESG = 'No source categories selected! You must define the MRG_SOURCE ' //     &
               CRLF() // BLANK10 // 'environment variable using the letters A, B,  M, and P'
    ...
    LMETCHK = ENVYN( 'MRG_METCHK_YN',       &
                     'Check consistency of met headers or not', .TRUE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "MRG_METCHK_YN"', 2 )
    END IF

    LMKTPON = ENVYN( 'MRG_MARKETPEN_YN',    &
                     'Use market penetration from reactivity matrices', .TRUE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "MRG_MARKETPEN_YN"', 2 )
    END IF

    LGRDOUT = ENVYN( 'MRG_GRDOUT_YN',       &
                     'Output gridded I/O API  files or not', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "MRG_GRDOUT_YN"', 2 )
    END IF

    LREPSTA = ENVYN( 'MRG_REPSTA_YN',       &
                     'Output state total ASCII reports or not', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "MRG_REPSTA_YN"', 2 )
    END IF

    LREPCNY = ENVYN( 'MRG_REPCNY_YN',       &
                     'Output county total ASCII reports or not', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "MRG_REPCNY_YN"', 2 )
    END IF

    LREPINV = ENVYN( 'MRG_REPINV_YN',       &
                     'Report inventory emissions or not', .TRUE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "MRG_REPINV_YN"', 2 )
    END IF
    INVIOS = IOS

    LREPSPC = ENVYN( 'MRG_REPSPC_YN',       &
                     'Report speciated emissions or not', .TRUE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "MRG_REPSPC_YN"', 2 )
    END IF
    SPCIOS = IOS

    LREPCTL = ENVYN( 'MRG_REPCTL_YN',       &
                     'Report controlled emissions', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
    ...
    VARFLAG = ENVYN( 'USE_VARIABLE_GRID',   &
    ...
    SRCGRPFLAG = ENVYN( 'SMK_SRCGROUP_OUTPUT_YN',   &
                        'Use source grouping', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PROGNAME,0,0, 'Bad env vble "SMK_SRCGROUP_OUTPUT_YN"', 2 )
    END IF

    IF( SRCGRPFLAG .AND. XFLAG ) THEN
        MESG = 'Source grouping cannot be used with multiple source categories.'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    !.....  Check if source sub-grouping should be used
    SUBSECFLAG = ENVYN( 'SMK_SUB_SECTOR_OUTPUT_YN',     &
                        'Use sub-sector source grouping', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
    ...
    IF ( PFLAG ) THEN
        ...
        IF( EXPLFLAG .AND. .NOT. ELEVFLAG ) THEN
            ELEVFLAG = .TRUE.
            MESG = 'NOTE: ASCII elevated output switched on to be consitent with ' //    &
                   CRLF() // BLANK10// 'EXPLICIT_PLUMES_YN = Y setting.'
            CALL M3MSG2( MESG )
        END IF
        ...
        IF ( SRCGRPFLAG ) THEN
            IF ( LFLAG ) THEN
                MESG = 'Source grouping cannot be used with layer fractions.'
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            END IF

            IF ( .NOT. INLINEFLAG ) THEN
                MESG = 'Source grouping must be used with in-line CMAQ outputs.'
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
            END IF
        END IF
