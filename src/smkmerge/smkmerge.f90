...
PROGRAM SMKMERGE
    ...
    !***********************************************************************
    !  program SMKMERGE body starts at line 148
    !
    !  DESCRIPTION:
    !      The purpose of this program is to merge the inventory or hourly
    !      emissions files from the Temporal program with gridding matrices and
    !      with optionally any combination of speciation matrices and 3 control
    !      matrices (different types).  The program can operate on from 1 to 4
    !      source categories (area, biogenic, mobile, or point sources), or any
    !      combination of these.  If a layer fractions file is input, then the
    !      output file is 3-d.  This program is not used for the MPS/MEPSE files
    !      for CMAQ.
    !
    ...
    USE MODELEV, ONLY: INDXH, NHRSRC, GRPGID, ELEVFLTR, ELEVSRC, GROUPID
    ...
    !.....   Local temporary array for input and output variable names
    CHARACTER(NAMLEN3), ALLOCATABLE :: VARNAMES( : )
    CHARACTER(NAMLEN3), ALLOCATABLE :: INNAMES ( : )

    !.....   Local allocatable arrays for creating list of all explicit srcs
    ...
    !.....    Local allocatable array for tracking whether species have
    !       already been processed for elevated sources.
    LOGICAL, ALLOCATABLE :: ELEV_SPCSET( : )

    !.....    Local variables for array sizes
    ...
    CHARACTER(NAMLEN3) LBUF    ! previous species or pollutant name
    CHARACTER(NAMLEN3) PBUF    ! tmp pollutant or emission type name
    CHARACTER(NAMLEN3) SBUF    ! tmp species or pollutant name
    CHARACTER(PLSLEN3) VBUF    ! pol to species or pol description buffer

    !***********************************************************************
    !   begin body of program SMKMERGE
    ...
    !.....  Open input files and retrieve episode information
    CALL OPENMRGIN( SRGNROWS, SRGNCOLS, SRGGRDNM, SRGFMT )

    !.....  Do setup for biogenic state and county reporting or source
    !       apportionment
    IF( BFLAG .AND. ( LREPANY .OR. SRCGRPFLAG .OR. SUBSECFLAG ) ) THEN

        !.....  Read gridding surrogates
        ...
        IF ( OFFLAG ) THEN
            MESG = 'NOTE: gridding surrogates (for biogenic '//    &
                   'totals) extracted for output'//    &
                   CRLF()// BLANK10 //'grid from grid "' // TRIM( SRGGRDNM ) // '"'
            CALL M3MSG2( MESG )
        END IF

    END IF

    !.....  Create arrays of sorted unique pol-to-species
    !.....  Create arrays of sorted unique pollutants
    !.....  Create arrays of sorted unique species
    CALL MRGVNAMS

    !.....  Determine units conversion factors
    CALL MRGUNITS

    !.....  Read in any needed source characteristics
    CALL RDMRGINV

    !.....  Do setup for state and county reporting
    !.....  Do this even if there LREPANY is false, in order to allocate
    !       memory for the state and county total arrays to ensure
    !       MRGMULT will work.

    !.....  Read the state and county names file and store for the
    !       states and counties in the grid
    !.....  For anthropogenic source categories, use FIPS list
    !       from the inventory for limiting state/county list
    ...
    !.....  Otherwise, for biogenic merge only, use list of codes from the
    !       surrogates file needed for state and county totals
    ELSE
        IF( USEEXPGEO() ) THEN
    ...
    !.....  Allocate memory for fixed-size arrays by source category...

    CALL ALLOCMRG( MXGRP, MXVARPGP, AMULSIZ, MMULSIZ, PMULSIZ,    &
                   ASPCSIZ, MSPCSIZ, PSPCSIZ, APOLSIZ, MPOLSIZ,   &
                   PPOLSIZ )

    !.....  Read in elevated sources and plume-in-grid information, if needed
    !.....  Reset flag for PinG if none in the input file
    IF( PFLAG .AND. (ELEVFLAG .OR. PINGFLAG .OR. INLINEFLAG) ) THEN
        ...
        IF( ELEVFLAG .AND. NMAJOR .EQ. 0 ) THEN
            MESG = 'WARNING: No sources are major elevated sources in input file, ' //    &
                   CRLF() // BLANK10 // 'so elevated source emissions ' //    &
        ...
        IF( PINGFLAG .AND. NPING .EQ. 0 ) THEN
            MESG = 'WARNING: No sources are PinG sources in input file, so ' //    &
                   CRLF() // BLANK10 // 'PinG emissions file will not be written.'
            CALL M3MSG2( MESG )
            PINGFLAG = .FALSE.
        END IF

        !.....  Read stack group IDs
        IF ( .NOT. READ3( PVNAME, 'ISTACK', 1,PVSDATE,PVSTIME, GRPGID ) ) THEN

            MESG = 'Could not read "ISTACK" from file "' // TRIM( PVNAME ) // '"'
            CALL M3EXIT( PROGNAME, SDATE, 0, MESG, 2 )

        END IF

        !.....  Update elevated sources filter for elevated sources
        DO S = 1, NPSRC
    ...
    !.....  Create complete source list for explicit elevated sources
    IF( EXPLFLAG ) THEN

        !.....  Allocate memory for temporary unsorted list and index
        ALLOCATE( SRCFLG( NPSRC ),     &
                  TMPSRC( NHRSRC ),    &
                  TMPIDX( NHRSRC ), STAT=IOS )
        CALL CHECKMEM( IOS, 'TMPIDX', PROGNAME )
        SRCFLG = .FALSE.

        !.....  Loop through hours in PLAY_EX file and determine all sources
        !       that are listed for all hours
        JDATE  = SDATE
        JTIME  = STIME
        N      = 0
        DO T = 1, NSTEPS
            IF( .NOT. READ3( PHNAME, 'INDXH', ALLAYS3,JDATE,JTIME, INDXH ) ) THEN
            ...
            !.....  Loop through this hour's indices, and store any new ones
            DO I = 1, NHRSRC

                S = INDXH( I )

                !.....  Exit loop if done sources for this hour
                IF ( S .EQ. 0 ) EXIT
    ...
    !.....  Read gridding matrices (note, must do through subroutine because of
    !       needing contiguous allocation for integer and reals)
    ...
    !.....  Open NetCDF output files, open ASCII report files, and write headers

    CALL OPENMRGOUT( NGRP )

    !.....  In case reactivity does not exist, initialize temporary arrays
    !       for reactivity information anyway.  These are used even without
    !       reactivity matrix inputs so that the code does not need even
    !       more conditionals in the matrix multiplication step.
    IF( AFLAG ) ARINFO = 0.      ! array
    IF( MFLAG ) MRINFO = 0.      ! array
    IF( PFLAG ) PRINFO = 0.      ! array

    !.....  Allocate memory for temporary list of species and pollutant names
    !.....   Allocate memory for logical array for whether species set for elevated output
    ALLOCATE( VARNAMES( MXVARPGP ),    &
               INNAMES( MXVARPGP ),    &
           ELEV_SPCSET( NSMATV ), STAT=IOS )
    CALL CHECKMEM( IOS, 'ELEV_SPCSET', PROGNAME )
    ELEV_SPCSET = .FALSE.      ! array

    !....  Allocate memory for tmp control/projection array
    IF( PFACFLAG ) THEN
    ...
    !.....  Loop through processing groups (if speciation, this will be specia-
    !       tion groups, but if no speciation, this will be pollutant groups,
    !       for purposes of memory usage if many pollutants and/or species)
    PGID = IMISS3
    DO N = 1, NGRP

        !.....  Set the number of variables per group
        NVPGP = VGRPCNT( N )

        !.....  If pollutants in current group are different from those
        !       in the previous group, read pollutant-specific control matrices
        !.....  For reactivity matrices, read inventory emissions that will
        !       be needed for getting ratios of inventory to hourly for applying
        !       reactivity-based projection to hourly emissions
        !.....  Note that only the pollutants in this group that are actually
        !       in the control matrices are stored, and the index that says
        !       which are valid is *U_EXIST and *A_EXIST
        IF( IDVGP( N ) .NE. PGID ) THEN

            IF( AUFLAG ) THEN
                IF( PFACFLAG ) THEN
                    CALL RD3MASK( AUNAME,0,0, NASRC, 1,1, 'pfac', 1, TCUMATX )
                    DO NG = 1, NVPGP
                        J = AU_EXIST( NG,1 )
                        ACUMATX( :,J ) = TCUMATX( : )
                    END DO
            ...
            IF( MUFLAG ) THEN
                IF( PFACFLAG ) THEN
                    CALL RD3MASK( MUNAME,0,0, NMSRC, 1,1,'pfac', 1, TCUMATX )
                    DO NG = 1, NVPGP
                        J = MU_EXIST( NG,1 )
                        MCUMATX( :,J ) = TCUMATX( : )
                    END DO
            ...
            IF( PUFLAG )  THEN
                IF( PFACFLAG ) THEN
                    CALL RD3MASK( PUNAME,0,0, NPSRC, 1,1, 'pfac', 1 , TCUMATX )
                    DO NG = 1, NVPGP
                        J = PU_EXIST( NG,1 )
                        PCUMATX( :,J ) = TCUMATX( : )
                    END DO
        ...
        !.....  Loop through variables in current group...
        OCNT = 0
        LBUF = ' '
        INNAMES  = ' '      ! array
        VARNAMES = ' '      ! array
        DO V = 1, NVPGP      ! No. variables per group
            ...
            !.....  Extract name of variable in group
            VBUF = GVNAMES( V,N )

            !.....  For speciation...
            IF( SFLAG ) THEN

            !.....  Update list of output species names for message
            ...
            !.....  Set position for input of speciation matrix
                IF( AFLAG ) K1 = AS_EXIST( V,N )
                IF( MFLAG ) K2 = MS_EXIST( V,N )
                IF( PFLAG ) K3 = PS_EXIST( V,N )

            !.....  Read speciation matrix for current variable and
            !       position
                IF ( K1 .GT. 0 )    &
            ...
            !.....  For no speciation, prepare list of variables for output mesg
            ELSE

            !.....  Update list of pollutants names for message
                PBUF = EANAM( SIINDEX( V,N ) )
        ...
        END DO              ! End variables in group loop
            ...
            !.....  Write out message about data currently being processed
        ...
        DO T = 1, NSTEPS               ! at least once for time-independent
            ...
            !.....  Reinitialize array for flagging elevated species as "set"
            ELEV_SPCSET = .FALSE.              ! array

            !..... For time-dependent processing, write out a few messages...
            IF( TFLAG ) THEN

            !.....  Determine weekday index (Monday is 1)
                DAY = WKDAY( JDATE )

            !.....  Write out message for new day.  Note, For time-
            !       independent, LDATE and JDATE will both be zero.
            ...
            !.....  Initialize source-category current dates
            AJDATE = JDATE
            MJDATE = JDATE
            PJDATE = JDATE

            !.....  Reset the date for each source category when by-day
            !       processing is being done for that category
            IF( AFLAG_BD ) AJDATE = ASDATE( DAY )
            IF( MFLAG_BD ) MJDATE = MSDATE( DAY )
            IF( PFLAG_BD ) PJDATE = PSDATE( DAY )

            !.....  If area sources, read inventory emissions for this time
            !       step for all area-source pollutants in current pol group
            !.....  The *_EXIST are counters that point to the position in
            !       the source category emissions of the variables names
            !       in INNAMES. Data are stored in *EMSRC in the global order.
            IF( AFLAG ) THEN

            !.....  If using map-formatted inventory for time-independent
            ...
            !.....  If mobile sources, read inventory emissions or activities
            !       for this time step for all mobile-source pollutants in
            !       current pol group
            IF( MFLAG ) THEN
            ...
            !.....  If point sources, read inventory emissions for this time
            !       step for all point-source pollutants in current pol group
            IF( PFLAG ) THEN
            ...
            !.....  If layer fractions, read them for this time step
            IF( LFLAG ) THEN
            ...
            !.....  Otherwise, if explicit plume rise, read fractions and
            ...
            !.....  Loop through variables in the current group
            LBUF = ' '
            DO V = 1, NVPGP

                !.....  Set species or pollutant/activity name for this
                !       iteration
                IF( SFLAG ) THEN
                    SBUF = EMNAM( SPINDEX( V,N ) )
                ...
                !.....  If area reactivity matrix applies, pre-compute
                !       source array of reactivity emissions  mkt pentrtn
                IF( ARFLAG ) THEN
                    K1 = A_EXIST ( V,N )
                    K2 = AR_EXIST( V,N )
                    IF( K2 .GT. 0 ) THEN
                ...
                !.....  For biogenic sources, read gridded emissions,
                !       add to totals and store
                IF( BFLAG ) THEN
                    ...
                    IF( K4 .GT. 0 ) THEN
                        ...
                        !.....  Update country, state,  county totals
                        !.....  Also convert the units from the gridded output
                        !       units to the totals output units
                ...
                !.....  If mobile reactivity matrix applies, pre-compute
                !       source array of reacvty emissions and mkt pntrtn
                IF( MRFLAG ) THEN
                    K1 = M_EXIST ( V,N )
                    K2 = MR_EXIST( V,N )
                    IF( K2 .GT. 0 ) THEN
                ...
                !.....  If reactivity matrix applies, pre-compute source
                !       array of reactivity emissions and market penetration
                IF( PRFLAG ) THEN
                    K1 = P_EXIST ( V,N )
                    K2 = PR_EXIST( V,N )
                    IF( K2 .GT. 0 ) THEN
                ...
                IF( PFLAG ) THEN
                    ...
                    !.....  Apply matrices for elevated and plume-in-grid
                    !       outputs, if this pollutant is used for point srcs.
                    IF( K1 .GT. 0 .AND.    &
                     (ELEVFLAG .OR. PINGFLAG .OR. INLINEFLAG) ) THEN

                        !.....  Determine whether or not this species has been
                        !       merged before to set initialization flag. This
                        !       additional step is to handle the case where multiple
                        !       pollutants feed the same species.
                        INITELEV = .TRUE.
                        IF ( SFLAG ) THEN
                            IF( ELEV_SPCSET( SPINDEX( V,N ) ) )  INITELEV = .FALSE.
                            ELEV_SPCSET( SPINDEX( V,N ) ) = .TRUE.
                        END IF

                        CALL MRGELEV( NPSRC, NMAJOR, NPING, K1, K2, K4, F1, INITELEV )

                    END IF

                END IF

                !.....  Check the flag that indicates the entries for which
                !       we need to output the gridded data
                IF( GVLOUT( V,N ) ) THEN

                    !.....  Write out gridded data and Models-3 PinG file
                    CALL WMRGEMIS( SBUF, JDATE, JTIME )

                    !.....  Write out ASCII elevated sources file
                    IF( ELEVFLAG ) THEN
                        CALL WMRGELEV( SBUF, NPSRC, NMAJOR,JDATE, JTIME )
                    END IF
            ...
            !.....  Write country, state, and county emissions (all that apply)
