PROGRAM MRGPT
    ...
    !
    !  DESCRIPTION:
    !    Program MRGPT reads STACK_GROUPS and INLN I/O API files and merges them
    !    into a single STACK_GROUP/INLN file pair(depending on the inputs)
    !
    ...
    !
    !  REVISION  HISTORY:
    !       Original by G. Pouliot 11/30/2007
    !       Revised by M. Omary 08/05/2010
    !       Revised by H. Tran 09/25/2018
    ...
    !        System
    ! File: @(#)$Id: mrggrid.f,v 1.19 2007/07/11 19:30:43 bbaek Exp $
    !
    ...
    ! smoke@unc.edu
    !
    ! Pathname: $Source: /afs/isis/depts/cep/emc/apps/archive/smoke/smoke/src/smkmerge/mrggrid.f,v $
    ! Last updated: $Date: 2007/07/11 19:30:43 $
    !
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'MRGPT'      ! program name
    ...
    IDEV = PROMPTFFILE( MESG, .TRUE., .TRUE., 'FILELIST_S', PROGNAME   )

    MESG = 'Enter logical name for INLINEPT INPUTS list'
    JDEV = PROMPTFFILE( MESG, .TRUE., .TRUE., 'FILELIST_E', PROGNAME   )
    ...
    ALLOCATE( DURATA( MXNFIL ,NLIST),    &
              ...
              STIMEA( MXNFIL ,NLIST),    &
               FNAME( MXNFIL, NLIST),    &
              LVOUTA( MXVARS3,MXNFIL,NLIST ),    &
              VNAMEA( MXVARS3,MXNFIL,NLIST ),    &
              VUNITA( MXVARS3,MXNFIL,NLIST ),    &
              VDESCA( MXVARS3,MXNFIL,NLIST ),    &
              VTYPEA( MXVARS3,MXNFIL,NLIST ),    &
                NSRC( NLIST,MXNFIL+1), STAT=IOS )
    CALL CHECKMEM( IOS, 'DURATA...NSRC', PROGNAME )

    !.....  Loop through input files (1) and open them
    F = 0
    IREC = 0
    DO
...
27  CONTINUE
    ...
    !.....  Loop through input files (2) and open them
    F = 0
    IREC = 0
    DO
    ...
    !.....  Read file names - exit if read is at end of file
        ...
        IF( F .LE. MXNFIL ) THEN
            ...
            IF ( .NOT. OPEN3( FNAME(F,2), FSREAD3, PROGNAME )) THEN
                ...
                MESG = 'Could not open file "' // TRIM( FNAME(F,2) )// '".'
                CALL M3MSG2( MESG )
                MESG = 'Ending program "MRGGRID".'
                CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
...
28  CONTINUE
    ...
    ALLOCATE( NSTATES( NFILE ),    &
             USEFIRST( NFILE ),    &
            STATEFIPS( MAXFIP, NFILE ),    &
            STATEFIP2( MAXFIP, NFILE ),  STAT = IOS )
    CALL CHECKMEM( IOS, 'USEFIRST', PROGNAME )

    !#########################################################################
    !.....  Get file descriptions and store for all input files
    !.....  Loop through 2 sets of input files
    NLAYS = 1
    DO S = 1,NLIST     !2
    DO F = 1, NFILE
    ...
    !.....  Check if the # of recrords (NROWS) are tha same in the Stack group file and the Emmissions file
    ...
    DO F = 1, NFILE
        IF( NROWSA( F ,1) .NE. NROWSA( F ,2) ) THEN
            ...
            MESG = 'Inconsistent number of sources between:' //CRLF() //    &
                    ...
                    'and Emissions file '//TRIM(FNAME( F,2 ))//   &
                    CRLF() // BLANK10 //'The order of the input files in the FILELISTE_S should be'//    &
    ...
    DO S = 1,NLIST       ! THIS loop is to read in the input files only
        ...
        DO F = 1, NFILE
            ...
            DO V = 1, NVARSA( F,S )
                ...
                ELSE IF (VNM(2:5) .ne. 'LOCA') THEN
                    ! don't check units on XLOCA and YLOCA

                    IF ( VUNITA( V,F,S ) .NE. VUNITU( K,S ) ) THEN
                        EFLAG = .TRUE.
                        WRITE( MESG,94010 ) 'ERROR: Variable "' //  TRIM( VNM ) //  &
                               '" in file', F, 'has units "'//TRIM( VUNITA( V,F,S ) )//'"'//  &
                               CRLF() // BLANK10 //  'that are inconsistent with a '//    &
                               'previous file that had units "' //    &
                               TRIM( VUNITU( K,S ) )//'" for this variable'
                        CALL M3MSG2( MESG )
        ...
        IF (S .EQ. 1) THEN
            ...
            ALLOCATE( STACK_PARAM_I_IN( NSRC(S,NFILE+1), NVOUT_I(S), NFILE ),                 &
                      STACK_PARAM_R_IN( NVOUT_R(S), NSRC(S,NFILE+1), NFILE ),                 &
                     STACK_PARAM_I_OUT( NSRC(S,NFILE+1), NVOUT_I(S) ),                 &
                     STACK_PARAM_R_OUT( NSRC(S,NFILE+1), NVOUT_R(S) ), STAT=IOS)
            CALL CHECKMEM( IOS, 'STACK_PARAM_R_OUT', PROGNAME )

        ELSE IF ( S .EQ. 2) THEN

            ALLOCATE (EIN( NSRC(S, NFILE+1), NVOUT(S),NFILE ),   &
                     EOUT( NSRC(S, NFILE+1), NVOUT(S) ),         &
                    EOUT1( NSTEPS, NSRC(S,NFILE+1), NVOUT(S) ),  &
                EMIS_TOTALS_BA( MAXSTATES, NVOUT(S), NFILE ),    &
                EMIS_TOTALS_AA( MAXSTATES, NVOUT(S), NFILE ), STAT=IOS)
                ...
                DO F = 1, NFILE
                    ...
                    IF( LVOUTA( V,F,S ) ) THEN                       !If Pollutant is in the file
                        ...
                        IF (VTYPE .EQ.M3REAL) THEN
                            ...
                            IF(.NOT. READ3( NAM, VNM, ALLAYS3 , RDATE, JTIME, VAR_R) )THEN
                                MESG = 'Could not read "' // VNM // '" from file "' // TRIM( NAM )// '".'
                            ...
                            IF ( (S .EQ. 2)) THEN
                                ...
                                IF(ADEV .GT. 0) THEN
                                    ...
                                    !.....  Warning missing emissions logical file names from the ADJ_FACS list
                                    LNAM = ' '
                                    DO I = 1, NADJ
                                        NAM1 = ADJ_LFN( I )
                                        ...
                                        IF( L <= 0 ) THEN
                                            MESG = 'WARNING: The logical file '//TRIM(NAM1) //    &
                                                ...
                                                'found in the FILELIST_E on DATE : '// MMDDYY(SDATE)
                                            IF( LNAM /= NAM1 ) THEN
                                                CALL M3MSG2( MESG )
                                                LNAM = NAM1
                                            END IF
                                        END IF
                                    END DO
                                    !.....  Search adjustment factor for the current emissions file
                                ...
                                DO J = 1,NSRC(S,F)
                                    ...
                                    !.....  Look for state fips in output list
                                    ...
                                    EMIS_TOTALS_BA(IFIP1,V,F) = EMIS_TOTALS_BA(IFIP1,V,F) + VAR_R(1,J,1)
                                END DO
                                ...
                                DO J = 1,NSRC(S,F)
                                    IFIP1 =STACK_PARAM_I_IN(J,IDX_FIP,F)/1000
                                    EMIS_TOTALS_AA(IFIP1,V,F) = EMIS_TOTALS_AA(IFIP1,V,F) + VAR_R(1,J,1)*FACS
                                END DO

                            END IF      ! S = 2

                        ELSEIF (VTYPE .EQ. M3INT) THEN
                            IF(.NOT. READ3( NAM, VNM, ALLAYS3 , RDATE,JTIME, VAR_I) )THEN
                                MESG = 'Could not read.... "' // TRIM( VNM )//    &
                                       '" from file "' // TRIM( NAM ) // '".'
                                CALL M3EXIT( PROGNAME, RDATE, JTIME, MESG, 2 )
                            END IF
                            ...
                            IF (S .EQ. 1) THEN
                                ...
                                IF (VNM .NE. 'ISTACK') THEN
                                    ...
                                    STACK_PARAM_I_IN(1:NSRC(S,F),NVAR_INT ,F) = VAR_I(1,1:NSRC(S,F),1)
        ...
        DO T = 1, TMAX
            ...
            DO V = 1, NVOUT(S)
                ...
                IF (S .EQ. 2 ) THEN
                    ...
                    IF( .NOT. WRITE3( ONAME_E, VNM, JDATE, JTIME, EOUT(1,V) ) ) THEN
                ...
                IF ((S .EQ. 1) .AND. (VTYPE3D(V) .EQ. M3REAL)) THEN
                    ...
                    IF( .NOT. WRITE3( ONAME_S, VNM, JDATE,JTIME, STACK_PARAM_R_OUT(1,NVAR_REAL) ) ) THEN
                        MESG = 'Could not write "'// VNM// '" to file "'//TRIM(ONAME_S)// '".'
                        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
                    END IF
                END IF
    ...
    !.... writing emissions report before adjustemnt
    ...
    IF( CREATE_REPORT ) THEN
        ...
        !.... writing emissions report after adjustemnt, if there is any
...
960 FORMAT(A11,100A17)
    !x901     FORMAT(A4,3I6,3F13.5)
    !x90000   FORMAT(A20,I4,2A16,I10)
        ...
        DO
            ...
            IF( ADJ_FACTOR( F ) < 0 ) THEN
                MESG = 'WARNING: ' // TRIM( ADJ_SPC(F) ) //    &
                   ...
                   ' will be substracted due to a negative adjustment factor'
                CALL M3MSG2( MESG )
