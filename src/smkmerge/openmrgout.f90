...
SUBROUTINE OPENMRGOUT( NGRP )
    ...
    IF( PYEAR .NE. BYEAR )  WRITE( FDESC3D( 6 ),94010 ) '/PROJECTED YEAR/ ', PYEAR
    IF( VARFLAG ) THEN
    ...
    IF( PINGFLAG ) THEN
        ...
        PINGNAME = PROMPTSET( 'Enter name for PING EMISSIONS OUTPUT file',    &
    ...
    IF( INLINEFLAG ) THEN
        ...
        INLINENAME = PROMPTSET( 'Enter name for INLINE EMISSIONS OUTPUT file',    &
    ...
    IF( ELEVFLAG .AND. NGRP .GT. 1 ) THEN
        ...
        WRITE( MESG, 94010 ) 'The number of processing groups is ',    &
               NGRP, ', but it cannot be greater than' //     &
               CRLF()//BLANK10 // '1 for ASCII elevated output.  ' //    &
               'Not enough memory.'
    ...
    IF( SRCGRPFLAG ) THEN
        ...
        SRCGRPNAME = PROMPTMFILE( 'Enter name for OUTPUT SOURCE GROUPS file',    &
                       FSUNKN3, 'SRCGROUPS_OUT', PROGNAME )

        !.....  Set up variables for emissions output file
        CALL SETUP_VARIABLES( NIPPA, NMSPC, EANAM, EMNAM )

        SGINLNNAME = PROMPTSET( 'Enter name for INLINE EMISSIONS OUTPUT file',    &
    ...
    IF( LREPSTA .OR. LREPCNY ) THEN
        ...
        IF( AFLAG ) THEN
            FILEDESC = 'AREA-SOURCE ' // TRIM( RTYPNAM ) // ' REPORT'
            CALL OPEN_LNAME_OR_PNAME( FILEDESC, 'ASCII', AREPNAME, ARDEV )
        END IF

        IF( BFLAG ) THEN
            FILEDESC = 'BIOGENIC ' // TRIM( RTYPNAM ) // ' REPORT'
            CALL OPEN_LNAME_OR_PNAME( FILEDESC, 'ASCII', BREPNAME, BRDEV )
        END IF

        IF( MFLAG ) THEN
            FILEDESC = 'MOBILE-SOURCE '// TRIM( RTYPNAM )// ' REPORT'
            CALL OPEN_LNAME_OR_PNAME( FILEDESC, 'ASCII', MREPNAME, MRDEV )
        END IF

        IF( PFLAG ) THEN
            FILEDESC = 'POINT-SOURCE '// TRIM( RTYPNAM )// ' REPORT'
            CALL OPEN_LNAME_OR_PNAME( FILEDESC, 'ASCII', PREPNAME, PRDEV )
        END IF

        IF( XFLAG ) THEN
            FILEDESC = 'TOTAL '// TRIM( RTYPNAM )// ' REPORT'
            CALL OPEN_LNAME_OR_PNAME( FILEDESC, 'ASCII', TREPNAME, TRDEV )
        END IF
    ...
    SUBROUTINE OPEN_LNAME_OR_PNAME( FILEDESC, FTYPE, LNAME, FDEV      )
        ...
        IF( IOS1 .EQ. 0 .AND.    &
            ...
            MESG = 'NOTE: Opening "' // TRIM( LNAME ) //    &
                   '" file using Smkmerge-defined physical file name.'
            CALL M3MSG2( MESG )
        ...
        IF( .NOT. ENVFLAG ) THEN
            ...
            MESG = 'Enter name for '// FILEDESC
            SELECT CASE( FTYPE )
              CASE( 'NETCDF' )
                LNAME = PROMPTSET( mesg, FSUNKN3, LNAME, PROGNAME )
              CASE( 'ASCII' )
                FDEV  = PROMPTFFILE( MESG, .FALSE., .TRUE., LNAME, PROGNAME )
