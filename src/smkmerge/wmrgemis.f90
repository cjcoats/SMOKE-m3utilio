...
SUBROUTINE WMRGEMIS( VNAME, JDATE, JTIME )
    ...
    !
    !  DESCRIPTION:
    !      The purpose of this subroutine is to write out all I/O API files
    ...
    IF( IOUTFLAG .AND. PINGFLAG )    CALL SAFE_WRITE3( PINGNAME, PGRPEMIS )

    !.....  For plume-in-grid, output file...
    IF( IOUTFLAG .AND. INLINEFLAG )  CALL SAFE_WRITE3( INLINENAME, ELEVEMIS )
...
CONTAINS
    ...
    SUBROUTINE SAFE_WRITE3( FILNAM, EMDATA )
        ...
        IF( .NOT. WRITESET( FILNAM, VNAME, ALLFILES,JDATE,JTIME, EMDATA ) ) THEN
