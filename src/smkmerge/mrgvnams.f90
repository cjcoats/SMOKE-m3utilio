...
SUBROUTINE MRGVNAMS
    ...
    IF( AFLAG )  CALL SET_MASTER_POL_STAT( 'area'    , ANIPOL, AEINAM, EFLAG )
    IF( BFLAG )  CALL SET_MASTER_POL_STAT( 'biogenic', BNIPOL, BEINAM, EFLAG )
    IF( MFLAG )  CALL SET_MASTER_POL_STAT( 'mobile'  , MNIPPA, MEANAM, EFLAG )
    IF( PFLAG )  CALL SET_MASTER_POL_STAT( 'point'   , PNIPOL, PEINAM, EFLAG )
    ...
    IF( AFLAG )  CALL BUILD_NAMES_UNITS( ANIPOL, AEINAM, AONAMES, AOUNITS, TONAMES, TOUNITS )
    IF( BFLAG )  CALL BUILD_NAMES_UNITS( BNIPOL, BEINAM, BONAMES, BOUNITS, TONAMES, TOUNITS )
    IF( MFLAG )  CALL BUILD_NAMES_UNITS( MNIPPA, MEANAM, MONAMES, MOUNITS, TONAMES, TOUNITS )
    IF( PFLAG )  CALL BUILD_NAMES_UNITS( PNIPOL, PEINAM, PONAMES, POUNITS, TONAMES, TOUNITS )
    ...
    IF( ARFLAG )  CALL BUILD_VDESC_UNSORT( NCNT, ARNMSPC, MXSMATV, ARVDESC(JA) )     ! area rctvty
    IF( MRFLAG )  CALL BUILD_VDESC_UNSORT( NCNT, MRNMSPC, MXSMATV, MRVDESC(JM) )     ! mobile rctvty
    IF( PRFLAG )  CALL BUILD_VDESC_UNSORT( NCNT, PRNMSPC, MXSMATV, PRVDESC(JP) )     ! point rctvty
    IF( AFLAG .AND. SFLAG )    &
                  CALL BUILD_VDESC_UNSORT( NCNT, ANSMATV, MXSMATV, ASVDESC )     ! area spectn
    IF( BFLAG )   CALL BUILD_VDESC_UNSORT( NCNT, BNSMATV, MXSMATV, BSVDESC )     ! biogenics
    IF( MFLAG .AND. SFLAG )    &
    ...
    IF( AFLAG .AND. SFLAG )  CALL BUILD_SPECIES_ARRAY( ANSMATV, ASVDESC, ANMSPC, AEMNAM )
    IF( BFLAG )              CALL BUILD_SPECIES_ARRAY( BNSMATV, BSVDESC, BNMSPC, BEMNAM )
    IF( MFLAG .AND. SFLAG )  CALL BUILD_SPECIES_ARRAY( MNSMATV, MSVDESC, MNMSPC, MEMNAM )
    IF( PFLAG .AND. SFLAG )  CALL BUILD_SPECIES_ARRAY( PNSMATV, PSVDESC, PNMSPC, PEMNAM )
    IF( SFLAG )              CALL BUILD_SPECIES_ARRAY(  NSMATV, TSVDESC, NMSPC, EMNAM )
    ...
    SUBROUTINE SET_MASTER_POL_STAT( CATDESC, N, PNAMES, EFLAG )
        ...
        DO I = 1, N
            ...
            ELSE
                ...
                EFLAG = .TRUE.
                MESG = 'ERROR: Variable "' // TRIM( VBUF ) // '" from ' // &
                       CATDESC // ' inventory file is not in ' //    &
