SUBROUTINE SETOUTDATE( G_SDATE, G_STIME, G_NSTEPS,    &
    ...
    !***********************************************************************
    !  subroutine SETOUTDATE body starts at line 90
    !
    !  DESCRIPTION:
    !      This subroutine determines the output date, time, and number
    !      of time steps for the programs Mrggrid and Mrgelev.
    !
    !  PRECONDITIONS REQUIRED:
    !      Arrays containing start date, time, and time steps for each file
    !      If merging different days, already retrieved environment variables
    !
    ...
    !
    !  REVISION  HISTORY:
    !       Created 5/2005 by C. Seppanen based on code from mrggrid.f
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'SETOUTDATE'      ! program name

    !***********************************************************************
    !   begin body of subroutine SETOUTDATE

    !.....  Check if we have logical or physical file names; this will
    !       determine how error messages are printed
    ...
    IF( MRGDIFF ) THEN
        ...
        DO I = 2, NFILE
            ...
            IF( STIME( I ) /= TMPTIME ) THEN
                ...
                WRITE( MESG,94010 ) 'ERROR: Start time ', STIME( I ), 'in file'

                IF( ISLOGNAM ) THEN
                    MESG = TRIM( MESG ) // '"' // TRIM( NAM ) // '"'
                ELSE
                    MESG = TRIM( MESG ) // CRLF() // BLANK10 //TRIM( NAM )
                END IF
        ...
        DO I = 1, NFILE
            ...
            !.....  Check if file contains output start date and
            !       enough data to cover output duration
            IF( .NOT. EFLAG ) THEN
                SECS = SECSDIFF( SDATE( I ), STIME( I ), G_SDATE, G_STIME )
                TMPSTEP = SECS / 3600
                ...
                IF( TMPSTEP > 0 .AND. TMPSTEP < NSTEP( I ) ) THEN
                    ...
                    ELSE
                        ...
                        WRITE( MESG,94010 ) TRIM( MESG ) //                 &
                          CRLF() // BLANK10 // 'contains the requested ' // &
                          'output start date ', G_SDATE, 'and' //           &
                          CRLF() // BLANK10 // 'start time ', G_STIME,      &
                          'but does not contain enough data to ' //         &
                          CRLF() // BLANK10 // 'cover the ' //              &
                          'requested number of time steps', G_NSTEPS, '.'
                        CALL M3MSG2( MESG )

                        WRITE( MESG,94010 ) BLANK5 // 'Data from the start date of the file ',    &
                          SDATE( I ), 'will be used instead.'
                        CALL M3MSG2( MESG )
                    END IF
                ELSE

                    !.....  File doesn't contain output start date and time
                    USEFIRST( I ) = .TRUE.
                END IF
            END IF
        END DO
    ...
    !.....  Otherwise, only merge input data from same day; set start date
    !       and time as latest start date and time in input files and number
    !       of time steps based on shortest input file
    ELSE

        !.....  Generate reference date to compare against files (first day
        !       of year)
        TMPDATE = ( SDATE( 1 )/1000 ) * 1000 + 1

        !.....  Find maximum difference between files and reference date
        SECSMAX = 0
        DO I = 1, NFILE
            SECS = SECSDIFF( TMPDATE, STIME( 1 ), SDATE( I ), STIME( I ) )
            SECSMAX = MAX( SECSMAX, SECS )
        END DO

        !.....  Calculate latest start date and time of all files
        ...
        !.....  Initialize number of output time steps with longest
        !       possible duration
        TMPDATE = G_SDATE
        TMPTIME = G_STIME
        ...
        !.....  Find minimum duration based on already determined start
        !       date and time
        DO I = 1, NFILE
            TMPDATE = SDATE( I )
            TMPTIME = STIME( I )
        ...
        !.....  Calculate shortest number of time steps
