PROGRAM MRGELEV
    ...
    IF( PINGFLAG ) THEN
        ...
        IF( MXFILES /= MXPFILES ) THEN
            MESG =      &
              'ERROR: Number of ASCII elevated files does not match number of' //    &
    ...
    DO J = 1, 2
        ...
        DO I = 1, MXFILES
            ...
            !.....  Open input file
            TDEV = JUNIT()
            OPEN( UNIT=TDEV, FILE=TRIM( LINE ), STATUS='OLD', ACTION='READ', IOSTAT=IOS )
            ...
            IF( J == 2 ) THEN
                FILELNS( K ) = GETFLINE( TDEV, 'Elevated point source file' )
            END IF
        END DO
    ...
    !.....  Open output files
    IF( OUTFLAG ) THEN
        MESG = 'Enter logical name for output ASCII ELEVATED SOURCES file'
        ODEV = PROMPTFFILE( MESG, .FALSE., .TRUE., 'OUTFILE', PROGNAME )
    END IF

    MESG = 'Enter logical name for output BINARY ELEVATED SOURCES file'
    BDEV = PROMPTFFILE( MESG, .FALSE., .FALSE., 'PTSOURCE', PROGNAME )

    !.....  Open report file
    MESG = 'Enter logical name for the MRGELEV REPORT file'
    RPTDEV = PROMPTFFILE( MESG, .FALSE., .TRUE., 'REPMRGELEV', PROGNAME )
    ...
    DO L = 1, G_NSTEPS
        ...
        DO I = 1, NFILES
            ...
            IF( MRGDIFF .AND. USEFIRST( I ) ) THEN
                IDUM    = 0
                TMPSTEP = SEC2TIME( SECSDIFF( G_SDATE, IDUM, JDATE, IDUM ) )
                RDATE   = SDATE( I )
            ...
            READ( TDEV, 93000 ) DUMMY
            LNUM = LNUM + 1
            CALL CHECK_HEADER( DUMMY, 'VERTICAL METHOD', FILENAM( I ), LNUM )
            ...
            READ( TDEV, 93000 ) DUMMY
            LNUM = LNUM + 1
            CALL CHECK_HEADER( DUMMY, 'EMISSIONS VALUES', FILENAM( I ), LNUM )

            READ( TDEV, 93000 ) DUMMY
            LNUM = LNUM + 1
            CALL CHECK_HEADER( DUMMY, 'ALL       ALL            0.000', &
                               FILENAM( I ), LNUM )
        ...
        IF( OUTFLAG ) THEN
            ...
            WRITE( ODEV, 93000 ) 'STD       ALL       EMVALUES  0.        50000.'
            WRITE( ODEV, 93000 ) 'END'
            WRITE( ODEV, 93000 ) 'VERTICAL METHOD'
            WRITE( ODEV, 93000 ) 'STD       ALL       ' // VERTTYPE // ' 0.       10000.'
            WRITE( ODEV, 93000 ) 'END'
        ...
        DO I = 1, NMSPC
            ...
            WRITE( RPTDEV,RPTFMT ) IBD, IBT, SPCNAM( I ),    &
                ( RPTBYFILE( K ), K = 1, NFILES ), RPTFILESUM, RPTASCII, RPTBINARY
    ...
    SUBROUTINE CHECK_HEADER( STRING, VALUE, FILENAM, LNUM )
        ...
        IF( STRING /= VALUE ) THEN
            WRITE( MESG,94010 ) 'ERROR: Header incorrect in file' //    &
                   ...
                   CRLF() // BLANK10 // 'at line ', LNUM
            CALL M3MESG( MESG )
            MESG = BLANK10 // 'Expected string "' // TRIM( VALUE ) //   &
                  '" but read "' // TRIM( STRING ) // '"'
            CALL M3MESG( MESG )
...
94020   FORMAT( 8X, 'at time ', A8 )
    ...
    SUBROUTINE WRITE_REPORT_HEADER
        ...
        DO I = 1, NFILES
            WRITE( BUFFER, '(A,I2,A)' ) BUFFER( 1:LEN ) // 'File ', I, '     ; '
            LEN = LEN + 14
        END DO

        BUFFER = BUFFER( 1:LEN ) // 'Sum         ; ' // 'ASCII       ; Binary      ;'
