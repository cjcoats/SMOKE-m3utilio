...
SUBROUTINE PROCTPRO( NFLAG, METFLAG, METNAME )
    ...
    !*******************************************************************************
    !    DESCRIPTION
    !      Processes (read/sort/filter) GENTPRO-style (ASCII CSV)
    !      cross-references and profiles and MET-based profiles.
    !
    !    PRECONDITIONS REQUIRED:
    !      setenv  TREF            <path for cross-reference file>
    !      setenv  TPRO_MONTHLY    <path for month-of-year profiles file>
    !      setenv  TPRO_WEEKLY     <path for day-of-week   profiles file>
    !      setenv  TPRO_DAILY      <path for day-of-month  pfrofiles file>
    !      setenv  TPRO_HOUR       <path for M3IO met based        profiles file>
    !
    !    INTERNAL SUBROUTINES AND FUNCTIONS:
    !      TIMEFLAGS() sets active month/day-of-week flags and SDATE:EDATE
    !      for the set of episodes being run
    !
    !      CSVPROF() opens and counts CSV-profile files, allocates arguments,
    !      and filters and sorts the input table
    !
    !      CSVDOMP() opens and counts TPRO_DAILY day-of-month CSV-profile files,
    !      allocates arguments, and filters and sorts the input table
    !
    !      CSVOPEN() opens and counts lines and data-lines in the
    !      CSV-file FNAME, rewinding after completion
    !
    !      SORTPRO() sorts profiles for CSVPRROF() and CSVDOMP()
    !
    !      SORTREF() allocates sorted XREF arrays and sorts XREF tables
    !      into them.
    !
    !      ISLEAP()  is TRUE iff its argument date is a leap-year
    !
    !    REVISION HISTORY:
    !       Adapted 7/2014 by Carlie J. Coats, Jr. from SMOKE 3.5.1
    !        RDTREF(), RDTPROF(), and ASGNTPRO() for new GENTPRO CSV profiles
    !        and cross-references
    !       Version 11/2023 by CJC:  USE M3UTILIO, conversion to ".f90",  and
    !       related changes
    !*****************************************************************************/
    !
    ! Project Title: Sparse Matrix Operator Kernel Emissions (SMOKE)
    !        Modeling System
    ! File: @(#)$Id$
    !
    ! COPYRIGHT (C) 2004-2014, Environmental Modeling for Policy Development
    ! All Rights Reserved
    !
    ...
    !.......  Unsorted, sorted cross-reference data structures filtered
    !.......  from the input file
    !.......  Note that week-day, week-end, and all-days references are
    !.......  mapped onto the day-specific MON, ... SUN diurnal-profile
    !.......  references:
    !.......  Doc claims IDs are CHAR*15.  Here, we embed them into CHAR*16
    ...
    !....... Data structures for filtering XREF file by actually-occurring
    !....... and default FIPS and pollutants.
    ...
    !.......  Sort the FIPS codes and defaults, and store them for use in
    !.......  filtering the XREF file:
    ...
    !.......  Set active day-of-week, month-of-year flags for SDATE:EDATE

    CALL TIMEFLAGS( )


    !.......  Open and count TREF file

    CALL M3MSG2( 'Reading temporal cross-reference file...' )
    ANAME = CATEGORY(1:1) // 'TREF'
    XDEV  = CSVOPEN( ANAME, NLINES, NDATA )

    !.......  Allocate/initialize scratch data structures
    ...
    !.......  Read and process the TREF file
    ...
    !.......   Now sort all these tables and check for duplicates:
    ...
    !.......   Read in the relevant profile-tables:
    ...
    !.......  hour-of-day:  all these use TPROF_HOURLY:
    ...
    IF ( METCOUNT .GT. 0 ) THEN           !  met based
        ...
        !.......  Determine which hourly profiles to apply
        ...
        ELSE
            ...
            DO V = 1, NIPPA
                ...
                DO S = 1, NSRC
                    ...
                    !.......  If this source in XREF and this FIP in METFIPS
                    !.......  use subscript into (unsorted) COUNTIES:
    ...
    !.......  Map cross references into profile-subscripts:
    !.......  Use zero-subscript as a sentinel-value for "none exist"
    ...
    !.......  NOTE:  must search N+1 elements of zero-based arrays *IDP(0:N)
    !.......         and then adjust by (-1) to skip element 0
...
CONTAINS
    ...
    SUBROUTINE TIMEFLAGS()
        ...
        ! Set active-month/active day-of-week flags for the time period SDATE:EDATE.
        ! Compute SDATE, EDATE for this set of episodes.
        ...
        DO IT = 1, SIZE( ITDATE )                    !  loop over episodes for this run
            ...
            !.......  because of local-time corrections may need one day on each side
            !        of run-start, run-end:
    ...
    !-----------------------------------------------------------------------------------
    !   Generate "null" profile TFAC( NFIELDS, 0:0 )
    !-----------------------------------------------------------------------------------
    ...
    INTEGER FUNCTION  NULLPROF( NFIELDS, IDSTR, TFAC  )
        ...
        IF ( ISTAT .NE. 0 ) THEN
            WRITE( MESG, '( A, I10 )' ) 'ERROR:  Allocation failure:  STAT=', ISTAT
            CALL M3EXIT( PNAME, 0,0, MESG, 2 )
        END IF
    ...
    INTEGER FUNCTION  CSVPROF( FNAME, NFIELDS, IDSTR, TFAC,    &
        ...
        !  Read FNAME, and sort data onto output arguments,
        !  filtering out data only for ID's in IDLIST
        !
        !  Lines must be ASCII CSV of the form
        !      <character-string ID>, TFAC(1), ..., TFAC(NFIELDS) [comment...]
        !------------------------------------------------------------------------------
        ...
        !.......  body of function CSVPROF()  .......
        !.......  Create sorted-unique list of input IDs:
...
99      CLOSE( FDEV )               !  completed input of this file
    ...
    !------------------------------------------------------------------------------
    !   Generate "null" profile DMFAC( 31,12,0:0 )
    !-----------------------------------------------------------------------------------
    ...
    INTEGER FUNCTION  CSVDOMP( FNAME, SDATE, EDATE, IDSTR, DMFAC,    &
        ...
        !  Open and count the CSV-day-profile file FNAME.
        !  Allocate  both arguments and local arrays.
        !  Read FNAME, and sort data onto output arguments,
        !  filtering out data only for ID's in IDLIST
        !
        !  Lines must be ASCII CSV of the form
        !      <character-string ID>, MONTH, TFAC(1), ..., TFAC(MON_DAYS(MONTH)) [comment...]
        !
        !  Output ID's in IDSTR are TRIM(ID)//MM where MM is the 2-digit month
        !  for the indicated profile-line
        !------------------------------------------------------------------------------
        ...
        !.......  Check if run is impacted by leap year
        LEAPYEAR = .FALSE.
        JTIME = 0
        JDATE = SDATE
        ...
        !.......  Read file:  CKEY = ID//MONTH, and FACS
...
99      CLOSE( FDEV )           !  completed input of this file
    ...
    !.......  Sort/process IDs: Count actually-occurring IDs:
    ...
    INTEGER FUNCTION  CSVOPEN( FNAME, NLINES, NDATA )
        ...
        !  Open, and count lines and data-lines in the CSV-profile file FNAME.
...
11      CONTINUE
    ...
    !------------------------------------------------------------------------------
    !  Sort profile data into output data-structures
    !------------------------------------------------------------------------------
    ...
    !------------------------------------------------------------------------------
    !  Sort XREF data into output data-structures
    !------------------------------------------------------------------------------
    ...
    LOGICAL FUNCTION  SORTREF( TYPE, COUNT, NDIM, IDLIST, KEYLIST, IDSORT, KEYSORT )
        ...
        !......  Always need to allocate tables, even if size is zero:
