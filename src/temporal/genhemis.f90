...
SUBROUTINE GENHEMIS( IGRP, NGRP, NGSZ, JDATE, JTIME, TZONE, DNAME,    &
        ...
        IF( FIREFLAG ) THEN
            IF( .NOT. READ3( DNAME, 'BEGHOUR', ALLAYS3,JDATE,JTIME, STHOUR ) ) THEN
                WRITE(MESG,94010)'WARNING: Could not read "BEGHOUR" from "'// &
                    TRIM( DNAME )//'", at', JDATE, ':', JTIME
                CALL M3MESG( MESG )
                FIREFLAG = .FALSE.
            END IF          !  if read3() failed on dname

            IF( .NOT. READ3( DNAME, 'ENDHOUR', ALLAYS3,JDATE,JTIME, EDHOUR ) ) THEN
    ...
    DO V = 1, NGSZ
        ...
        IF( LHSPOA( V ) .AND. RHFLAG ) THEN
            ...
            IF( .NOT. READ3( HNAME, NAMBUF, ALLAYS3,JDATE,JTIME, EMACH     ) ) THEN

                MESG = 'Could not read "' // TRIM( NAMBUF ) //    &
                       '" from file "' // HNAME //'".'
