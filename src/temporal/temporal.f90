...
PROGRAM TEMPORAL
    ...
    DO II = 1, NTPERIOD
        ...
        EARLYDATE = SDATE
        EARLYTIME = STIME
        CALL NEXTIME( EARLYDATE, EARLYTIME, -( TZMAX - TZONE )*10000 )
        ...
        LATEDATE = EDATE
        LATETIME = ETIME
        CALL NEXTIME( LATEDATE, LATETIME,  -( TZMIN - TZONE )*10000 )
    ...
    IF( DFLAG ) THEN
        ...
        IF( .NOT. DESC3( DNAME ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0,    &
                         'Could not get description of file "' // TRIM( DNAME ) // '"', 2 )
        END IF
    ...
    IF( HFLAG ) THEN
        ...
        IF( .NOT. DESC3( HNAME ) ) THEN
            CALL M3EXIT( PROGNAME, 0, 0,    &
                         'Could not get description of file "' // TRIM( HNAME ) // '"', 2 )
        ENDIF
