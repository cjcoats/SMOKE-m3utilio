...
SUBROUTINE WRTSUP( FDEV, NSRC, NVAR, VARNAM )
    ...
    !
    !  DESCRIPTION:
    !      This subroutine writes the temporal supplemental ASCII file
    !
    !  PRECONDITIONS REQUIRED:
    !      Outfile file is opened
    !      Used modules are populated for use by temporal
    !
    ...
    !
    !  REVISION  HISTORY:
    !       Created by M. Houyoux 10/2001
    !       Modified by C. Coats 07/2014:  new GENTPRO CSV profiles and cross-references
    ...
    INCLUDE 'EMCNST3.h90'       !  emissions parameters
    ...
    !***********************************************************************
    !   begin body of subroutine WRTSUP
    ...
    !.......  Write header with current variables
    ...
    !.......  Create CSV output formats

    OUTFMT = '(A, ",", I8, ",", 2X,    2049( A,  :, "," ) )'       !  supports up through MXVARS3=2048 vbles

    DO S = 1, NSRC

        !.......  Build source charaterisitics
        CALL MKSOURCE( ICAT, S, SOURCE )

        !.......  Search index for inv source with day/hour inventory
        IDY = FIND1( S, NDYSRC, INDXD )
        IHR = FIND1( S, NHRSRC, INDXH )

        !.......  Retrieve profile numbers for all pollutants
        !.......  Implement profile-hierarchy and count active profiles
        ...
        !.......  Write profile information by pollutant
