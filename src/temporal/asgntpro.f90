...
SUBROUTINE ASGNTPRO
    ...
    !***********************************************************************
    !  subroutine body starts at line 145
    !c
    !  DESCRIPTION:
    !      For each source and pollutant or emission type, find the most specific
    !      temporal profile that applies to that source. Do this using the
    !      grouped tables of temporal cross references from RDTREF. The hierarchical
    !      order is defined in this subroutine, and can be determined from the
    !      in-source comments below. Once a profile code has been identified,
    !      search for this code in the temporal profile tables (from RDTPROF) and
    !      save the index to these tables for each source and pollutant.
    !
    ...
    !
    !  REVISION  HISTORY:
    !       Created 1/1999 by M. Houyoux
    !
    !       Revised ??/???? by ??
    !
    !       Version 07/2014 by C.Coats for  new GENTPRO CSV profiles and cross-references
    !       Does source-to-profile mapping for all sources,species.
    !       NOTES:
    !       1) Met-based profiles are managed in PROCTPRO()
    !       2) Uses profile-zero and xref-zero entries as sentinels for
    !        the "profile/xref not supplied" cases.
    ...
    !.......   MODSOURC contains the source ararys
    ...
    !.......  Note that "no pollutant specified" initializes *PDEX arrays where possible;
    !.......  then V=1,...,NIPPA override it
    !.......  BADSTR initialization is retained for non-full-SCC entries
    !.......  whenever FULLSCC is set.  (The logic is considerably simpler
    !.......  to do all the searches, and use BADSTR to force the irrelevant
    !.......  ones to fail.)
    ...
    !***********************************************************************
    !   begin body of subroutine ASGNTPRO
    ...
    !.......  [Met-based profiles are managed in PROCTPRO()]

    !.......  Set category-specific source characteristic combinations
    !.......  Recall that profile-zero is the profile for the "none exist" case
    ...
    SELECT CASE ( CATEGORY )
      ...
      CASE ( 'AREA' )       ! Already set above
        ...
        DO S = 1, NSRC                  !  loop on area sources
            ...
            !.......  First, set all pollutants for pollutant independent part of hierarchy:
            ...
            !.......  Find hour-of-day profile for each day of the week:
            ...
            !.......  Now, overrides for pollutant dependent part of hierarchy:
      ...
      CASE ( 'POINT' )
        ...
        DO S = 1, NSRC                  !  loop on point sources
            ...
            !.......  pollutant-independent search targets:
            ...
            !.......  Note that pollutant dependent and pollutant independent
            !.......  parts of the point source heirarchy search are tangled together:
            ...
            DO V = 1, NIPPA             !  loop on pollutants
                ...
                !.......  Find hour-of-day profiles for each day of the week:
    ...
    !.......  Check that all sources, pollutants have assigned profiles:
