...
SUBROUTINE WRREPHDR( FDEV, RCNT, FILENUM, LH, OUTFMT )
    ...
    !
    !  DESCRIPTION:
    !     This subroutine writes the header lines for a report based on the
    !     settings of the report-specific flags.  The header lines include lines
    !     for identifying the type of report as well as the column headers,
    !     which are set based on the labels for the generic output data columns.
    !     It also determines the column widths
    !
    ...
    CHARACTER(QAFMTL3), INTENT(OUT) :: OUTFMT         ! output record format
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'WRREPHDR'     ! program name

    !***********************************************************************
    !   begin body of subroutine WRREPHDR

    !.......  Initialize output subroutine arguments
    LH     = 0
    OUTFMT = ' '

    !.......  Initialize local variables for current report
    GEO1MISS = .FALSE.
    CNRYMISS = .FALSE.
    STATMISS = .FALSE.
    CNTYMISS = .FALSE.
    ...
    !.......  Initialize report-specific settings
    ...
    !................
    !.......  Pre-process output bins to determine the width of the stack
    !         parameter and variable-length string columns.
    !.......  For country, state, county, SCC names, and SIC names only flag
    !         which ones are being used by the selected sources.
    !................
    PDSCWIDTH = 1
    DO I = 1, NOUTBINS

        !.......  Include geo code level 1 name in string
        ...
        !.......  Include country name in string
    ...
    !................
    !.......  Set the widths of each output column, while including the
    !         width of the column header.
    !.......  Build the formats for the data in each column
    !.......  Build the header as we go along
    !................

    !.......  The extra length for each variable is 1 space and 1 delimiter width
    ...
    IF( RPT_%BYCELL ) THEN
        ...
        !.......  Write format to include both x-cell and y-cell
    ...
    IF( RPT_%BYGEO1NAM ) THEN
        ...
        !.......  For regions in the inventory, get max name width
        NWIDTH = 0
        DO I = 1, NGEOLEV1
            IF( LGEO1USE( I ) ) THEN
        ...
        !.......  If any missing region names, check widths
        IF( GEO1MISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set geo code name column width
    ...
    IF( RPT_%BYCONAM ) THEN
        ...
        !.......  For countries in the inventory, get max name width
        NWIDTH = 0
        DO I = 1, NCOUNTRY
            IF( LCTRYUSE( I ) ) THEN
        ...
        !.......  If any missing country names, check widths
        IF( CNRYMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set country name column width
        IHDRIDX = IHDRCNRY
    ...
    IF( RPT_%BYSTNAM ) THEN
        ...
        !.......  For states in the inventory, get max name width
        NWIDTH = 0
        DO I = 1, NSTATE
            IF( LSTATUSE( I ) ) THEN
        ...
        !.......  If any missing state names, check widths
        IF( STATMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set state name column width
        IHDRIDX = IHDRSTAT
    ...
    IF( RPT_%BYCYNAM ) THEN
        ...
        !.......  For counties in the inventory, get max name width
        NWIDTH = 0
        DO I = 1, NCOUNTY
            IF( LCNTYUSE( I ) ) THEN
        ...
        !.......  If any missing county names, check widths
        IF( CNTYMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set county name column width
        IHDRIDX = IHDRCNTY
    ...
    IF( RPT_%BYMON ) THEN              ! Monthly
        ...
        !.......  Set speciation profiles column width
    ...
    !.......  Road class.  By roadclass can only be true if by source is not
    !         being used.
    IF( RPT_%BYRCL ) THEN
    ...
    !.......  Source characteristics. NOTE - the source characteristics have
    !         already been rearranged and their widths reset based on the
    !         inventory.  The SCC has been removed if its one of the source
    !         characteristics, and NCHARS reset accordingly.
    IF( RPT_%BYSRC ) THEN
        ...
        DO K = MINC, NCHARS
            ...
            !.......  Build source characteristics output format for WRREPOUT
            TMPFMT = CHARFMT
            L  = LEN_TRIM( TMPFMT )
    ...
    !.......  Stack parameters.  +3 for decimal and 2 significant figures
    IF( RPT_%STKPARM ) THEN

        J = LEN_TRIM( HEADERS( IHDRHT ) )
        PWIDTH( 1 ) = 10
        CALL ADD_TO_HEADER( PWIDTH( 1 ), HEADERS( IHDRHT ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 1 ), ATTRUNIT( 6 ), LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRDM ) )
        PWIDTH( 2 ) = 10
        CALL ADD_TO_HEADER( PWIDTH( 2 ), HEADERS( IHDRDM ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 2 ), ATTRUNIT( 7 ), LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRTK ) )
        PWIDTH( 3 ) = 10
        CALL ADD_TO_HEADER( PWIDTH( 3 ), HEADERS( IHDRTK ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 3 ), ATTRUNIT( 8 ), LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRVE ) )
        PWIDTH( 4 ) = 10
        CALL ADD_TO_HEADER( PWIDTH( 4 ), HEADERS( IHDRVE ), LH, HDRBUF )
    ...
    !.......  Fugitive parameters.  +3 for decimal and 2 significant figures
    IF( RPT_%FUGPARM ) THEN

        J = LEN_TRIM( HEADERS( IHDRFUGHT ) )
        PWIDTH( 5 ) = 10
        CALL ADD_TO_HEADER( PWIDTH( 5 ), HEADERS( IHDRFUGHT ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 5 ), ATTRUNIT( 6 ), LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRFUGWD ) )
        PWIDTH( 6 ) = 10
        CALL ADD_TO_HEADER( PWIDTH( 6 ), HEADERS( IHDRFUGWD ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 6 ), ATTRUNIT( 7 ), LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRFUGLN ) )
        PWIDTH( 7 ) = 10
        CALL ADD_TO_HEADER( PWIDTH( 7 ), HEADERS( IHDRFUGLN ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 7 ), ATTRUNIT( 8 ), LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRFUGAN ) )
        PWIDTH( 8 ) = 10
        CALL ADD_TO_HEADER( PWIDTH( 8 ), HEADERS( IHDRFUGAN ), LH, HDRBUF )
    ...
    !.......  Point-source latitude and longitude
    IF( RPT_%LATLON ) THEN

        J = LEN_TRIM( HEADERS( IHDRLAT ) )
        PWIDTH( 1 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 1 ), HEADERS( IHDRLAT ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 1 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRLON ) )
        PWIDTH( 2 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 2 ), HEADERS( IHDRLON ), LH, HDRBUF )
    ...
    !.......  Point-source grid lamber-x&y, and grid utm_x,y,zone
    IF( RPT_%GRDCOR ) THEN

        J = LEN_TRIM( HEADERS( IHDRLAMBX ) )
        PWIDTH( 1 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 1 ), HEADERS( IHDRLAMBX ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 1 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRLAMBY ) )
        PWIDTH( 2 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 2 ), HEADERS( IHDRLAMBY ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 2 ), '     ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRUTMX ) )
        PWIDTH( 3 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 3 ), HEADERS( IHDRUTMX ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 3 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRUTMY ) )
        PWIDTH( 4 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 4 ), HEADERS( IHDRUTMY ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 4 ), '     ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRUTMZ ) )
        PWIDTH( 5 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 5 ), HEADERS( IHDRUTMZ ), LH, HDRBUF )
    ...
    IF( RPT_%GRDPNT ) THEN
        ...
        PWIDTH( 1 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 1 ), HEADERS( IHDRSWLAT ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 1 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRSWLON ) )
        PWIDTH( 2 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 2 ), HEADERS( IHDRSWLON ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 2 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRNWLAT ) )
        PWIDTH( 3 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 3 ), HEADERS( IHDRNWLAT ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 3 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRNWLON ) )
        PWIDTH( 4 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 4 ), HEADERS( IHDRNWLON ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 4 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRNELAT ) )
        PWIDTH( 5 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 5 ), HEADERS( IHDRNELAT ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 5 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRNELON ) )
        PWIDTH( 6 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 6 ), HEADERS( IHDRNELON ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 6 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRSELAT ) )
        PWIDTH( 7 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 7 ), HEADERS( IHDRSELAT ), LH, HDRBUF )
        CALL ADD_TO_HEADER( PWIDTH( 7 ), '    ', LU, UNTBUF )

        J = LEN_TRIM( HEADERS( IHDRSELON ) )
        PWIDTH( 8 ) = 13
        CALL ADD_TO_HEADER( PWIDTH( 8 ), HEADERS( IHDRSELON ), LH, HDRBUF )
    ...
    !.......  Stack group IDs when BY ELEVSTAT (RPT_%BYELEV)
    ...
    !.......  Emissions release point type column
    ...
    IF( RPT_%ORISNAM ) THEN
        ...
        !.......  If any missing ORIS names, check widths
        IF( ORISMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set ORIS name column width
    ...
    IF( RPT_%SCCNAM ) THEN
        ...
        !.......  For SCC descriptions in the inventory, get max name width
        NWIDTH = 0
        DO I = 1, NINVSCC
            IF( LSCCUSE( I ) ) THEN
                J = RPT_%SCCRES
        ...
        !.......  If any missing SCC names, check widths
        IF( SCCMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set SCC name column width
    ...
    IF( RPT_%SICNAM ) THEN
        ...
        !.......  For SIC descriptions in the inventory, get max name width
        NWIDTH = 0
        DO I = 1, NINVSIC
            IF( LSICUSE( I ) ) THEN
        ...
        !.......  If any missing SIC names, check widths
        IF( SICMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set SIC name column width
    ...
    IF( RPT_%GSPRONAM ) THEN
        ...
        !.......  For GSPRO descriptions in the inventory, get max name width
        NWIDTH = 0
        DO I = 1, NGSPRO
            IF( LSPCUSE( I ) ) THEN
        ...
        !.......  If any missing GSPRO desc names, check widths
        IF( SPCMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set GSPRO desc name column width
    ...
    IF( RPT_%MACTNAM ) THEN
        ...
        !.......  For MACT descriptions in the inventory, get max name width
        NWIDTH = 0
        DO I = 1, NINVMACT
            IF( LMACTUSE( I ) ) THEN
        ...
        !.......  If any missing MACT names, check widths
        IF( MACTMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set MACT name column width
    ...
    IF( RPT_%NAICSNAM ) THEN
        ...
        !.......  If any missing NAICS names, check widths
        IF( NAICSMISS ) NWIDTH = MAX( NWIDTH, LEN_TRIM( MISSNAME ) )

        !.......  Set NAICS name column width
    ...
    !.......  Determine the format type requested (if any) - either float or
    !         scientific. Also determine the number of decimal places
    !         requested.
    !.......  The data format will already have been QAed so not need to worry
    !         about that here.
    ...
    !.......  Data values. Get width for columns that use the "F" format instead
    !         of the "E" format.  The code will not permit the user to specify
    !         a width that is too small for the value requested.
    IF( RPT_%NUMDATA .GT. 0 ) THEN
        ...
        IF( RPT_%NUMFILES .EQ. 1 ) THEN
            ...
            IF( RPT_%RPTMODE .EQ. 2 ) THEN
                ...
                ELSE
                    IF( EDIDX + RPT_%RPTNVAR .GT. RPT_%NUMDATA ) THEN
        ...
        DO J = STIDX, EDIDX
            ...
            !.......  Build temporary units fields and get final width
            ...
            IF ( DATFLOAT ) THEN
                ...
                !.......  For database output get max data value for all columns
                IF( RPT_%RPTMODE .EQ. 3 ) THEN
                ...
                ELSE
                    ...
                    !.......  Get maximum data value for this column
                    VAL = MAXVAL( BINDATA( :,J ) )
                ...
                !.......  Store the minimum width of the left part of the format.
                W1 = LEN_TRIM( BUFFER )

                !.......  Increase the width to include the decimal places
                W1 = W1 + NDECI + 1                           ! +1 for decimal point

                !.......  Set the left part of the format.  Compare needed width
                !         with requested width and width of the column header
                !         and units header
                IF( RPT_%RPTMODE .EQ. 3 ) THEN
            ...
            ELSE
                ...
                TMPFMT = OUTFMT
                L2 = LEN_TRIM( TMPFMT )
                WRITE( OUTFMT, '(A,I2.2,A,I2.2)' ) TMPFMT( 1:L2 ) // 'E', W1, '.', NDECI

            END IF

            !.......  Add delimeter to output formats except for last value
            TMPFMT = OUTFMT
            L1 = LEN_TRIM( TMPFMT )
            IF( J .NE. EDIDX ) THEN
                IF( L1 .LT. QAFMTL3-8 ) THEN
                    OUTFMT = TMPFMT( 1:L1 ) // ',"' // RPT_%DELIM // '",1X,'
                ELSE
                    GO TO 988
                END IF
            ...
            !.......  Add next entry to header buffers
            IF( RPT_%RPTMODE .EQ. 3 ) THEN
                CALL ADD_TO_HEADER( W1, HEADERS( IHDRDATA ), LH, HDRBUF )

            ELSE
                CALL ADD_TO_HEADER( W1, OUTDNAM( J,RCNT ), LH, HDRBUF )

            END IF

            !.......  Add next entry to units line buffer
    ...
    !................
    !.......  Write out the header to the report
    !................

    !.......  Write line to separate reports from each other and from metadata
    IF( PDEV == FDEV ) WRITE( FDEV, '(/,A,/)' ) REPEAT( '#', LH )

    !.......  If multifile report, write out number of current file
    ...
    !.......  Source category processed
    WRITE( FDEV,93000 ) '# Processed as ' // TRIM( CATDESC ) // ' sources'
    ...
    !.......  Whether projection factors were applied and for what year
    ...
    !.......  Whether multiplicative control factors were applied
    IF( RPT_%USECUMAT ) THEN
        WRITE( FDEV,93000 ) '# Multiplicative control factors applied'
    END IF

    !.......  Whether a gridding matrix was applied and the grid name
    IF( RPT_%USEGMAT .OR. AFLAG ) THEN
        WRITE( FDEV,93000 ) '# Gridding matrix applied for grid' // TRIM( GRDNM )
    ELSE
        WRITE( FDEV,93000 ) '# No gridding matrix applied'
    END IF

    !.......  Whether a speciation matrix was applied and mole- or mass-based
    ...
    !.......  What pollutant was used for speciation profiles
    IF( RPT_%BYSPC ) THEN
        WRITE( FDEV,93000 )'# Speciation profiles for pollutant "'// TRIM( RPT_%SPCPOL ) // '"'
    END IF

    !.......  Whether hourly data or inventory data were input
    !.......  For hourly data, the time period processed
    IF( RPT_%USEHOUR .OR. AFLAG ) THEN
        ...
        !.......  Compare average day setting in configuration file with what
        !         is actually available in the hourly emissions file.  Give
        !         messages and titles accordingly.
        DYSTAT = .FALSE.
    ...
    !.......  Write normalization status
    IF( RPT_%NORMCELL ) THEN
        WRITE( FDEV, 93000 ) '# Data divided by grid cell areas '//&
                             'based on grid ' // TRIM( GRDNM )
    END IF

    IF( RPT_%NORMPOP ) THEN
        WRITE( FDEV, 93020 ) '# Data divided by year ', STCYPOPYR, ' population'
    END IF

    !.......  The name of the group used to select the data
    IF( RPT_%REGNNAM .NE. ' ' ) THEN
        WRITE( FDEV,93000 ) '# Region group "'//TRIM( RPT_%REGNNAM )//'" applied'
    END IF

    !.......  The name of the subgrid used to select the data
    IF( RPT_%SUBGNAM .NE. ' ' ) THEN
        WRITE( FDEV,93000 ) '# Subgrid "' // TRIM( RPT_%SUBGNAM )//'" applied'
    END IF

    !.......  Column headers  ...........

    IF( RPT_%RPTMODE .NE. 3 ) THEN
        !.......  Remove leading spaces from column units
        L = LEN_TRIM( UNTBUF )
        UNTBUF = UNTBUF( 2:L )
        L = L - 1

        !.......   Write data output units
        WRITE( FDEV, 93000 ) UNTBUF( 1:L )

    END IF

    !.......  Remove leading spaces from column headers
    L = LEN_TRIM( HDRBUF )
    HDRBUF = HDRBUF( 2:L )
    L = L - 1
    ...
    !.......  Store previous output file unit number
    PDEV = FDEV

    !.......  Successful completion of routine
    RETURN

    !.......  Unsuccessful completion of routine
...
94640 FORMAT( '(', 3('1X,F', I2.2, '.5,"', A, '",'), '1X,F', I2.2, '.5,"', A, '")' )

94642 FORMAT( '(1X,F',I2.2,'.8,"', A,'",1X,F',I2.2,'.8,"',A,'")' )      ! lat/lons

94643 FORMAT( '(', 4('1X,F', I2.2, '.2,"', A, '",'), '1X,F', I2.2, '.2,"', A, '")' )
...
CONTAINS
    ...
    !.......  This internal subprogram builds the report header
    SUBROUTINE ADD_TO_HEADER( LCOL, LABEL, LHDR, HDRBUF )
        ...
        !.......  If this is the firstime for this report
        IF( LHDR .EQ. 0 ) THEN

        !.......  Initialize header and its length
            HDRBUF = ' ' // '#' // LABEL
            LHDR   = LCOL + LV
    ...
    INTEGER FUNCTION INTEGER_COL_WIDTH( NVAL, IARRAY )
        ...
        !.......  Find maximum integer value in list
        M = MAXVAL( IARRAY )

        !.......  Write integer to character string
        WRITE( NUMBUF, '(I16)' ) M
