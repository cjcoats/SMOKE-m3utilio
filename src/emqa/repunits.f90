...
SUBROUTINE REPUNITS( RCNT )
    ...
    DO V = 1, NSVARS
        ...
        IF( TOSOUT( V,RCNT )%AGG .GT. 0 ) THEN
            ...
            CALL UPDATE_OUTUNIT( NDATA, TOSOUT( V,RCNT )%SPC,    TMPUNIT, OUTUNIT, UCNVFAC )
            CALL UPDATE_OUTUNIT( NDATA, TOSOUT( V,RCNT )%ETPSPC, TMPUNIT, OUTUNIT, UCNVFAC )
            CALL UPDATE_OUTUNIT( NDATA, TOSOUT( V,RCNT )%PRCSPC, TMPUNIT, OUTUNIT, UCNVFAC )
            CALL UPDATE_OUTUNIT( NDATA, TOSOUT( V,RCNT )%SUMETP, TMPUNIT, OUTUNIT, UCNVFAC )
            CALL UPDATE_OUTUNIT( NDATA, TOSOUT( V,RCNT )%SUMPOL, TMPUNIT, OUTUNIT, UCNVFAC )
            CALL UPDATE_OUTUNIT( NDATA, TOSOUT( V,RCNT )%SUMSPC, TMPUNIT, OUTUNIT, UCNVFAC )
    ...
    DO E = 1, NV
        ...
        IF( TODOUT( E,RCNT )%AGG .GT. 0 ) THEN
            ...
            !.......  Set units and conversion factors for appropriate columns
            CALL UPDATE_OUTUNIT( NDATA, TODOUT( E,RCNT )%ETP, TMPUNIT, OUTUNIT, UCNVFAC )
            CALL UPDATE_OUTUNIT( NDATA, TODOUT( E,RCNT )%DAT, TMPUNIT, OUTUNIT, UCNVFAC )
...
CONTAINS
    ...
    SUBROUTINE UPDATE_OUTUNIT( NREC, OUTCOL, I_UNIT, UNITS, FACTOR )
