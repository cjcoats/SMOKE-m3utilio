...
SUBROUTINE RDRPRTS( FDEV )
    ...
    !
    !  DESCRIPTION:
    !      The RDRPRTS routine reads the report information from the REPCONFIG
    !      and sets report arrays from the MODRPRT module
    !
    ...
    CHARACTER(NAMLEN3) :: CBUF         !  tmp pollutant buffer

    CHARACTER(16), PARAMETER :: PROGNAME = 'RDRPRTS'     ! program name
    ...
    ALLOCATE( ALLRPT( NREPORT ),            &
            ALLOUTHR( 24,NREPORT ),         &
             ALLUSET( MXINDAT, NREPORT ),   &
              INDNAM( MXINDAT, NREPORT ),   &
              TITLES( MXTITLE, NREPORT ), STAT=IOS )
    ...
    !.......  Read lines of file and store report characteristics
    IREC = 0
    DO I = 1, NLINE_RC
        ...
        !.......  Get count of report packets
        N = PKTCOUNT( RPT_IDX )

        !.......  Store settings for current report
        ALLRPT( N ) = RPT_
        ...
        !.......  Conditional settings - only set if current line is type...
        !.......  Title line
        IF( LIN_TITLE   )  TITLES( RPT_%NUMTITLE , N ) = TITLE
    ...
    !.......  If speciation codes selected, allocate memory for pollutants list
    !         and flag to indicate which of the selected pollutants were in the
    !         supplemental speciation file.
    IF ( PSFLAG ) THEN
        ALLOCATE( SPCPOL( NIPOL ),  &
                 LSPCPOL( NIPOL ), STAT=IOS )
    ...
    DO N = 1, NREPORT
        ...
        !.......  Post-process output times and update logical array
        !.......  If reporting "BY HOUR" or if temporal allocation not used for
        !         the report, set output for all "hours" to true.
        IF( ALLRPT( N )%BYHOUR .OR. .NOT. ALLRPT( N )%USEHOUR ) THEN

            ALLOUTHR( 1:24, N ) = .TRUE.

        !.......  Output for a specific hour (set as HHMMSS)
        ELSE
        ...
        IF ( ALLRPT( N )%BYSPC ) THEN
            ...
            !.......  Search for pollutant from REPCONFIG in inventory list
            ...
            !.......  Only allow one pollutant in REPCONFIG file
