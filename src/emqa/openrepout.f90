...
SUBROUTINE OPENREPOUT( FILNAM, FDEV, MDEV )
    ...
    IF( L .LE. 16 ) THEN
        ...
        END IF
    ELSE
        MESG = 'Logical file name "'//TRIM(FILNAM)// '" exceeds max 16 characters'
    ...
    DO I = 1, RPT_%NUMFILES
        ...
        IF( IOS .EQ. 0 ) THEN
            ...
            FDEV( I ) = GETEFILE( FILNAM, .FALSE., .TRUE., PROGNAME )
        ...
        ELSE
            ...
            FDEV( I ) = JUNIT()
            OPEN( FDEV( I ),ERR=1006,FILE=NNAME, STATUS='UNKNOWN',RECL=2500 )
    ...
    IF( RPT_%SRCMAP ) THEN
        ...
        IF( .NOT. SETENVVAR( 'SRC_CROSSWALK', NNAME ) ) THEN
            MESG = 'Could not set logical file name for file ' // TRIM( NNAME )
            CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
        END IF

        MDEV = GETEFILE( 'SRC_CROSSWALK', .FALSE., .TRUE., PROGNAME )
...
1006 WRITE( MESG,94010 ) ' Could not open file:' // CRLF() // BLANK5 // TRIM( FILNAM )
    CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
