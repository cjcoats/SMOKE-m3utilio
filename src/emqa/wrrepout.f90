...
SUBROUTINE WRREPOUT( FDEV, RCNT, NDATA, JDATE, JTIME,       &
    ...
    !
    !  DESCRIPTION:
    !    The WRREPOUT routine outputs the report information, depending on the
    !    specifications of the output reports and the contents of the bins.C
    !
    !  PRECONDITIONS REQUIRED:
    !    The output file FDEV is opened
    !    The column widths and internal write formats have been created
    !    The output delimeter has been specified,
    !    The report count RCNT is set
    !    The bins have been populated with characteristics and data values
    !    The number of data values NDATA has been set
    !    The output date string has been set, if needed
    !
    ...
    USE MODLISTS, ONLY: SCCDESC, SCCDLEV, SICDESC, MACTDESC, NAICSDESC
    ...
    USE MODSTCY, ONLY: CTRYNAM, STATNAM, CNTYNAM, NORIS, ORISDSC, GEOLEV1NAM

    !.......  This module contains the information about the source category
    USE MODINFO, ONLY: MXCHRS, NCHARS, BYEAR

    !.......  MODULES for I/O API INTERFACEs, geo-transform codes:
    ...
    !.......  Create hour for output
    OUTHOUR = JTIME / 10000 + 1

    !.......  Width of delimeter
    LV = LEN_TRIM( DELIM )

    !.......  When a new report is starting...
    IF( RCNT .NE. PRCNT ) THEN

        !.......  Transfer array info to scalar info for this report
    ...
    !.......  Loop through variables for the database format
    DO V = 1, NDATA

        !.......  Loop through entries for all bins for current date and hour
        DO I = 1, NOUTBINS

            !.......  Check for zero emissions if flag is not set
            ...
            !......  Include user-defined label in string
            ...
            !.......  Include date in string for CARB QADEF report
            IF( RPT_%CARB ) THEN

                !.......  Temporal resolution (A:Annual, D:Daily, H:Hourly)
                ...
                !.......  Compute year, month, julidan day, day of week (1,,,,,,7)
                YEAR = JDATE / 1000
                IF( AGT == 'A' ) THEN
                    MONTH = 0
                    WDAY = 0
            ...
            !.......  Include date in string
            IF( RPT_%BYDATE .AND. .NOT. RPT_%CARB ) THEN

                !.......  Get month and day from Julian date
                CALL DAYMON( JDATE, MONTH, DAY )
            ...
            !.......  Include country/state/county code in string
            IF( LREGION ) THEN
            ...
            !.......  Include region level 1 name in string
            IF( RPT_%BYGEO1NAM ) THEN
                J = BINGEO1IDX( I )
                L = GEO1WIDTH
                L1 = L - LV - 1                                    ! 1 for space
                IF( J .LE. 0 ) THEN
                    STRING = STRING( 1:LE ) // BADRGNM( 1:L1 ) // DELIM
                ELSE
                    STRING = STRING( 1:LE ) // GEOLEV1NAM( J )( 1:L1 ) // DELIM
                END IF
            ...
            IF( RPT_%BYCONAM ) THEN
                ...
                IF( J .LE. 0 ) THEN
                    STRING = STRING( 1:LE ) // BADRGNM( 1:L1 ) // DELIM
                ELSE
                    STRING = STRING( 1:LE ) // CTRYNAM( J )( 1:L1 ) // DELIM
                END IF
            ...
            IF( RPT_%BYSTNAM ) THEN
                ...
                IF( J .LE. 0 ) THEN
                    STRING = STRING( 1:LE ) // BADRGNM( 1:L1 ) // DELIM
                ELSE
                    STRING = STRING( 1:LE ) // STATNAM( J )( 1:L1 ) // DELIM
                END IF
                MXLE = MXLE + L
                LE = MIN( MXLE, STRLEN )
            END IF
            ...
            IF( RPT_%BYCYNAM ) THEN
                ...
                IF( J .LE. 0 ) THEN
                    STRING = STRING( 1:LE ) // BADRGNM( 1:L1 ) // DELIM
                ELSE
                    STRING = STRING( 1:LE ) // CNTYNAM( J )( 1:L1 ) // DELIM
                END IF
                MXLE = MXLE + L
                LE = MIN( MXLE, STRLEN )
            END IF

            !.......  Include SCC code in string
            IF( RPT_%BYSCC ) THEN
                L = SCCWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                TSCC = BINSCC( I )
            ...
            IF( RPT_%BYSIC ) THEN
                L = SICWIDTH
                L1 = L - LV - 1
                STRING = STRING( 1:LE ) // BINSIC( I )( 1:MIN(L1,SICLEN3) ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include INTEGRATE code in string
            IF( RPT_%BYINTGR ) THEN
                L = INTGRWIDTH
                L1 = L - LV - 1                                ! 1 for space
                STRING = STRING( 1:LE ) // BININTGR( I )( 1:MIN(L1,INTLEN3) ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include MACT code in string
            IF( RPT_%BYMACT ) THEN
                L = MACTWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // BINMACT( I )( 1:MIN(L1,MACLEN3) ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include NAICS code in string
            IF( RPT_%BYNAICS ) THEN
                L = NAIWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // BINNAICS( I )( 1:MIN(L1,NAILEN3) ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include SRCTYP code in string
            IF( RPT_%BYSRCTYP ) THEN
                L = STYPWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE+8 ) // BINSRCTYP( I )( 1:L1-8 ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include primary surrogate code
            ...
            !.......  Include fallback surrogate code
            ...
            !.......  Include monthly temporal profile
            IF( RPT_%BYMON ) THEN
                L = MONWIDTH
            ...
            !.......  Include weekly temporal profile
            IF( RPT_%BYWEK ) THEN
                L = WEKWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINWEKID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include day of month temporal profile
            IF( RPT_%BYDOM ) THEN
                L = DOMWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINDOMID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Monday diurnal temporal profile
            IF( RPT_%BYMND ) THEN
                L = MNDWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINMNDID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Tuesday diurnal temporal profile
            IF( RPT_%BYTUE ) THEN
                L = TUEWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINTUEID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Wednesday diurnal temporal profile
            IF( RPT_%BYWED ) THEN
                L = WEDWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINWEDID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Thursday diurnal temporal profile
            IF( RPT_%BYTHU ) THEN
                L = THUWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINTHUID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Friday diurnal temporal profile
            IF( RPT_%BYFRI ) THEN
                L = FRIWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINFRIID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Saturday diurnal temporal profile
            IF( RPT_%BYSAT ) THEN
                L = SATWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINSATID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Sunday diurnal temporal profile
            IF( RPT_%BYSUN ) THEN
                L = SUNWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINSUNID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Met-based diurnal temporal profile
            IF( RPT_%BYMET ) THEN
                L = METWIDTH
                L1 = L - LV - 1 - 1                              ! 1 for space
                STRING = STRING( 1:LE ) // ' ' // BINMETID( I )( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include speciation profile
            IF( RPT_%BYSPC ) THEN
                L = SPCWIDTH
                L1 = L - LV - 1 - SPNLEN3                              ! 1 for space
                STRING = STRING( 1:LE ) // BINSPCID( I )// BLANK16( 1:L1 )// DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF
            ...
            IF( RPT_%BYPLANT ) THEN
                L = CHARWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // BINPLANT( I )( 1:L1 ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF
            ...
            IF( RPT_%BYUNIT ) THEN
                L = UNITIDWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // BINUNITID( I )( 1:L1 ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include road class code
            IF( RPT_%BYRCL ) THEN
            ...
            !.......  Include source characteristics
            IF( RPT_%BYSRC ) THEN
                S = BINSMKID( I )

                !.......  Disaggregate source characteristics
                CALL PARSCSRC( CSOURC( S ), MXCHRS, LOC_BEGP, LOC_ENDP, LF, NC, CHARS )
            ...
            !.......  Include ORIS code in string
            IF( RPT_%BYORIS ) THEN
                L = ORSWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // BINORIS( I )( 1:MIN(L1,ORSLEN3) ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include Boiler code in string
            IF( RPT_%BYBOILER ) THEN
                L = ORSWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // BINBOILER( I )( 1:MIN(L1,ORSLEN3) ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF
            ...
            !.......  Include lat/lons for point sources
            IF( RPT_%LATLON ) THEN
                S = BINSMKID( I )
                BUFFER = ' '
            ...
            !.......  Include grid lambert/utm coordinates for point source
            IF( RPT_%GRDCOR ) THEN
                ...
                CALL GRID2XY( LATGRD3, 0.0D0, 0.0D0, 0.0D0, 0.0D0, 0.0D0,   &
                              ...
                              1, 1, DXORIG, DYORIG, XCELL, YCELL, X0, Y0 )
            ...
            !.......  Include grid cell corner coordinates
            IF( RPT_%GRDPNT ) THEN
                ...
                BUFFER = ' '
                WRITE( BUFFER, LLGRDFMT )       &
                    SWLAT, SWLON, NWLAT, NWLON, NELAT, NELON, SELAT, SELON
                STRING = STRING( 1:LE ) // BUFFER
            ...
            !.......  Include elevated sources flag
            IF( RPT_%BYELEV ) THEN
                L = ELEVWIDTH
                L1 = L - LV  - 2                  ! 1 for space, minus 1 for how used
                STRING = STRING( 1:LE ) // BLANK16( 1:L1 ) // BINELEV( I ) // DELIM
                MXLE = MXLE + L + LX
                LE = MIN( MXLE, STRLEN )
                LX = 0
            END IF

            !.......  Include stack group IDs when elevated sources flag = Y
            ...
            !.......  Include emissions release point type
            ...
            IF( RPT_%SRCNAM ) THEN
                ...
                L = PDSCWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // CPDESC( S )( 1:L1 ) // DELIM
                MXLE = MXLE + L
                LE = MIN( MXLE, STRLEN )
            END IF

            !.......  Include ORIS description
            !.......  This is knowingly including extra blanks before final quote
            ...
            IF( RPT_%SCCNAM ) THEN
                ...
                STRING = STRING( 1:LE ) // SCCDESC( J )( 1:K ) // REPEAT( " ", L1-K ) // DELIM
                MXLE = MXLE + L + 2
                LE = MIN( MXLE, STRLEN )

                !.......  Write warning msg when the description is unavailable
                N = INDEX( SCCDESC( J ), 'Description unavailable' )
                MESG = 'WARNING: Description of SCC ' // TRIM(BINSCC( I )) // ' is not available'
            ...
            IF( RPT_%SICNAM ) THEN
                ...
                L = SIDSWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // '"'// SICDESC( J )( 1:L1 )// '"' // DELIM
                MXLE = MXLE + L + 2
                LE = MIN( MXLE, STRLEN )

                !.......  Write warning msg when the description is unavailable
                N = INDEX( SICDESC( J ), 'Description unavailable' )
                MESG = 'WARNING: Description of SIC ' // BINSIC( I ) // ' is not available'
                IF ( N .GT. 0 ) CALL M3MESG( MESG )
            ...
            IF( RPT_%GSPRONAM ) THEN
                ...
                L = SPDSWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // '"'// GSPRDESC( J )( 1:L1 )// '"' // DELIM
                MXLE = MXLE + L + 2
                LE = MIN( MXLE, STRLEN )

                !.......  Write warning msg when the description is unavailable
                N = INDEX( GSPRDESC( J ), 'Description unavailable' )
                MESG = 'WARNING: Description of GSPRO ' // BINSPCID( I ) // ' is not available'
                IF ( N .GT. 0 ) CALL M3MESG( MESG )
            ...
            IF( RPT_%MACTNAM ) THEN
                ...
                L = MACDSWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // '"'// MACTDESC( J )( 1:L1 )// '"' // DELIM
                MXLE = MXLE + L + 2
                LE = MIN( MXLE, STRLEN )

            !.......  Write warning msg when the description is unavailable
                N = INDEX( MACTDESC( J ), 'Description unavailable')
                MESG = 'WARNING: Description of MACT ' // TRIM(BINMACT( I )) // ' is not available'
                IF ( N .GT. 0 ) CALL M3MESG( MESG )
            ...
            IF( RPT_%NAICSNAM ) THEN
                ...
                L = NAIDSWIDTH
                L1 = L - LV - 1                                    ! 1 for space
                STRING = STRING( 1:LE ) // '"'// NAICSDESC( J )( 1:L1 )// '"' // DELIM
                MXLE = MXLE + L + 2
                LE = MIN( MXLE, STRLEN )

                !.......  Write warning msg when the description is unavailable
                N = INDEX( NAICSDESC( J ),'Description unavailable')
                MESG = 'WARNING: Description of NAICS ' // TRIM(BINNAICS( I )) // ' is not available'
                IF ( N .GT. 0 ) CALL M3MESG( MESG )
            ...
            IF( RPT_%RPTMODE .EQ. 3 ) THEN
                ...
                L = VARWIDTH
                L1 = L - LV
                STRING = STRING( 1:LE ) // OUTDNAM( V, RCNT )( 1:L1 ) // DELIM
                MXLE = MXLE + L
                LE = MIN( MXLE, STRLEN )

                L = UNITWIDTH
                L1 = L - LV
                STRING = STRING( 1:LE ) // OUTUNIT( V )( 1:L1 ) // DELIM
                MXLE = MXLE + L
                LE = MIN( MXLE, STRLEN )

            END IF

            !.......  Remove leading spaces and get new length
            STRING = STRING( 2:LE )
            LE = LE - 1

            !.......  Output error message of string is getting shortened
            ...
            !.......  If current bin has bad values, update those now.
            ...
            IF( RPT_%NUMFILES .EQ. 1 ) THEN
                ...
                IF( RPT_%RPTMODE .EQ. 2 ) THEN
                    ...
                    ELSE
                        IF( EDIDX + RPT_%RPTNVAR .GT. NDATA ) THEN
            ...
            IF( RPT_%RPTMODE .NE. 3 ) THEN
                ...
                WRITE( FDEV, OUTFMT ) STRING( 1:LE ), ( BINDATA( I,J ), J=STIDX, EDIDX )
    ...
    !.......  Save report number for next time routine is called
777 PRCNT = RCNT
