...
SUBROUTINE ASGNBINS( RCNT )
    ...
    !
    !  DESCRIPTION:
    !       The ASGNBINS routine is responsible for assigning a bin number to each
    !       output record.  A bin is a group of records for which the emissions,
    !       activity, or emission-type data will be summed to provide a single
    !       record in a report.
    !
    ...
    !
    !       Version 9/2014 by C Coats:  promote MXOUTREC to INTEGER*8 for CARB;
    !       OpenMP parallel; incremental construction of SORTBUF; use SORTINC8();
    !       major cleanup of post-sort data reorganization; construction of
    !       binning matrices MODREPBN:<NBINS,ISRCB,GFACB>
    !
    !       Version 10/2022 by Christos Efstathiou: Added BY BOILER and
    !       by UNIT ID reports
    !
    ...
    !......  MODLISTS contains the lists of unique source characteristics
    !......  MODREPRT contains Smkreport-specific settings
    !......  MODREPBN contains report arrays for each output bin
    !......  MODGRID contains the global variables for the 3-d grid
    !......  MODELEV contains arrays for plume-in-grid and major sources
    !......  MODSTCY contains the arrays for state and county summaries
    ...
    !***********************************************************************
    !   begin body of subroutine ASGNBINS
    ...
    !.......  Memory check to check exceeding integer4 maxval=2,147,483,647
    IF( MXOUTREC < 1 ) THEN
        MESG = 'ERROR: Problem processing the size of inventory'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF

    !.......  Consistency checking:  inventory vs report
    ...
    !.......  Allocate (and deallocate) memory for sorting arrays
    ...
    !.......  Create sorting arrays SORTIDX  SORTBUF for all output records
    !.......  Add fields to SORTBUF in sorting-order
    !.......  [Really wish this could be one parallel loop 1...NOUTREC
    !.......  but the previous-plant dependency prevents this.
    !.......  parallel-loop; plant-loop; parallel-loop instead;-( ]
    ...
    DO I = 1, NOUTREC
        ...
        !.......  Reporting by plant doesn't assume that the plant ID is unique across counties.
        !.......  When building the sorting array, the SRCID for the first source of a plant/county
        !.......  combination will be used for all sources at that plant. This happens later in the
        !.......  code, so for now save space for the SRCID.
    ...
    !.......  Assign bins to output records based on sorting array
    !.......  NOTE:  sequential dependency for B
    ...
    !.......  If memory is allocated for bin arrays, then deallocate
    ...
    !.......  Populate the bin characteristic arrays (not the data array)
    ...
    DO B = 1, NOUTBINS
        ...
        !.......  Store country name index. Note for population that some
        !         form of "by region is required"
        ...
        IF( RPT_%BYCONAM ) THEN
            ...
            IF ( RPT_%NORMPOP ) THEN
                ...
                !.......  If population data unavailable, then flags bins
                !         that will not be able to have normalization by pop.
                ELSE
        ...
        !.......  Store state name index. Note for population that some
        !         form of "by region is required"
        ...
        IF( RPT_%BYSTNAM ) THEN
            ...
            !.......  If using population normalization, reset with state population
        ...
        !.......  Store county name index. Note for population that some
        !         form of "by region is required"
        ...
        IF( RPT_%BYCYNAM ) THEN
            ...
            !.......  If using population normalization, reset with county population
        ...
        !.......  Store SCC, SIC, MACT, NAICS, ORIS name index
        !.......  (for full name, regardless of SCC truncation.
        !.......  Note: have confirmed that using the OUTSRC(J)
        !.......  index with CSCC maps to SCC from BUFFER properly)
