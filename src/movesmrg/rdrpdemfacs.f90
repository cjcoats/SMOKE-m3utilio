...
SUBROUTINE RDRPDEMFACS( REFIDX, MONTH )
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDRPDEMFACS'        ! program name
    ...
    !***********************************************************************
    !   begin body of subroutine RDRPDEMFACS
...
100 CONTINUE
    ...
    IF( TOGIDX == 0 .AND. NHTOGIDX == 0 ) THEN
        MESG = 'ERROR: Emission factors file does not contain data for both TOG/NONHAPTOG pollutants'
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    END IF
    ...
    !.......  Assumptions:
    !             File will contain data for all SCCs in the inventory and
    !               all 16 speed bins.
    !             Lines are sorted by:
    !                 temperature
...
200     CONTINUE
    ...
    IF( ALLOCATED( RPDEMFACS ) )    DEALLOCATE( RPDEMFACS )
    IF( ALLOCATED( EMTEMPS ) )      DEALLOCATE( EMTEMPS )

    ALLOCATE( RPDEMFACS( NINVSCC, 16, NEMTEMPS, NMVSPOLS ), &
                EMTEMPS( NEMTEMPS ),                        &
    ...
    DO
        ...
        !.......  Set speed bin for current line
