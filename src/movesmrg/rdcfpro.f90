...
SUBROUTINE RDCFPRO( CFDEV )
    ...
    !
    !  DESCRIPTION:
    !       Reads control factor data from CFPRO file
    !
    !  PRECONDITIONS REQUIRED:
    !       CFDEV must be opened
    !
    !  SUBROUTINES AND FUNCTIONS CALLED:  none
    !
    !  REVISION  HISTORY:
    !       06/12: Created by Dongmei Yang
    !       06/15: Updated by BH Baek
    ...
    CHARACTER(NAMLEN3) POLNAM, SPCNAM, CBUF, CPOL, BLKPOL, FILPOL      ! current pollutant-species name
    CHARACTER(FPSLEN3+CASLEN3+2) DUPTMP, CFIPSCC, BLKFIPSCC        ! tmp FIPS code // SCC : CHRT06

    CHARACTER(16), PARAMETER :: PROGNAME = 'RDCFPRO'        ! program name

    !***********************************************************************
    !   begin body of subroutine RDCFPRO
    ...
    IF( FIRSTIME ) THEN
        ...
        !.......  Allocate locate FIPS/SCC hierarchy arrays
        ...
        ALLOCATE( CHRT01( NCHRT01 ),&
                  ...
                  CHRT18( NCHRT18 ),&
                  CHRT19( NCHRT19 ),&
                  CHRT20( NCHRT20 ), STAT=IOS )
        CALL CHECKMEM( IOS, 'CHRT01...CHRT20', PROGNAME )
        ...
        !.......  Build FIPS and SCC hierarchy tables
        BLKMON = '00'
        FILMON = 'MM'
        BLKFIP = '000000000000'
        ...
        ALLOCATE(  CFPRO( NINVIFIP, NINVSCC, NIPPA+NMSPC, 12 ),&
             CFIPSCCLIST( NLINES ),&
                  INDXCF( NLINES ),&
                DUPCHECK( NLINES ),&
                  NLFIPS( NINVIFIP ),&
                  ...
                  NLPOLS( NIPPA+NMSPC ),&
                  NLMONS( 12 ),         STAT=IOS )
        CALL CHECKMEM( IOS, 'CFPRO...NLMONS', PROGNAME )
    ...
    !.......  Processing a list of valid CFPRO entries
    DO N = 1, 24         ! six hierarchy
        ...
        DO I = 1, NXREF
            ...
            !.......  Convert FIP to integer
            K = 0
            !.......  State-level is not applicable when REF_CFPRO_YN is set to Y
            IF( CFIP == BLKFIP  ) THEN
                IF( REFCFFLAG ) THEN
            ...
            ELSE                     ! FIPS is not zero
                ...
                IF( L1 > 0 ) THEN
                    ...
                    !.......  Propagate reference-county-specific control factor to inventory counties.
                    IF( REFCFFLAG ) THEN
                        ...
                        !.......  find ref county and apply CF to ref-inventory counties
                        DO J = 1, NINVC
            ...
            !.......  check no of fips/scc/poll/mon
            IF( NFIPS < 1 .OR. NSCCS < 1 .OR. NPOLS < 1 .OR. NMONS < 1 )  CYCLE

            !.......  check and get up control factor values
