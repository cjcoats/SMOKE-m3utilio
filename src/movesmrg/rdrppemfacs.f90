...
SUBROUTINE RDRPPEMFACS( REFIDX, MONTH )
    ...
    INCLUDE 'MVSCNST3.h90'      !  MOVES contants
    ...
    CHARACTER(16), PARAMETER :: PROGNAME = 'RDRPPEMFACS'        ! program name
    ...
    !***********************************************************************
    !   begin body of subroutine RDRPPEMFACS
    ...
    IF( IOS .NE. 0 ) THEN
        MESG = 'ERROR: Could not open emission factors file ' // FILENAME
        CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
    ELSE
        MESG = 'Reading emission factors file ' // FILENAME
        CALL M3MESG( MESG )
    END IF
...
100 CONTINUE
    ...
    !               both day values (2 and 5), and all 24 hours.
    !             Inventory will only have SCCs without road types.
    !             Lines are sorted by:
    !                 temperature profile
    !                 day value
    !                 SCC (matching sorting of INVSCC)
    !                 emission process (matching sorting of MVSPPROCS)
    !                 hour
    !             Each SCC will have data for same set of emission processes, temperatures, and
    !               pollutants.

    !.......  Limitations:
    !             If inventory doesn't contain every SCC but emission factors file
    !               does, the program will quit with an error.
    !             Program doesn't know if emission factors file is missing values.

    !.......  Expected columns:
    ! MOVESScenarioID,yearID,monthID,dayID,hourID,countyID,SCCsmoke,smokeProcID,temperature,THC,NMHC ...
...
200     CONTINUE
    ...
    !.......  Allocate memory to store emission factors
    IF( ALLOCATED( RPPEMFACS ) ) THEN
        DEALLOCATE( RPPEMFACS )
    END IF
    ALLOCATE( RPPEMFACS( 2, NINVSCC, 24, NEMTEMPS, NMVSPOLS ),    &
    ...
    IF( ALLOCATED( EMXTEMPS ) )  DEALLOCATE( EMXTEMPS )
    IF( ALLOCATED( EMTEMPIDX ) ) DEALLOCATE( EMTEMPIDX )

    ALLOCATE( EMTEMPS( NEMTEMPS ),&
             EMXTEMPS( NEMTEMPS ),&
            EMTEMPIDX( NEMTEMPS ), STAT=IOS )
    CALL CHECKMEM( IOS, 'EMTEMPS...EMTEMPIDX', PROGNAME )
    ...
    DO
        ...
        IF( TSCC .NE. PSCC ) THEN
            ...
            !.......  Skip over SCCs that start with 223 since rate-per-profile
            !                   emissions don't apply
            DO
                SCCIDX = SCCIDX + 1
        ...
        !.......  Set profile index for current line
        TPROFID = TRIM( SEGMENT( 1 ) )
        IF( TPROFID .NE. PPROFID ) THEN
            PROFIDX = PROFIDX + 1
        ...
        !.......  Check min and max temperatures for profile
...
300 CONTINUE
    ...
    !.......  Sort temperature profiles by min temps then max temps
